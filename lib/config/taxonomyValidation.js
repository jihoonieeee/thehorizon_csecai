 ...
 *   - ai_enabled_roles must be in AE01–AE10
...
export function validateThreatTag(rawTag, source = {}, opts = {}) {
  ...
  if (!quote || quote.trim().length < 15) {
    ...
    caveat_if_any = !quote || quote.trim().length === 0
      ...
      : "supporting quote too short (< 15 chars)";
...
export function validateAiEnabledOverlay(raw = {}) {
  ...
  let aiEnabled = raw.ai_enabled === true;

  // 4.5 — AI is only an offensive tool if ≥1 valid offensive AE role is named.
  // ai_enabled=true with zero roles is unsubstantiated (the common false-positive
  // when AI is the TARGET, not the attacker's tool) → hard downgrade to false.
  if (aiEnabled && roles.length === 0) {
    aiEnabled = false;
    caveats.push("ai_enabled=true but no valid ai_enabled_roles provided — downgraded to false");
  }

  // When AI is not the offensive tool, it carries no offensive roles/capabilities.
  const finalRoles = aiEnabled ? roles : [];
  const finalCaps  = aiEnabled ? caps  : [];

  // 4.6 — automation/autonomy cannot be elevated above "unknown" without a
  // capability/role signal. An "autonomous"/"multi_agent" label with no AE role
  // and no ai_capability is an inference, not evidence → reset to "unknown".
  const hasAutonomySignal = roles.length > 0 || caps.length > 0;
  let automation_level = AUTOMATION.has(raw.automation_level) ? raw.automation_level : "unknown";
  let autonomy_level   = AUTONOMY.has(raw.autonomy_level)   ? raw.autonomy_level   : "unknown";
  if (!hasAutonomySignal) {
    if (automation_level !== "unknown") {
      caveats.push("automation_level not evidenced (no role/capability) — set to unknown");
      automation_level = "unknown";
    }
    if (autonomy_level !== "unknown") {
      caveats.push("autonomy_level not evidenced (no role/capability) — set to unknown");
      autonomy_level = "unknown";
    }
  }

  return {
    valid: caveats.length === 0,
    ai_enabled: aiEnabled,
    ai_enabled_roles: finalRoles,
    ai_capabilities: finalCaps,
    automation_level,
    autonomy_level,
    caveats,
  };
