 ...
 *   const { system, user } = loadPrompt('validation/source-typing');
 ...
 *
 * @param {string} name — e.g. "validation/source-typing"
...
/**
 * Extract a fenced block that sits under an arbitrary heading.
 * Used for shared prompt fragments that are interpolated into other prompts
 * (e.g. "scoring/reading-value" under a "## Rubric" heading).
 *
 * @param {string} name — prompt file name without .md
 * @param {string} heading — heading text without the leading "## "
 * @returns {string} fenced block content, trimmed
 */
export function loadPromptSection(name, heading) {
  const raw = loadPromptRaw(name);
  const escaped = heading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const match = raw.match(new RegExp(`## ${escaped}\\s*\\n\`\`\`([\\s\\S]*?)\`\`\``));
  if (!match) {
    throw new Error(`Prompt section not found: ${name} → "## ${heading}"`);
  }
  return match[1].trim();
}
