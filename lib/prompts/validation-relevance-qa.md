...
- summary
- ai_threat_focus verdict ("central" = a genuine OFFENSIVE AI/ML finding; "adjacent" = genuinely about AI cyber-security but landmark REFERENCE context, not an offensive finding; "passing" = only mentions AI incidentally; "none" = unrelated)
- source_type

CHECK:
1) summary_grounded — Is the summary accurate and fully supported by the excerpt, with no invented facts and no filler? 
2) verdict_correct — Is the ai_threat_focus verdict correct?
   - A source that only name-drops "AI" or a model while being about an unrelated breach, product, or non-AI vulnerability must be "passing", not "central"/"adjacent".
   - A source genuinely about an AI/ML attack, vulnerability, abuse, or AI-enabled offensive capability must be "central".
   - A source that is centrally about AI cyber-security but is landmark REFERENCE context rather than an offensive finding — an authoritative framework/standard/taxonomy (OWASP, NIST AI 100-2, MITRE ATLAS, SAIF, NSA/CISA guidance), a dual-use autonomous CAPABILITY milestone (DARPA AIxCC, Google Big Sleep), a standalone defensive method/framework, or a landmark survey/SoK — must be "adjacent", NOT "central" and NOT "passing"/"none".
...
OUTPUT FORMAT:
{
  ...
  "corrected_ai_threat_focus": "central" | "adjacent" | "passing" | "none",
...
- "corrected_is_ai_threat" should be true only when corrected_ai_threat_focus is "central" (an offensive finding), and false for "adjacent", "passing", and "none".
