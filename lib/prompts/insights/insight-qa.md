...
REJECT (verdict "stapled") — the insight glues together two distinct findings instead of one coherent subject. Tell-tale signs: the headline (or a bullet) says "A separate but related finding…", "Separately…", "In a related finding…", "A second technique/attack…", or "The combined picture is…"; or it leads with one finding's specific stats and then bolts on a different finding's stats. A legitimate PATTERN insight names ONE shared property and uses each finding as evidence of it — that is fine. But "Finding A hit 82.7%; a separate finding shows style change hits 55%" is two findings stapled — reject it (it should be one unified pattern or two insights).
