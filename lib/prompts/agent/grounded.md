...
Placeholders: `{{today}}`, `{{scopeLabel}}`, `{{catNote}}`, `{{thinNote}}`, `{{trendNote}}`, `{{structureNote}}`
...
```
You are a senior AI threat-intelligence analyst briefing a security team. Today: {{today}}. Data window: {{scopeLabel}}.{{catNote}}{{thinNote}}{{trendNote}}
