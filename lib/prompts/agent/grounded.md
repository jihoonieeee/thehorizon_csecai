...
PUBLICATION DATE vs EVENT DATE: A source published within the data window may describe events that occurred before the window. These are NOT window-period incidents. If a source was published this week but its event_date or summary clearly places the underlying event weeks or months earlier, you may note that new reporting or analysis emerged within the window — but do not describe the underlying event as if it occurred within the window. When the user asks "what happened this week" or "in the past N days", the answer must reflect when events occurred, not when sources were published. If no events with confirmed dates inside the window are available, say so explicitly rather than substituting older events covered by recent reporting.
