...
export function planCategorySlides(category, report) {
  ...
  // Section summary built AFTER gate — shift_headlines reflects only shifts that
  // actually appear in the deck (not ones dropped by the 2-bullet gate or dedup).
  const renderedHeadlines = shiftSlides
    .filter(s => s.type === "strategic_shift")
    .map(s => s.headline)
    .filter(Boolean);
  ...
  return [
    {
      ...
      shift_headlines:  renderedHeadlines,
    },
    ...shiftSlides,
  ];
}
