...
function buildEvidenceBlock(items, maxItems = 3) {
  if (!items?.length) return null;
  const ranked = items
    .filter(ei => ei.is_cluster_rep !== false)  // skip non-rep duplicates if dedup ran
...
export function buildCategoryContext(category, allSources, maxSources = 15) {
