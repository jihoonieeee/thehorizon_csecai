...
const MATURITY_LABEL = {
  operational: "OPERATIONAL — repeated or at-scale adversary use",
  observed:    "OBSERVED — used against real-world targets",
  validated:   "VALIDATED — confirmed against a real system, no adversary use",
  research:    "RESEARCH — research, simulation, or lab-only finding",
};
...
// Primary intelligence publishers: government agencies, major threat intel firms,
// and AI labs reporting on their own incidents. These are the only sources that
// can support state-actor/APT attribution at confidence moderate or higher.
// Secondary aggregators (Cyware, news outlets) have the same importance_tier
// ("realized") but are not primary intel for attribution purposes.
//
// TWO separate checks:
//   isPrimaryIntel   — used for maturity ceiling (trusted trust_tier=primary + URL patterns)
//   isMajorTISource  — used for attribution authority (URL patterns ONLY, no trust_tier bypass)
//     Rationale: trust_tier=primary may be mis-tagged in the DB for vendors/aggregators.
//     Attribution authority requires direct-reporting provenance — only the URL list can guarantee that.
const PRIMARY_INTEL_PATTERNS = [
  /cisa\.gov/, /ncsc\.gov\.uk/, /csa\.gov\.sg/, /nist\.gov/, /nsacyber/,
  ...
  /securelist\.com/,         // Kaspersky GReAT
  /sentinelone\.com\/labs/,
  /recorded-future\.com/,
  /secureworks\.com\/blog/,
  /talos-intelligence\.com/,
  /ibm\.com\/security/,      // IBM X-Force
  /anthropic\.com\/news/,    // primary when reporting on own incidents
  /openai\.com\/.*security/, // primary when reporting on own incidents
  /hiddenlayer\.com\/research/,
  /mitre\.org/,
  /zscaler\.com\/blogs\/security-research/, // Zscaler ThreatLabz original research
  /blog\.checkpoint\.com/,
  /research\.checkpoint\.com/,
];

// url may be null (curated/PDF imports carry no URL) — a default parameter only
// covers undefined, so coerce explicitly the way norm() does.
function isPrimaryIntel(url, publisher = "", trustTier = "") {
  if (trustTier === "primary") return true; // CSA/CISA/NIST already tagged primary
  const u = (url || "").toLowerCase();
  return PRIMARY_INTEL_PATTERNS.some(re => re.test(u));
}

// Attribution authority check — URL patterns ONLY. Used by the attribution cap
// in generateSlides.js to avoid trust_tier=primary mis-tags on aggregators/vendors
// incorrectly bypassing the cap for nation-state/APT attribution claims.
export function isMajorTISource(url) {
  const u = (url || "").toLowerCase();
  return PRIMARY_INTEL_PATTERNS.some(re => re.test(u));
}
...
const INSIGHT_MATURITY_TO_SLIDE = {
  ...
  validated:   "disclosed_vulnerability",
...
export function buildCategoryContext(category, selectedSources, clusterContext = null, insightPoints = null) {
  ...
  const entries = pool.map((s, i) => {
    ...
    sourceIndex[label] = {
      ...
      is_primary_intel: isPrimaryIntel(s.url, s.publisher, s.trust_tier),
    };
