...
async function gateEntailment(report, sourceIndex, opts = {}) {
  ...
  for (const shift of report.strategic_shifts || []) {
    ...
    const outcomes = await Promise.all(evs.map(async (ev) => {
      if (!ev.cited_sources?.length) return { keep: true, ev };   // uncited — resolveCitations handles it
      const grounding = groundingFor(ev.cited_sources);
      if (!grounding) return { keep: true, ev };                  // no resolvable text — don't judge on absence
      ...
      // Two-pass (entailment judgment is stochastic): take the WORST verdict of
      // two passes (unsupported > correctable > ok). ok → keep; correctable →
      // rewrite the fact to the source-supported version; unsupported → drop.
      const runCheck = async () => {
        ...
        } catch { return { verdict: "ok", correction: null, reason: "" }; }  // non-blocking
      };
      const a = await runCheck();
      const b = a.verdict === "ok" ? await runCheck() : a;        // 2nd pass only if 1st cleared
      ...
      // unsupported, or correctable with no usable correction → drop
