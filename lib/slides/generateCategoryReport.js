...
const CATEGORY_SCOPE = {
  ...
  agentic_ai_threats: {
    ...
    in_scope: "agent goal hijack via untrusted inputs or retrieved content, tool/function-call abuse, MCP-server and plugin/skill-marketplace compromise, memory/context poisoning, coding-agent supply-chain attacks (poisoned rules/hooks/config files), agent privilege escalation through misused tool permissions.",
    out_of_scope: "single-shot prompt injection against a chatbot with no tools (LLM Threats); classical ML-model attacks (Traditional); AI-generated attack content (AI-Enabled). CRITICAL BOUNDARY: incidents where an AI agent ACTS AS the offensive actor — autonomously exploiting vulnerabilities, escaping evaluation sandboxes, or breaching production infrastructure without being manipulated by an external attacker — belong in AI-Enabled Threats. Classify by what the agent DID, not where it was deployed. An agent hijacked by a poisoned config file is Agentic (agent is victim). An agent that autonomously exploits a zero-day and pivots to a production server is AI-Enabled (agent is attacker). If unsure, place in AI-Enabled.",
  },
  ai_enabled_threats: {
