...
function checkVisualizationSpecs(checks, analytics) {
  ...
  const REQUIRED_SPEC_FIELDS = ["visualization_id","visualization_type","title","chart_data"];

  const malformed = specs.filter((s) => REQUIRED_SPEC_FIELDS.some((f) => !s[f]));
...
function checkAnalyticsEvidence(checks, analytics) {
  ...
  const REQUIRED_FIELDS = ["dimension","category","top_entries","insight"];
  const malformed = evidence.filter((e) => REQUIRED_FIELDS.some((f) => !e[f]));
  check(checks, "evidence_items_well_formed", malformed.length === 0,
    malformed.length > 0
      ? `${malformed.length} evidence items missing required fields`
      : `All ${evidence.length} evidence items are well-formed`);

  const categories = new Set(evidence.map((e) => e.category));
  check(checks, "evidence_covers_threat_patterns", categories.has("threat_pattern"),
    "analytics_evidence includes threat_pattern category", false);

  check(checks, "evidence_covers_maturity", categories.has("maturity"),
    "analytics_evidence includes maturity category", false);
}
