...
export const VALID_ADVERSARY_ADOPTION_STAGES = new Set([
  "none","experimentation","limited_use","operational_use","scaled_use","unknown",
]);

export const VALID_CAPABILITY_STAGES = new Set([
  "concept","demonstrated","benchmarked","operationally_plausible","operationalized","unknown",
]);
...
adversary_adoption_stage (only for threat_intelligence/adversary_adoption_signal): none | experimentation | limited_use | operational_use | scaled_use | unknown

capability_stage (only for research_finding/benchmark_evaluation/capability_demonstration): concept | demonstrated | benchmarked | operationally_plausible | operationalized | unknown
...
const TYPE_TO_ADVERSARY_ADOPTION = {
  adversary_adoption_signal: "limited_use",
  threat_intelligence:       "experimentation",
  incident:                  "operational_use",
};

const TYPE_TO_CAPABILITY_STAGE = {
  research_finding:          "demonstrated",
  benchmark_evaluation:      "benchmarked",
  capability_demonstration:  "operationally_plausible",
  exploit_disclosure:        "operationally_plausible",
  incident:                  "operationalized",
  threat_intelligence:       "operationalized",
  vulnerability:             "operationally_plausible",
};
...
function buildUserPrompt(source) {
  ...
  const frameworkTags = [
    ...(u.primary_threat_tags || []).slice(0, 4).map((t) => `${t.tag}(${t.domain || "?"})`),
    ...(u.secondary_dimensions || []).slice(0, 3).map((d) => (typeof d === "string" ? d : d.tag)),
  ].join(", ");
...
// Deterministic taxonomy features from the source's validated understanding.
// These power the taxonomy-aware aggregations. Secondary dimensions are kept in
// their OWN field and are deliberately NOT part of primary_threat_tags, so they
// never inflate primary-threat frequency.
function taxonomyFeatures(source) {
  const u = source.understanding || {};
  const ptt = (u.primary_threat_tags || []).filter((t) => t && t.tag);
  const validated = ptt.filter((t) => t.validation_status === "validated");
  return {
    primary_domain:        source.primary_domain || u.primary_domain || source.main_category || "unknown",
    primary_threat_tags:   validated.map((t) => t.tag),
    primary_threat_tags_all: ptt.map((t) => t.tag),
    parent_tags:           [...new Set(validated.map((t) => t.parent_tag).filter(Boolean))],
    agentic_subdomains:    [...new Set(validated.filter((t) => t.domain === "agentic_ai_threats" && t.subdomain).map((t) => t.subdomain))],
    ai_enabled_tags:       validated.filter((t) => t.domain === "ai_enabled_threats").map((t) => t.tag),
    prompt_injection_subtypes: validated.filter((t) => t.parent_tag === "prompt_injection").map((t) => t.tag),
    secondary_dimensions:  (u.secondary_dimensions || []).map((d) => (typeof d === "string" ? d : d.tag)).filter(Boolean),
    taxonomy_validation_status: u.validation_status || "needs_manual_review",
  };
}
