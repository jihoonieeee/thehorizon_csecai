...
- attack_surface_signal: dependency type, trust-boundary shift type, AI layer, technology, attack surface
...
adversary_adoption_stage (only for threat_intelligence/adversary_adoption_signal/incident/exploit_disclosure):
  ...
  Rules: incident/exploit_disclosure/threat_intelligence can support limited_operational_use or above. research_finding can support research_claimed. governance/attack_surface signals → use none_observed or unknown.
...
dependency_types (only for attack_surface_signal): model_api | tool_integration | mcp_server | plugin | data_pipeline | cloud_service | open_source_model | foundation_model | unknown

trust_boundary_shift_types (only for attack_surface_signal): authority_delegation | human_oversight_reduction | automated_decision_making | cross_system_trust | implicit_trust_assumption | unknown
...
const TYPE_TO_OPERATIONAL_STATUS = {
  ...
  attack_surface_signal:            "limited_operational_use",
};
...
function buildDeterministicFeatures(source) {
  ...
  if (rt.recurring_themes && rt.recurring_themes.length) {
    ...
    if (st === "attack_surface_signal") { themeSet.add("trust_boundary_failure"); themeSet.add("dependency_growth"); }
  ...
  const dependency_types = (st === "attack_surface_signal")
    ? ["unknown"] : [];

  const trust_boundary_shift_types = (st === "attack_surface_signal")
    ? ["unknown"] : [];
...
function toAnalyticsTaxonomy(source, features) {
  ...
  return {
    ...
    rawfact_strength:           source.rawfact_evidence_summary?.strongest_strength || null,
