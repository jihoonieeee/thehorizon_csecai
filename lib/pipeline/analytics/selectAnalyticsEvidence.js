...
function threatFrequencyEvidence(aggregates) {
  ...
  if (topTags.length > 0) {
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "frequency_distribution",
      finding:        `The most frequent validated primary threat tag is '${topTags[0].key}' (${topTags[0].count} sources).`,
  ...
  for (const [domain, catMat] of Object.entries(ma.category_by_operational_status || {})) {
    ...
    if (catActive > 0) {
      items.push({
        analytics_evidence_id: aeId(),
        evidence_type:  "maturity_distribution",
        finding:        `For ${domain}: ${catActive} of ${catTotal} sources show active operational status within the corpus.`,
...
function adversaryAdoptionEvidence(aggregates) {
  ...
  items.push({
    ...
    finding:        `Within the ${total} adversary-type sources (TI, incident, adoption signal, exploit), ${activeCount} show limited-or-greater adversary adoption of AI capabilities.`,
...
function capabilityPipelineEvidence(aggregates) {
  ...
  items.push({
    analytics_evidence_id: aeId(),
    evidence_type:  "maturity_distribution",
    finding:        `Within the corpus, ${rtp} research/benchmark sources show research-to-PoC progression, ${p2o} show PoC-to-operational signals, and ${watchlist} are on the capability watchlist.`,
    data:           {
...
function coverageGapEvidence(aggregates) {
  ...
  if (gaps.length > 0) {
    items.push({
      ...
      finding:        `${gaps.length} frequently observed attack vector(s) have no corresponding defensive-capability source in the corpus.`,
  ...
  for (const flag of (matrix.thin_coverage_flags || [])) {
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "coverage_gap",
      finding:        `For ${flag.domain}: ${flag.reason}`,
      data:           flag,
      source_ids:     [],
...
function timelineEvidence(aggregates) {
  ...
  for (const flag of (trend.insufficient_trend_data || [])) {
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "timeline",
      finding:        `Insufficient data for trend claim on '${flag.key}': ${flag.reason}`,
      data:           flag,
      source_ids:     [],
      ...
      source_types:   [],
      confidence:     "low",
      caveat_if_any:  `Need ≥ 3 non-zero monthly buckets for a trend direction claim.`,
    });
  }
...
function derivedIndexEvidence(derived_metrics) {
  ...
  for (const [key, metric] of Object.entries(derived_metrics || {})) {
    ...
    if (metric.label === "very_high" || metric.label === "high") {
      items.push({
        analytics_evidence_id: aeId(),
        evidence_type:  "derived_index",
        finding:        `${metric.index_id}: score ${metric.score}/100 (${metric.label}) — ${metric.explanation || metric.formula || ""}`,
        data:           {
...
function structuralSignalEvidence(aggregates) {
  ...
  if (govTotal > 0) {
    ...
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "frequency_distribution",
      finding:        `Within the corpus, ${govTotal} governance signal sources were collected${topFn ? `; most common governance function: '${topFn[0]}'` : ""}.`,
      data:           gov,
      source_ids:     [],
  ...
  if (defTotal > 0) {
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "frequency_distribution",
      finding:        `Within the corpus, ${defTotal} defensive capability sources were collected.`,
  ...
  if (ecoTotal + tbTotal > 0) {
    items.push({
      analytics_evidence_id: aeId(),
      evidence_type:  "frequency_distribution",
      finding:        `Within the corpus, ${ecoTotal} infrastructure/ecosystem dependency sources and ${tbTotal} trust boundary shift sources were collected.`,
      data:           {
      ...
      domain:         null,
      source_types:   ["infrastructure_dependency_signal","trust_boundary_shift","ecosystem_signal"],
