...
function buildTrendAnalytics(features, monthlyDomainCounts) {
  ...
  for (const domain of ["traditional_ai_threats","llm_threats","agentic_ai_threats","ai_enabled_threats"]) {
    ...
    if (nonZero >= MIN_BUCKETS && srcTypeCount >= MIN_SOURCE_TYPES) {
      ...
      if (maxCount > avg * 2 && nonZero < MIN_BUCKETS) {
        // A burst from a single source type (or one publication wave) reflects
        // PUBLICATION VOLUME, not threat activity. Label it accordingly so it is
        // never read as a real-world spike (5B.2).
        const publicationDriven = srcTypeCount < MIN_SOURCE_TYPES;
        burst_clusters.push({
          key: domain,
          type: "domain",
          ...
          source_type_count: srcTypeCount,
          source_ids: [],
          is_publication_cluster: publicationDriven,
          caveat_if_any: publicationDriven
            ? "Publication cluster — concentrated in a single source type, so it reflects publication volume, not real-world threat activity."
            : "Burst pattern, not a trend — needs ≥ 3 non-zero months to qualify as a trend.",
        });
      } else {
  ...
  return {
    ...
    // Input traceability: which source IDs fed these trend calculations
    input_source_ids: features.map((f) => f.source_id).filter(Boolean),
    // Analytics claim note — must be carried with any trend claim from this output
    claim_scope_note: "corpus-scoped: describes volume in collected sources, not real-world prevalence",
  };
}
...
// Keyword-fallback / unmapped classifications (taxonomy_validation_status =
// needs_manual_review / no_tags_found / no_domain_match) are classification
// GUESSES, not evidenced taxonomy tags. They still count toward corpus totals,
// but are excluded from TAXONOMY-derived frequency charts (attack vector / surface /
// AI layer) so classification bias is not presented as real-world prevalence.
function isTaxonomyChartEligible(f) {
  const s = f.taxonomy_validation_status;
  return s !== "needs_manual_review" && s !== "no_tags_found" && s !== "no_domain_match";
}
...
function buildCategoryAnalytics(features) {
  ...
  for (const cat of ALL_CATEGORIES) {
    ...
    // Tag-derived frequencies use only sources with evidenced taxonomy tags.
    const tagFeatures = catFeatures.filter(isTaxonomyChartEligible);
    const excludedFromTagCharts = catFeatures.length - tagFeatures.length;

    byCategory[cat] = {
      count:                    catFeatures.length,
      tag_charts_source_count:  tagFeatures.length,
      excluded_from_tag_charts: excludedFromTagCharts,
      ...
      attack_vector_frequency:  sortedCounts(countByArray(tagFeatures, (f) => f.attack_vectors)),
      attack_surface_frequency: sortedCounts(countByArray(tagFeatures, (f) => f.attack_surfaces)),
      ai_layer_frequency:       sortedCounts(countByArray(tagFeatures, (f) => f.ai_layers)),
      ...
      top_vectors:              topN(countByArray(tagFeatures, (f) => f.attack_vectors), 5),
...
export function aggregateAnalytics(sources) {
  ...
  // ── Deduplication: count unique independent events, not raw source count ────
  // Multiple outlets reporting the same incident from the same primary origin
  // are NOT independent events — they inflate trend/adoption counts. Group by
  // primary_origin_url when available, else by source_id.
  function countUniqueOrigins(features) {
    const seen = new Set();
    let count = 0;
    for (const f of features) {
      const key = f.primary_origin_url || f.source_id;
      if (key && !seen.has(key)) { seen.add(key); count++; }
      else if (!key) count++; // unknown origin still counts
    }
    return count;
  }
  ...
  const advFeaturesDeduped = flagDuplicateReporting(advFeatures);
  const uniqueAdoptionOriginCount = countUniqueOrigins(advFeatures);
  const duplicateAdoptionCount = advFeaturesDeduped.filter((f) => f.duplicate_reporting).length;

  if (duplicateAdoptionCount > 0) {
    adoption_caveats.push(`${duplicateAdoptionCount} adoption source(s) share primary origin with another source — counted only once in unique event totals.`);
  }
  ...
  const adversary_adoption_analytics = {
    ...
    adversary_adoption_evidence:         advFeaturesDeduped.filter(
      ...
      // Deduplication fields — downstream trend/adoption claims must use unique counts
      duplicate_reporting: f.duplicate_reporting || false,
      origin_cluster_id:   f.origin_cluster_id || null,
      primary_origin:      !f.duplicate_reporting,
    })),
    adoption_caveats,
    // Deduplication summary — trend and adoption claims MUST use unique_event_count, not total
    unique_event_count:                  uniqueAdoptionOriginCount,
    duplicate_reporting_count:           duplicateAdoptionCount,
    input_source_ids:                    advFeatures.map((f) => f.source_id).filter(Boolean),
