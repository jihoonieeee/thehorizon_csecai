...
// Build trend analytics with 3-bucket enforcement
function buildTrendAnalytics(features, monthlyDomainCounts) {
  ...
  // Identify signals with enough buckets to support trend claims
  const emerging_signals = [];
  const persistent_signals = [];
  const insufficient_trend_data = [];

  for (const [domain, monthData] of Object.entries(monthlyDomainCounts)) {
    const nonZero = months.filter((m) => (monthData?.[domain] || 0) > 0).length;
    // Check each domain key, not the outer domain
    // Actually monthlyCounts structure is { "YYYY-MM": { domain: count } }
    // so we check monthlyDomainCounts[month][domain]
    const _ = nonZero; // used above; iterate per key instead
  }

  // Per-domain trend sufficiency
  for (const domain of ["traditional_ai_threats","llm_threats","agentic_ai_threats","ai_enabled_threats"]) {
    const nonZero = months.filter((m) => (monthlyDomainCounts[m]?.[domain] || 0) > 0).length;
    if (nonZero >= MIN_BUCKETS) {
      const allCounts = months.map((m) => monthlyDomainCounts[m]?.[domain] || 0);
      const avg = allCounts.reduce((s, v) => s + v, 0) / allCounts.length;
      const last = allCounts[allCounts.length - 1] || 0;
      if (last > avg * 1.2) emerging_signals.push(domain);
      ...
      insufficient_trend_data.push({
        ...
        reason: `Only ${nonZero} non-zero month bucket(s); need ≥ ${MIN_BUCKETS} for a trend claim`,
      });
    }
  }
...
export function aggregateAnalytics(sources) {
  ...
  // governance_signal, ecosystem_signal, strategic_signal must NOT be used alone.
