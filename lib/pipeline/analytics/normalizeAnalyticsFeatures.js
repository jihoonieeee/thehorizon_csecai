...
import {
  VALID_PRIMARY_TAGS, VALID_SECONDARY_DIMENSIONS, DOMAINS, AGENTIC_SUBDOMAINS,
...
export function normalizeSourceAnalyticsFeatures(source) {
  ...
  let normalised = {
    ...
    // ── Validated AI Threat Taxonomy features ──
    // Secondary dimensions are kept separate and NOT in primary_threat_tags, so
    // taxonomy aggregations never count them as primary-threat frequency.
    ...
    primary_threat_tags:       keepValid(raw.primary_threat_tags, VALID_PRIMARY_TAGS),
    parent_tags:               keepValid(raw.parent_tags, VALID_PRIMARY_TAGS),
    agentic_subdomains:        [...new Set((raw.agentic_subdomains || []).filter((s) => AGENTIC_SUBDOMAINS.includes(s)))],
    ai_enabled_tags:           keepValid(raw.ai_enabled_tags, VALID_PRIMARY_TAGS),
    prompt_injection_subtypes: keepValid(raw.prompt_injection_subtypes, VALID_PRIMARY_TAGS),
    secondary_dimensions:      keepValid(raw.secondary_dimensions, VALID_SECONDARY_DIMENSIONS),
