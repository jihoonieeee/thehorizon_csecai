...
// Deterministic. Counts how a category's evidence distributes across the threat
// lifecycle (realized / proven / research), from each item's source_type.
function maturitySplit(evidenceItems = []) {
  const split = { realized: 0, proven: 0, research: 0, other: 0 };
  for (const ei of evidenceItems) {
    const r = realityOf(ei);
    if (r in split) split[r]++; else split.other++;
  }
  // dominant = the most-real bucket that has any evidence (realized > proven > research)
  const dominant = split.realized ? "realized" : split.proven ? "proven" : split.research ? "research" : "none";
