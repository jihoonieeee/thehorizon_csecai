...
import { DOMAIN_COLOURS, buildEvidenceGraph } from "./analysis/corpusSummary.js";
import { DOMAINS, tagsForDomain, buildTaxonomyPromptBlock } from "./understand/taxonomy.js";
...
// ── Threat maturity split ─────────────────────────────────────────────────────
// Counts how a category's evidence distributes across the threat lifecycle,
// using maturity_level (the LLM-assessed threat lifecycle field).
const MATURITY_BUCKET = {
...
function maturitySplit(evidenceItems = []) {
  const split = { operational: 0, observed: 0, disclosed: 0, demonstrated: 0, research: 0, other: 0 };
  for (const ei of evidenceItems) {
    const m = maturityOf(ei);
    if (m in split) split[m]++; else split.other++;
  }
  // dominant = the highest-maturity bucket that has any evidence
  const dominant =
    split.operational ? "operational" :
    split.observed    ? "observed"    :
    split.disclosed   ? "disclosed"   :
    split.demonstrated? "demonstrated":
    split.research    ? "research"    : "none";
  return { ...split, dominant };
}
...
export function buildDashboardState(runResult) {
  ...
  // ── Category cards ─────────────────────────────────────────────────────────
  // Accepts both new shape (insights[]) and legacy shape (judgments[])
  const category_cards = category_analyses
    .filter(ca => ca.category !== "unclear_or_adjacent")
    .map(ca => {
      // New shape uses insights[]; legacy uses judgments[]
      const approved = ca.insights?.length
        ? (ca.insights || []).filter(i => !i.blocked)
        : (ca.judgments || []).filter(j => !j.blocked);
      ...
      const catRepEvidence = evidence_items.filter(
        ei => ei.category === ca.category && ei.is_cluster_rep
      );
      const evCount  = catRepEvidence.length;
      // Threat maturity: where this category's evidence sits on the lifecycle. A
      // category whose evidence is mostly `realized` is a live threat; one that is
      // mostly `research` is still emerging. Deterministic — derived from each
      // evidence item's source_type via the shared importance reality facet.
      const threat_maturity = maturitySplit(catRepEvidence);
      ...
      return {
        ...
        threat_maturity: threat_maturity,
        judgment_count:  approved.length,

        // Top insight/judgment for the card headline (supports both shapes)
        top_insight: top ? {
          insight_id:    top.insight_id || top.judgment_id,
          title:         top.title || top.short_takeaway || top.judgment?.slice(0, 80),
          evidence_maturity: top.evidence_maturity,
          cited_sources: top.cited_sources || [],
        } : null,
        // Legacy alias
        top_judgment: top ? {
          judgment_id:   top.insight_id || top.judgment_id,
          short_takeaway: top.title || top.short_takeaway || top.judgment?.slice(0, 80),
          evidence_ids:  top.evidence_item_ids || top.evidence_for || [],
        } : null,

        // Insights for the drilldown panel (new shape)
        insights: approved.map(i => ({
          insight_id:       i.insight_id || i.judgment_id,
          title:            i.title || i.judgment,
          what_changed:     i.what_changed,
          mechanism:        i.mechanism || i.causal_mechanism,
          implication:      i.implication || i.why_this_matters,
          evidence_maturity: i.evidence_maturity,
          confidence:       i.confidence,
          technique_tags:   i.technique_tags || i.technique_focus || [],
          monitoring_signal: i.monitoring_signal ||
            (i.monitoring_signals?.[0]?.signal) || "",
          caveats:          i.caveats || [],
          cited_sources:    i.cited_sources || [],
        })),

        coverage_gaps: ca.coverage_gaps || ca.evidence_gaps || [],
      };
    });
...
// Prompts loaded from lib/prompts/dashboard/
let _queryParsePrompts = null;
let _querySynthPrompts = null;
function getQueryParsePrompts() {
  if (!_queryParsePrompts) {
    const raw = loadPrompt("dashboard/query-parse");
    _queryParsePrompts = {
      system: interpolate(raw.system, { taxonomy_block: buildTaxonomyPromptBlock() }),
      user:   raw.user,
    };
  }
  return _queryParsePrompts;
}
function getQuerySynthPrompts() {
  if (!_querySynthPrompts) _querySynthPrompts = loadPrompt("dashboard/query-synthesize");
  return _querySynthPrompts;
}

async function parseQuery(question) {
  const { system: sys, user: userTmpl } = getQueryParsePrompts();
  const usr = interpolate(userTmpl, { question });
...
async function synthesiseAnswer(question, retrievedItems, queryParams) {
  ...
  const { system: sys, user: userTmpl } = getQuerySynthPrompts();
  const usr = interpolate(userTmpl, {
    question,
    intent:          queryParams.intent,
    time_range_days: queryParams.time_range_days || 90,
    evidence_count:  retrievedItems.length,
