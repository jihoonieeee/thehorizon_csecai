 ...
 *
 * Specialist extractor for major AI capability announcements (new models,
 * APIs, features from OpenAI, Google, Microsoft, Anthropic, Meta, etc.).
 *
 * Key differences from generic extraction:
 *  - claim_epistemic_type is critical: distinguishes marketing_claim from
 *    observed_fact and analyst inference (misuse pathways)
 *  - Focus on: what's newly possible, who can access it, what safeguards are
 *    stated, what adversary cost reduction this creates
 */
...
const _prompt = loadPrompt("extraction/extract-evidence-capability");
const SYS     = _prompt.system;
const USR_TPL = _prompt.user;

const EVIDENCE_TYPES  = ["incident","capability_demonstration","attack_surface_signal","research_finding",
                         "vulnerability","statistical_measurement","expert_assessment","policy_or_standard"];
...
function normaliseItems(raw, source) {
  ...
  return items
    ...
    .map((ei, idx) => {
      ...
      // Inference items won't have a quote — mark them not grounded.
      ...
      return {
        ...
        source_family:  "major_capability_announcement",
        fact:           String(ei.fact || "").slice(0, 500),
        quote,
        quote_grounded:  actuallyGrounded,
        evidence_type:   EVIDENCE_TYPES.includes(ei.evidence_type)
          ? ei.evidence_type : "capability_demonstration",
        specificity:     ["high","medium","low"].includes(ei.specificity) ? ei.specificity : "medium",
        landscape_change: ei.landscape_change === true,
        ...
        claim_epistemic_type: VALID_EPISTEMIC.includes(ei.claim_epistemic_type)
          ? ei.claim_epistemic_type : "marketing_claim",
...
/**
 * @param {object} source - Post-understand source object (source_family=major_capability_announcement)
 * @param {object} [opts]
 * @returns {Promise<object[]>} Evidence items
 */
export async function extractCapabilityEvidence(source, opts = {}) {
