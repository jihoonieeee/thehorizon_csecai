...
export async function extractEvidence(source, opts = {}) {
  // Digest child fast path — runs BEFORE isEligible.
  //
  // Children whose full_text is the parent's HTML (set when understandAllSources
  // fetches the parent URL during date recovery) would otherwise pass isEligible
  // and run LLM extraction over the entire roundup, extracting sibling findings
  // as cross-contamination. The structured report_finding built during fanout is
  // already the right atomic evidence unit; use it unconditionally when present.
  //
  // Secondary effect: children whose focused text is < 600 chars (below the
  // isEligible threshold) still get one clean evidence item from report_finding,
