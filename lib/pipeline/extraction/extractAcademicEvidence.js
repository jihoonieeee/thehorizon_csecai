 ...
 */
// One LLM attempt (routedLLM → callLLM fallback) for a given text slice.
async function attemptExtraction(usr) {
  try {
    ...
    const fallback = await callLLM(SYS, usr, { json: true });
...
export async function extractAcademicEvidence(source, opts = {}) {
  ...
  const fullText = source.full_text || source.clean_text || "";
  ...
  // Attempt 1: 10k chars — full coverage for models that can handle it.
  // Attempt 2: 5k chars — smaller JSON output; more reliable on weaker models.
  for (const charLimit of [10000, 5000]) {
    try {
      const usr = interpolate(USR_TPL, { ...baseFields, text: fullText.slice(0, charLimit) });
      const raw  = await attemptExtraction(usr);
      const items = normaliseItems(raw, source);
      if (items.length > 0) return items;
    } catch {
      // Parse failed at this text size — try next (smaller) slice.
    }
  }
