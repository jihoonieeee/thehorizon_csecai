...
function hasRecent(pack) {
  const items = [
    ...(pack?.strong_evidence || []), ...(pack?.usable_evidence || []),
...
function hasWalkthrough(pack) {
  const items = [...(pack?.strong_evidence || []), ...(pack?.case_study_candidates || [])];
  return items.some((it) =>
    it.evidence_type === "exploit_chain" ||
    (Array.isArray(it.attack_steps) && it.attack_steps.length >= 2) ||
    /attack chain|step 1|exploit chain|walkthrough/i.test(it.fact || ""));
}
...
export function buildWebEvidenceNeeds(params = {}) {
  ...
  return categories.map((category) => {
    ...
    const critical = (pack?.strong_evidence || []).length;
    const high = (pack?.usable_evidence || []).length;
    const caseStudies = (pack?.case_study_candidates || []).length;
