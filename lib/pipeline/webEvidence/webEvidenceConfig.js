 ...
 * Layer 5E statistics search). It **auto-enables when a search provider key
 * (Tavily/SerpAPI) is configured** and is a graceful no-op otherwise.
 * `WEB_EVIDENCE_ENABLED` is an explicit override that forces the branch on or off.
...
export function getWebEvidenceConfig() {
  return {
    // Explicit WEB_EVIDENCE_ENABLED wins; otherwise auto-enable when a provider key exists.
    enabled: isSet("WEB_EVIDENCE_ENABLED") ? bool("WEB_EVIDENCE_ENABLED", false) : hasSearchProvider(),
