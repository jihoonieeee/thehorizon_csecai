...
export function validateWebEvidence(ev) {
  ...
  // 6. Statistic grounding: a usable statistic needs metric + value + quote, and the
  //    value's significant number must appear verbatim in its quote. Ungrounded stats
  //    are DROPPED (the item itself is not rejected for this).
  let validStatistics = [];
  if (Array.isArray(ev?.statistics) && ev.statistics.length) {
    validStatistics = ev.statistics.filter((s) => {
      if (!s || !s.metric || !s.value || !s.quote) return false;
      const nums = significantNumbers(String(s.value));
      if (nums.length === 0) return false;          // must carry a real number
      return nums.every((n) => String(s.quote).includes(n));
    });
    // Dropping ungrounded stats is not an item-level violation — the item keeps its depth.
  }

  // ── Apply outcome ───────────────────────────────────────────────────────────
  const out = { ...ev };
  out.statistics = validStatistics;
