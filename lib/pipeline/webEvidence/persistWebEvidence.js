 ...
 *
 * Writes the tables in docs/migrations/000_schema.sql (section 10). Stores accepted AND
  ...
  for (let i = 0; i < rows.length; i += 200) {
    ...
    if (error) {
      if (isMissingTableError(error)) {
        ...
        console.warn(`persistWebEvidence: table '${table}' missing — apply docs/migrations/000_schema.sql`);
