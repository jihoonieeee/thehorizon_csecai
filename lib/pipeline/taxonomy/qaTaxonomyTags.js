...
import { TAXONOMY, VALID_AI_ENABLED_ROLES } from "../../config/taxonomyRegistry.js";
...
function deterministicCheck(source) {
  ...
  const proposedTags = Array.isArray(u.primary_tags) ? u.primary_tags : (u.primary_threat_tags || []);
  const { all } = validateThreatTags(proposedTags, source);
  const kept = [];
  for (const r of all) {
    if (r.validation_status === "rejected") {
      issues.push(`primary_tag "${r.tag}" rejected: ${r.caveat_if_any}`);
      continue;
    }
    if (r.validation_status === "weak" || r.validation_status === "needs_manual_review") {
      issues.push(`primary_tag "${r.tag}" ${r.validation_status}: ${r.caveat_if_any}`);
    }
    kept.push(r);
  }

  if (kept.length > 4) {
    issues.push(`Over-assignment: ${kept.length} primary tags (max recommended 4)`);
  }

  // Validate AI-enabled overlay
  const overlay = validateAiEnabledOverlay({
    ai_enabled: source.ai_enabled || u.ai_enabled,
    ai_enabled_roles: source.ai_enabled_roles || u.ai_enabled_roles || [],
    ai_capabilities:  source.ai_capabilities || u.ai_capabilities || [],
  });
  for (const caveat of overlay.caveats) {
    issues.push(`ai_enabled_overlay: ${caveat}`);
  }

  // Check: if primary_domain is ai_enabled_threats but ai_enabled_roles are
  // also set as overlay on a different domain source — flag for review
  const primaryDomain = u.primary_domain || source.primary_domain;
  if (primaryDomain !== "ai_enabled_threats" && overlay.ai_enabled_roles.length > 0) {
    // This is the intended dual-role pattern — just note it
    const roles = overlay.ai_enabled_roles.join(", ");
    issues.push(`[info] ai_enabled overlay on ${primaryDomain}: roles=[${roles}] (correct dual-role pattern)`);
  }

  return {
    primary_tags:         kept,
    primary_threat_tags:  kept,  // back-compat
    ai_enabled_overlay:   overlay,
    issues,
    structurally_valid:   issues.filter((i) => !i.startsWith("[info]")).length === 0,
  };
}
...
// Build a compact tag reference for the second model (taxonomy-v9)
function buildTagReference() {
  const lines = ["PRIMARY TAGS (domain → id: description):"];
  const domains = {
  ...
  for (const [domain, abbr] of Object.entries(domains)) {
    for (const [tag, entry] of Object.entries(TAXONOMY)) {
      ...
      lines.push(`  [${abbr}] ${tag}: ${entry.description.slice(0, 100)}`);
    }
  }
  lines.push("\nAI-ENABLED OVERLAY ROLES (AE01–AE09 — can appear on ANY domain as overlay):");
  for (const role of VALID_AI_ENABLED_ROLES) {
    const entry = TAXONOMY[role];
    if (entry) lines.push(`  ${role}: ${entry.description.slice(0, 80)}`);
  }
  return lines.join("\n");
}
...
const SECOND_MODEL_SYSTEM = `You are a second-opinion validator for the AI Threat Taxonomy v9.
You are given a source and proposed PRIMARY TAGS. Validate quality — focus on REMOVING incorrectly applied tags, never adding new ones.

${_tagReference}

RULES
1. A tag is valid only if the source SUBSTANTIVELY describes that adversarial threat behaviour.
2. Domain rules:
   [TRAD] = ML model/training/inference/supply chain attacked (TAI01–TAI10)
   [LLM] = LLM-specific: prompts, guardrails, context, RAG, embeddings (LLM01–LLM10)
   [AGENT] = AI acts through tools/MCP/memory/workflow/orchestration (ASI01–ASI10)
   [AI_ENABLED as primary] = source is PRIMARILY about AI as attacker's operational tool (AE01–AE09)
3. ai_enabled_roles (AE01–AE09) as OVERLAY on non-AI_ENABLED domain is CORRECT — do NOT flag this as a domain mismatch.
4. Prefer 1–3 high-confidence tags over many uncertain ones.
5. For sub-techniques: verify the sub-technique belongs to the stated parent primary tag.

Return strict JSON:
- qa_pass: false if significant errors found
- corrected_primary_domain: the correct domain if wrong, else null
- tags_to_remove: primary tag IDs to remove (empty if all valid)
- ai_enabled_roles_correct: true/false (is the AI-enabled overlay correct or should it change?)
- sub_techniques_correct: true/false
- issues: specific problems
- confidence_adjustment: "downgrade" | "upgrade" | null`;

function buildSecondModelPrompt(source, tags, aiOverlay) {
  const u = source.understanding || {};
  const tagList = tags
    .map((t) => `  - ${t.tag} [${t.domain}] (${t.confidence}/${t.validation_status}): "${(t.supporting_quote || "").slice(0, 200)}"`)
    .join("\n");
  const subTechList = (Array.isArray(u.sub_techniques) ? u.sub_techniques : [])
    .map((s) => {
      const id = typeof s === "string" ? s : s?.id;
      const pt = typeof s === "object" ? s?.parent_tag : "";
      return `  - ${id} [sub-technique of ${pt}]`;
    }).join("\n");
  const overlayLine = aiOverlay
    ? `ai_enabled=${aiOverlay.ai_enabled}, roles=[${(aiOverlay.ai_enabled_roles || []).join(", ")}], capabilities=[${(aiOverlay.ai_capabilities || []).join(", ")}]`
    : "(not set)";

  return [
    `TITLE: ${source.title || "(no title)"}`,
    `PUBLISHER: ${source.publisher || "unknown"}`,
    `PRIMARY DOMAIN: ${u.primary_domain || source.primary_domain || "(none)"}`,
    `SUMMARY: ${(u.source_summary || "").slice(0, 500)}`,
    ``,
    `PROPOSED PRIMARY TAGS:`,
    tagList || "  (none)",
    subTechList ? `\nPROPOSED SUB-TECHNIQUES:\n${subTechList}` : "",
    ``,
    `AI-ENABLED OVERLAY: ${overlayLine}`,
    ``,
...
function applyCorrections(deterministicResult, secondModelResult) {
  let tags = deterministicResult.primary_tags;
  ...
  return {
    qa_pass,
    primary_tags:           tags,
    primary_threat_tags:    tags,   // back-compat
    ai_enabled_overlay:     deterministicResult.ai_enabled_overlay,
  ...
  if (shouldRunSecondModel(deterministicResult, opts)) {
    try {
      const { result } = await routedLLM(
        SECOND_MODEL_SYSTEM,
        buildSecondModelPrompt(source, deterministicResult.primary_tags, deterministicResult.ai_enabled_overlay),
  ...
  for (let i = 0; i < sources.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(batch.map(async (source) => {
      ...
      return {
        ...source,
        taxonomy_qa: qa,
        // Hoist corrected primary_domain if second model found an issue
        primary_domain: qa.corrected_primary_domain || source.primary_domain,
        understanding: {
          ...source.understanding,
          primary_tags:         qa.primary_tags,
          primary_threat_tags:  qa.primary_threat_tags,  // back-compat
        },
      };
    }));
