...
import {
  ...
  EVIDENCE_TYPES,
  EVIDENCE_CLASSES,
  CLAIM_SUPPORTING_USES,
} from "../../schemas/evidencePacketSchema.js";

// ── Mapping helpers ────────────────────────────────────────────────────────────

// Map L5A evidence_type strings to canonical EVIDENCE_TYPES.
// IMPORTANT: this MUST cover every token in evidenceExtractionProfiles.ALL_EVIDENCE_TYPES.
// Previously, 9 of the 14 L5A extraction types (incident_event, attack_method,
// threat_actor_activity, research_result, societal_harm, governance_action,
// defensive_control, mitigation, infrastructure_dependency) had no entry and silently
// collapsed to background_context, erasing real incidents from the canonical packet.
const L5A_TYPE_MAP = {
  // vulnerability
  ...
  incident_event:             "incident_report",
  ...
  research_result:            "research_finding",
  // technique
  attack_method:              "attack_method",
  // governance / defensive
  policy_or_governance:       "policy_or_governance",
  governance:                 "policy_or_governance",
  governance_action:          "policy_or_governance",
  defensive_capability:       "defensive_capability",
  defensive_control:          "defensive_capability",
  mitigation:                 "defensive_capability",
  // surface / society / context
  attack_surface_signal:      "attack_surface_signal",
  infrastructure_dependency:  "attack_surface_signal",
  societal_impact:            "societal_impact",
  societal_harm:              "societal_impact",
...
// L5C is web-sourced ENRICHMENT. It must NEVER be labelled "operational" — that class
// means confirmed real-world activity from the ingested corpus, and tagging an external
// web reference operational let it launder adoption/operational claims. All L5C is
// "external"; downstream gates decide whether a validated+grounded item may fact_support.
function l5cQualityToClass(_sourceQuality) {
  return "external";
}

// Build permitted_uses from triage_data.permitted_uses (L5A).
// The L5A triage (sourceTypeClaimPermissions) emits fact_support / adoption_support /
// capability_support / trend_input / exposure_analysis / outlook_input / case_study /
// recommendation_input / context_only. Previously these were dropped (no map keys),
// so "claim_support" was NEVER present and canSupportClaim() returned false for every
// L5A packet. The map now PRESERVES the triage's granular use (the schema vocabulary
// was unified to accept it) instead of erasing it.
const L5A_PERMITTED_USE_MAP = {
  // triage claim-supporting uses — preserved as-is
  ...
  "outlook_input":        "outlook_input",
  "recommendation_input": "recommendation_input",
  "context_only":         "background_context",
  // legacy / cross-branch aliases
...
export function normalizeL5AToPacket(item, source = {}) {
  ...
  // Build permitted_uses: claim-supporting uses are present only for passed packets.
  let permittedUses = mapPermittedUses(rawUses);
  if (admissibility !== "passed") {
    permittedUses = permittedUses.filter((u) => !CLAIM_SUPPORTING_USES.has(u));
    if (!permittedUses.includes("background_context")) permittedUses.push("background_context");
  }

  // Map the L5A quote-verification verdict onto the packet's grounding group.
  const qv = item.quote_verification || {};
  const quoteVerification = qv.quote_exists === true ? "exists"
    : qv.quote_exists === false ? "absent"
    : (item.quote_verified === true ? "exists" : null);
  ...
  return makeEvidencePacket({
    ...
    claim_relevance: {
      ...
      limitations,
      materiality:       triage.materiality || null,
      uniqueness:        triage.uniqueness  || null,
    },
    // Full quality axis — carried on the packet, no longer dropped.
    source_quality: {
      status:  source?.source_quality_status || null,
      reasons: source?.source_quality_reasons || [],
    },
    independence: {
    ...
    grounding: {
      ...
      observed_use:       triage.observed_use === true,
    },
    method: {
...
export function normalizeL5BToPacket(ae) {
  ...
  // Chart-safety metadata — lets the analytics packet keep its own chart honest.
  // (metricType is already declared at the top of this function.)
  const sourcePop   = (ae.source_ids || []).length;
  const isCountish  = [
    "frequency_distribution", "maturity_distribution", "source_coverage",
    "cross_tab_matrix", "adoption_signal", "burst_pattern",
  ].includes(metricType);
  const analytics_meta = {
    metric_definition:   ae.metric_definition || `${metricType.replace(/_/g, " ")} computed over the collected corpus`,
    source_population:   sourcePop,
    included_source_ids: ae.source_ids || [],
    // A corpus count IS a count over the collected population — that population is the
    // denominator. Non-count metrics declare why they have none.
    denominator:         isCountish ? sourcePop : (ae.denominator ?? null),
    no_denominator_reason: isCountish ? null
                            : (ae.denominator != null ? null : "metric is not a proportion over a fixed population"),
    date_range:          ae.date_range || ae.window || null,
    grouping_dimension:  ae.grouping_dimension || ae.dimension || null,
    corpus_scope:        "collected_corpus",
    // Count/distribution metrics measure how much was PUBLISHED, not how often the
    // threat OCCURS — flag that distinction explicitly.
    publication_vs_threat_activity: isCountish ? "publication_activity" : "unknown",
    chart_allowed:       ae.confidence !== "low" && sourcePop >= 2,
    chart_caveat:        ae.caveat_if_any
...
export function normalizeL5CToPacket(ev) {
  ...
  return makeEvidencePacket({
    ...
    grounding: {
      // L5C quote grounding (entailment is not yet computed for external evidence —
      // tracked as a known gap; verification reflects whether a usable quote exists).
      quote_verification: (ev.source_quote || ev.exact_quote || "").trim().length >= 20 ? "exists" : "absent",
      quote_entailment:   ev.quote_entailment || null,
      observed_use:       ev.observed_use === true,
    },
    content: {
