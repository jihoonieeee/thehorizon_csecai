...
import { canSupportClaim, validatePacket } from "../../schemas/evidencePacketSchema.js";
...
export class EvidencePacketRegistry {
  constructor() {
    ...
    this._byBranch  = new Map();   // branch_type → Set<evidence_id>
    this._claimIndex = new Map();  // claim_id    → Set<evidence_id>
    this._invalid   = [];          // { evidence_id, errors[] } for packets that failed validation
  }
  ...
  register(packets) {
    ...
    for (const p of arr) {
      ...
      // Validate at registration. We DON'T drop invalid packets (that would create
      // dangling claim citations), but we record them so the QA report can surface
      // schema violations instead of letting makeEvidencePacket silently coerce them.
      const errs = validatePacket(p);
      if (errs.length) this._invalid.push({ evidence_id: p.evidence_id, errors: errs });
      ...
      // Index by explicit branch_type (the discriminant) — falls back to the
      // extraction_layer mapping for any packet that predates branch_type.
      const branch = p.branch_type
        || (layer === "L5B" ? "analytics" : layer === "L5C" ? "web_enrichment" : "rawfact");
      if (!this._byBranch.has(branch)) this._byBranch.set(branch, new Set());
      this._byBranch.get(branch).add(p.evidence_id);
  ...
  summary() {
    ...
    const byBranch = {};
    for (const [branch, ids] of this._byBranch) byBranch[branch] = ids.size;
    return {
      ...
      invalid_packet_count: this._invalid.length,
      invalid_packets: this._invalid.slice(0, 20),
    };
  }

  /** Resolve all packets for a branch_type (rawfact | analytics | web_enrichment). */
  getByBranch(branch) {
    const ids = this._byBranch.get(branch) || new Set();
    return [...ids].map((id) => this._packets.get(id)).filter(Boolean);
  }

  /** All packets that failed schema validation at registration (for QA reporting). */
  getInvalidPackets() {
