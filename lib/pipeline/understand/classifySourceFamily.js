...
export function classifySourceFamily(source) {
  ...
  // 2. Roundup / digest — check BEFORE academic/threat_intel so a security-firm
  //    weekly digest is still classified as roundup, not threat_intel_report.
  //    Identified by detectDigest flag, content_quality=aggregation, or title pattern.
  const ROUNDUP_TITLE_RE = /\b(\d{1,2}|ten|fifteen|twenty)\s+(biggest|top|key|major|most\s+important)\s+(stories|developments|events|threats|incidents|things)\b|\b(weekly|daily|monthly)\s+(roundup|wrap[-\s]?up|digest|brief|newsletter)\b|\bnews\s+today\b|\bthis\s+week\s+in\b/i;
  if (
    source.is_digest === true ||
    source.content_quality === "aggregation" ||
    ROUNDUP_TITLE_RE.test(source.title || "")
  ) {
    return "roundup_digest";
  }

  // 3. Academic paper — arXiv connector, academic publisher, or academic source_type.
  // publisher/connector is the authoritative signal — arXiv papers carry any source_type
  // (research_finding, capability_demonstration, vulnerability, etc.) so we do NOT
  // gate on source_type here; the publisher alone is sufficient.
  if (
  ...
  // 4. Threat intelligence report — source_type is the strongest signal; also
  //    catch security-firm high-trust sources. arXiv / academic papers are already
  //    returned in step 3, so any security-firm source reaching here is a blog or
  //    research post, not an arXiv paper — no research_finding exclusion needed.
  if (
    sourceType === "threat_intelligence" ||
    (isSecurityFirm(source) && isHighTrust)
