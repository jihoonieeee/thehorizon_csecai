...
function buildEnrichmentPrompt(source) {
  ...
  return {
    // Prose lives in lib/prompts/understand/classify-defensive.md; data injected here.
    system: interpolate(loadPrompt("understand/classify-defensive").system, {
      focusAreas: DEFENSIVE_FOCUS_AREAS.join(", "),
    ...
    const enriched = await Promise.all(batch.map(async source => {
      ...
      if (!skipLlm) {
        try {
          ...
          // Union the understand-call techniques with the enrichment-call techniques
          // so neither overwrites the other (enrich may surface additional techniques).
          const mergedTechniques = [...new Set([...(source.defensive_techniques || []), ...techniques])].slice(0, 3);
