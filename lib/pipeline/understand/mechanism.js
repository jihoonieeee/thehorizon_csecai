...
// RAG/knowledge-store layers: a privacy attack (membership inference / model
// inversion) against these is LLM data leakage (LLM02), not a traditional-ML
// attack on a trained model's own training set.
const RAG_DATA_LAYERS     = new Set(["retrieval", "vector_database"]);
...
export function resolveDomain({ primary_exploit_mechanism: mech, affected_layer: layer, primary_consequence: cons, target_is_llm } = {}) {
  ...
  if (mech === "supply_chain_compromise") {
    ...
    // LLM-specific supply chain — a plugin/template, OR any LLM-serving-infra artifact
    // (gateway/proxy/serving package, e.g. LiteLLM/vLLM) flagged target_is_llm → LLM03.
    // General-ML artifacts (model file, dataset, ONNX/Keras) fall through to TAI10.
    if (layer === "plugin_extension" || target_is_llm === true) return "llm_threats"; // → LLM03
    return "traditional_ai_threats";                                   // general-ML model/pkg/artifact → TAI10
  }

  // Membership inference / model inversion target a DATASET, not a model's
  // parameters. Classic MIA is against a trained classifier's TRAINING set →
  // traditional. But the same mechanism against a RAG/retrieval CORPUS (an LLM
  // knowledge store) is LLM data leakage, not a traditional-ML model attack.
  // Route by the attacked data layer: retrieval/vector_database (or an explicit
  // target_is_llm) → llm_threats (LLM02); otherwise fall through to traditional.
  if ((mech === "membership_inference" || mech === "model_inversion") &&
      (target_is_llm === true || RAG_DATA_LAYERS.has(layer))) {
    return "llm_threats"; // → LLM02 sensitive info disclosure
  }
  ...
  // resource exhaustion on the model/inference layer → traditional (TAI09) for
  // general-ML serving; LLM-serving infra (target_is_llm, e.g. vLLM inference DoS)
  // is LLM unbounded consumption (LLM10), consistent with the supply-chain branch.
  if (mech === "resource_exhaustion" && TRADITIONAL_LAYERS.has(layer)) {
    return target_is_llm === true ? "llm_threats" : "traditional_ai_threats";
  }
...
export function buildMechanismPromptBlock() {
  ...
  lines.push("target_is_llm (boolean): true when the attacked/compromised artifact is LLM-SPECIFIC — an LLM model, its prompt/RAG surface, or LLM-serving infrastructure (a gateway/proxy/serving package such as LiteLLM, vLLM, Ollama, an inference gateway, or an MCP/LLM plugin). false for general-ML artifacts (a classifier model file, ONNX/Keras weights, a training dataset). This routes supply_chain_compromise and poisoning to the LLM domain (LLM03/LLM04) instead of traditional-ML (TAI10/TAI01).");
  lines.push("");
