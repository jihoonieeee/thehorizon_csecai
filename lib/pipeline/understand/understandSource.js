...
/**
 * Deterministic reading_value for sources classified via understandAllSources.
 * L3 normally computes this via LLM (layer3.md), but the L4b path bypasses L3
 * entirely. Re-running L3 on already-classified sources causes false rejections
 * (ATLAS case studies, Jina-fetched references) so we derive it from the
 * classification result we already have.
 *
 * Mirrors the editorial intent of layer3.md without an extra LLM call:
 *   essential    — confirmed real-world incident or primary threat intel
 *   recommended  — working PoC / demonstrated capability
 *   informative  — research finding, CVE, or governance signal
 *   background   — defensive, off-topic, or unclear
 */
function deriveReadingValue(r) {
  if (r.is_defensive) return "background";
  const imp = computeImportance(r);
  if (imp.tier === "noise")     return "background";
  if (imp.tier === "realized")  return "essential";
  if (imp.tier === "proven") {
    return r.source_type === "threat_intelligence" ? "essential" : "recommended";
  }
  if (imp.tier === "research")  return "informative";
  if (imp.tier === "reference") return "informative";
  return "informative";
}
...
export function normalise(raw, source, opts = {}) {
  ...
  if (opts.skipLlm) {
    ...
    // domain is allowed (a source can touch a neighbouring technique), with one
    // exception: AE (ai_enabled_threats) tags on non-AE sources require
    // ai_enabled_overlay=true — the LLM's explicit signal that a genuine
    // AI-as-weapon co-technique is present alongside the primary threat.
    // Without that flag, an AE secondary on a non-AE source is almost always
    // an outcome label ("jailbreak produced deepfakes" → AE10) rather than a
    // co-present attack technique, and should not pollute the tags column.
    const aiEnabledOverlay = flat.ai_enabled_overlay === true;
    const secondary = (Array.isArray(flat.secondary_tags) ? flat.secondary_tags : [])
      .map(t => String(t || "").trim())
      .filter(t => {
        if (!t || !isValidTag(t) || t === primaryTag) return false;
        if (domainOfTag(t) === "ai_enabled_threats" && resolvedCategory !== "ai_enabled_threats") {
          return aiEnabledOverlay;
        }
        return true;
      });
...
function fromDbRow(source) {
  ...
  return {
    ...
    is_defensive:         isDef,
    defended_category:    source.intelligence?.defended_category || (isDef && isOffensive ? source.main_category : null),
      ...
      const writebacks = batchResults
        ...
        .map(r => {
          ...
          const row = {
            ...
            ...(!orig.reading_value ? { reading_value: deriveReadingValue(r) } : {}),
            intelligence: {
              ...
              ...(r.event_date ? { event_date: r.event_date, event_date_confidence: r.event_date_confidence } : {}),
              ...(r.source_coverage_type ? {
                source_coverage_type: r.source_coverage_type,
                ...(r.covered_period_start ? { covered_period_start: r.covered_period_start } : {}),
                ...(r.covered_period_end   ? { covered_period_end:   r.covered_period_end   } : {}),
              } : {}),
            },
          };
