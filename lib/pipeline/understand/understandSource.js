 ...
 *   analyst      — research finding, CVE, or governance signal
...
function deriveReadingValue(r) {
  ...
  if (imp.tier === "reference") return "analyst";
  return "analyst";
}
      ...
      const writebacks = batchResults
        ...
        .map(r => {
          ...
          const row = {
            ...
            // Deterministic reading_value — L3 normally sets this via LLM but the
            // L4b path bypasses L3. Re-running L3 on already-classified sources causes
            // false rejections, so we derive it from the classification result here.
            // Only set when not already present (preserves L3 value for full-ingest sources).
