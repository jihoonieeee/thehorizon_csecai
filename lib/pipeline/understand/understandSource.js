...
export function normalise(raw, source, opts = {}) {
  ...
  // Force them to off_topic so they get discarded in write-back and not kept as adjacent.
  if (isDefensive && disposition !== "offensive") {
    disposition = "off_topic";
  }

  // Category resolution:
  //   offensive              → its resolved offensive domain.
  //   everything else        → unclear_or_adjacent (discarded).
  const finalCategory =
    disposition === "offensive" ? resolvedCategory : "unclear_or_adjacent";
...
export async function understandAllSources(sources, opts = {}) {
  ...
  for (const source of sources) {
    if (
      ...
      const cached = fromDbRow(source);
      // Defensive sources are out of scope regardless of cache status.
      // Force off_topic so they land in discarded and don't inflate offensive counts.
      if (cached.is_defensive) {
        results.push({ ...cached, relevant: false, keep: false, disposition: "off_topic" });
      } else {
        results.push(cached);
      }
      cacheHits++;
  ...
  for (let i = 0; i < toProcess.length; i += concurrency) {
    ...
    if (supabase && !opts.skipLlm) {
      ...
      const writebacks = batchResults
        .filter(r => r.keep && r.category && !r._from_cache)
        .map(r => {
