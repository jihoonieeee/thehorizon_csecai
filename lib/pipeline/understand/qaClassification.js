...
import { DOMAINS, isValidTag, buildTaxonomyPromptBlock } from "./taxonomy.js";
...
// ── Verifier prompt (loaded from lib/prompts/understand/qa-classification.md) ─

const _qaClassPrompts = loadPrompt("understand/qa-classification");
const QA_CLASS_SYSTEM = _qaClassPrompts.system;
const QA_CLASS_USER_TMPL = _qaClassPrompts.user;
...
function buildVerifyPrompt(source) {
  ...
  return interpolate(QA_CLASS_USER_TMPL, {
...
export async function qaClassificationLLM(sources, opts = {}) {
  ...
  const updatedSources = sources.map(s => {
    ...
    // Count as fixed only when the re-run produced an offensive classification
    // (not when the source was discarded as off_topic by the re-run).
    if (u.relevant) fixedIds.add(s.id);
    ...
    return {
      ...
      mechanism_classification: u.mechanism_classification || s.mechanism_classification,
      source_family:           u.source_family          || s.source_family,
      _qa_fixed: true,
  ...
  if (supabase && fixedIds.size > 0) {
    const writes = updatedSources
      ...
      .map(s => ({
        ...
        source_family: s.source_family || null,
        ...
        intelligence: {
          ...
          mechanism_classification: s.mechanism_classification || null,
        },
      }));
