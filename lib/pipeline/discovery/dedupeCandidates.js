...
/**
 * Cluster candidates by primary_origin_url/primary_origin_hash.
 * Multiple secondary articles that all report on the same primary source
 * must share candidate_origin_cluster_id so downstream can detect fake corroboration.
 */
function buildOriginClusters(items) {
  const originIndex = new Map();  // primary_origin_hash → origin_cluster_id

  for (const item of items) {
    const originHash = item.primary_origin_hash
      || sha256short((item.primary_origin_url || item.opened_url || "").toLowerCase().split("?")[0]);

    if (!item.primary_origin_hash) {
      item.primary_origin_hash = originHash;
    }

    if (!originIndex.has(originHash)) {
      originIndex.set(originHash, `oc_${randomUUID().slice(0, 8)}`);
    }
    item.candidate_origin_cluster_id = originIndex.get(originHash);
  }

  // Mark candidates where 2+ secondary reporters share the same origin cluster
  // as circular_reporting_risk
  const clusterCounts = {};
  for (const item of items) {
    if (item.candidate_origin_cluster_id) {
      clusterCounts[item.candidate_origin_cluster_id] = (clusterCounts[item.candidate_origin_cluster_id] || 0) + 1;
    }
  }
  for (const item of items) {
    const cid = item.candidate_origin_cluster_id;
    if (cid && clusterCounts[cid] >= 3 &&
        item.origin_role === "secondary_reporting" &&
        item.independence_level !== "circular_reporting_risk") {
      item.independence_level = "circular_reporting_risk";
    }
  }
}
...
export function dedupeCandidates(candidates = []) {
  ...
  // First pass: build origin clusters (before content-level dedup)
  buildOriginClusters(items);
  ...
  for (const cluster of clusters) {
    ...
    for (const member of cluster.slice(1)) {
      ...
      if (addsUniqueValue(member, rep)) {
        ...
        // Content differs from rep but they share origin → seen_same_origin
        if (member.primary_origin_hash === rep.primary_origin_hash) {
          member.processing_cache_status = "seen_same_origin";
        }
      } else {
        ...
        // Classify redundancy type for skip-reprocess logic
        if (member.content_hash && member.content_hash === rep.content_hash) {
          member.processing_cache_status = "seen_same_content";
        } else if (member.primary_origin_hash === rep.primary_origin_hash) {
          member.processing_cache_status = "seen_same_origin";
        } else {
          member.processing_cache_status = "seen_same_content";
        }
    ...
    // syndicated reposts AND not circular reporters, the technique has independent corroboration.
    const retainedReps = cluster.filter((m) => m.is_cluster_representative &&
      m.source_independence_status !== "syndicated" &&
      m.independence_level !== "circular_reporting_risk");
