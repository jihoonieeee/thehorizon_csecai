...
export function inferCandidateDate(candidate) {
  const explicit = candidate.published_date || candidate.event_date || candidate.last_updated;
  if (explicit) return explicit;

  const url = candidate.opened_url || "";
  const hay = `${candidate.title || ""}  ${(candidate.page_text || candidate.summary || "").slice(0, 400)}`;

  // ISO date in title/body
  let m = hay.match(/\b(20\d{2})-(\d{2})-(\d{2})\b/);
  if (m) return `${m[1]}-${m[2]}-${m[3]}`;

  // YYYY/MM[/DD] segment in the URL path (common on news/blog permalinks)
  m = url.match(/\/(20\d{2})\/(\d{1,2})(?:\/(\d{1,2}))?(?:\/|$|\?)/);
  if (m) return `${m[1]}-${pad2(m[2])}-${pad2(m[3] || 1)}`;

  // "Month YYYY" (e.g. "March 2026", "Mar. 2026") in title/body
...
export function candidateToSource(candidate) {
  ...
  // The window filter keys off date_published. Prefer a REAL date — explicit
  // candidate date, else one inferred from the URL/title/body — and only fall
  // back to discovery time as a last resort (so a dateless source is still
  // visible rather than dropped). date_published_actual carries the real date
  // when known, so downstream can tell inferred dates from defaults.
  const pageDate = inferCandidateDate(candidate);
