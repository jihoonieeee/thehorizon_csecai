 ...
 *
 * Only accepted candidates should be passed here; rejected candidates are stored
 * for audit but never become pipeline sources.
 */

import { normalizeSource, arxivIdFromUrl } from "../ingest/normalizeSource.js";
import { enrichArxivDate } from "../ingest/connectors/arxivConnector.js";
...
// arXiv sources discovered via web discovery won't have gone through the Atom
// API, so their dates are "estimated" at best. Enrich them sequentially here —
// before they enter Layer 2/3 — so they carry exact day-level dates.
export async function candidatesToSources(candidates = []) {
  const sources = candidates.map(candidateToSource);
  for (const source of sources) {
    if (source.date_confidence === "exact") continue;
    const arxivId = arxivIdFromUrl(source.url || "");
    if (!arxivId) continue;
    const enriched = await enrichArxivDate(arxivId);
    if (!enriched) continue;
    source.date_published  = enriched.date_published + "T00:00:00+00:00";
    source.date_confidence = "exact";
  }
  return sources;
}
