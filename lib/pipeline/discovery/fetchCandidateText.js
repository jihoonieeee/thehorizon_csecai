 ...
 * Handles Cloudflare-protected and JS-rendered pages that return empty/minimal
 * HTML to a plain fetch(). Rate-limited ~20 req/min on the free tier.
 */
async function fetchViaJina(url, { timeoutMs = 20000 } = {}) {
  ...
  try {
    ...
    // Strip Jina's header block (Title/URL Source/Published Time lines)
...
/**
 * Tavily Extract fallback — uses existing TAVILY_API_KEY.
 * Reserved for sites Jina also cannot reach. Costs API quota.
 */
async function fetchViaTavily(url, { timeoutMs = 20000 } = {}) {
  ...
  try {
    ...
    const raw = (data?.results?.[0]?.raw_content || "")
      .replace(/!\[.*?\]\(data:image\/[^)]+\)/g, ""); // strip base64 images
    return raw.trim();
  } catch {
    return "";
  } finally {
...
export async function fetchPageText(url, opts = {}) {
  ...
  // Attempt 1: direct fetch (fast, no quota cost)
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (!ctype || ctype.includes("html") || ctype.includes("text")) {
        ...
        if (html.length > 5000) {
          ...
          directText = cleanPlaintext(html).slice(0, maxChars);
        }
      }
    }
  } catch { /* fall through to reader fallbacks */ } finally {
    clearTimeout(timer);
  }

  if (directText.length >= MIN_BODY_CHARS && contentQualityOk(directText)) return directText;

  // Attempt 2: Jina AI Reader — handles JS-rendered pages and bot-blocked sites
  const jinaText = await fetchViaJina(url, { timeoutMs: 20000 });
  if (jinaText.length >= MIN_BODY_CHARS) return jinaText.slice(0, maxChars);

  // Attempt 3: Tavily Extract — last resort, costs API quota
  const tavilyText = await fetchViaTavily(url, { timeoutMs: 20000 });
  if (tavilyText.length >= MIN_BODY_CHARS) return tavilyText.slice(0, maxChars);

  return directText; // return whatever we got even if thin
}
