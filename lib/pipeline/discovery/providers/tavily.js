...
function mapResults(data, { mission, query, family, source_class_hint }) {
  ...
  for (const r of results) {
    ...
    // Tavily already extracted the page body — keep a substantial slice as the
    // candidate's body text so accepted sources are not reduced to a single
    // sentence. (Previously raw_content was discarded after the first sentence.)
    ...
    candidates.push({
      ...
      page_text,                             // full extracted body (used as full_text)
 ...
 */
// Recency window per query family — operational/fresh families search the last
// week/month so re-runs surface NEW incidents instead of re-finding old content
// (the throughput bug); research/taxonomy families keep a year (papers age slowly).
export function recencyForFamily(family) {
  // Backfill override: WEB_DISCOVERY_RECENCY=year|month|week forces the search
  // window (used to sweep a full year of incidents in one operational backfill).
  const override = process.env.WEB_DISCOVERY_RECENCY;
  if (override && ["day", "week", "month", "year"].includes(override)) return override;
...
export async function runTavilyQuery({ mission, query, family, source_class_hint }, opts = {}) {
  const { maxResults = 6, fetchImpl = fetch } = opts;
  const timeRange = opts.timeRange || recencyForFamily(family);
  ...
  for (const key of keys) {
    try {
      ...
      // Tavily returns HTTP 200 for quota errors with {"detail":{"error":"...usage limit..."}}
      // instead of a 4xx status. Detect and retire the key the same as a real 403/432.
      const bodyError = typeof data?.detail === "object" ? data.detail?.error : data?.detail;
      if (typeof bodyError === "string" && /usage limit|quota|upgrade your plan/i.test(bodyError)) {
        _disabledKeys.add(key);
        lastNote = "tavily_quota_or_auth";
        continue;
      }
