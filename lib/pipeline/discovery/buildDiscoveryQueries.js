...
export function buildDiscoveryQueries(mission, opts = {}) {
  ...
  for (const tagId of def.primary_tags) {
    ...
    taxonomy.push({ query: `${phrase} attack ${CURRENT_YEAR}`, family: "taxonomy", source_class_hint: null });
    const subs = getSubTechniques(tagId).slice(0, 2);
    for (const sub of subs) {
      taxonomy.push({ query: `${subTechPhrase(sub)} ${phrase} ${CURRENT_YEAR}`, family: "taxonomy", source_class_hint: null });
    }
  }
  ...
  for (const tagId of def.primary_tags.slice(0, 2)) {
    const phrase = tagPhrase(tagId);
    operational.push({ query: `${phrase} in the wild ${CURRENT_YEAR}`,           family: "operational", source_class_hint: "operational_ti" });
    operational.push({ query: `${phrase} campaign ${CURRENT_YEAR}`,              family: "operational", source_class_hint: "operational_ti" });
    operational.push({ query: `${phrase} exploited attackers ${CURRENT_YEAR}`,   family: "operational", source_class_hint: "operational_ti" });
    operational.push({ query: `${phrase} incident breach ${CURRENT_YEAR}`,       family: "operational", source_class_hint: "operational_ti" });
    operational.push({ query: `threat actor using ${phrase} ${CURRENT_YEAR}`,    family: "operational", source_class_hint: "operational_ti" });
  }
  ...
  for (const tagId of def.primary_tags.slice(0, 2)) {
    ...
    for (const term of ["PoC", "benchmark", "advisory", "incident"]) {
      artifact.push({ query: `${phrase} ${term} ${CURRENT_YEAR}`, family: "artifact", source_class_hint: null });
    }
  }
  ...
  for (const cls of def.target_source_classes) {
    ...
    for (const site of hints.slice(0, 2)) {
      site_scoped.push({ query: `${site} ${phrase} ${CURRENT_YEAR}`, family: "site_scoped", source_class_hint: cls });
    }
  }
  ...
  retry.push({ query: `${primaryPhrase} technical analysis ${CURRENT_YEAR}`, family: "retry", source_class_hint: null });
  retry.push({ query: `${primaryPhrase} PDF report ${CURRENT_YEAR}`, family: "retry", source_class_hint: null });
  retry.push({ query: `${primaryPhrase} dataset benchmark ${CURRENT_YEAR}`, family: "retry", source_class_hint: "benchmark_dataset" });
