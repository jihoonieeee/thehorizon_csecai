 ...
 *      categorical signal; routeCandidate() decides accept / reject from the
 *      gate states. Reason codes in candidate_route_reasons[] carry nuance.
 *
...
/**
 * Decide the route from gate states. Binary: accept or reject.
 * Reason codes in candidate_route_reasons[] carry the nuance for audit.
 */
export function routeCandidate(candidate) {
  ...
  const out = (route, ...reasonCodes) => ({
    route,
    route_reason:            reasonCodes[0] || route,
  ...
  // Weak specificity (single anchor): accept but flag for entailment QA.
  // Token overlap is unreliable for novel terminology — don't reject on that alone.
  if (candidate.ai_threat_specificity === "weak") {
    reasons.push("single_anchor_novelty");
    if (candidate.quote_support === "unsupported") reasons.push("quote_unsupported");
    if (candidate.requires_entailment_qa || candidate.quote_support === "requires_entailment_qa") {
      reasons.push("requires_entailment_qa");
    }
    return out("accept", "single_anchor_novelty");
  }
  ...
  // ── ACCEPT — flag ambiguities as reasons, never gate on them ─────────────────

  if (candidate.defensive_content_type === "defensive_only") {
    reasons.push("defensive_only");
  }
  if (candidate.defensive_content_type === "defensive_with_offensive_findings") {
    reasons.push("defensive_with_offensive_findings");
  }
  if (candidate.quote_status === "present" && candidate.quote_anchor_bearing === false) {
    reasons.push("quote_not_anchor_bearing");
  }
  if (candidate.quote_status === "missing_preclean") reasons.push("quote_pending_preclean");
  if (freshClass === "unknown_date" || candidate.freshness_status === "unknown") reasons.push("date_missing");
  if (candidate.hallucination_risk === "medium") reasons.push("hallucination_risk_medium");
  if (candidate.independence_level === "unknown") reasons.push("independence_unverified");
  if (candidate.quote_support === "partially_supported") reasons.push("quote_partially_supported");

  reasons.push("accepted");
  return out("accept", "accepted");
}
