...
import { runDiscoveryQuery, WEB_DISCOVERY_VERSION, buildGroundedUrlSet } from "./webDiscoverySearch.js";
...
export async function runWebDiscovery(opts = {}) {
  const {
    ...
    now = new Date(),
    useCache = true,
    searchFn = runDiscoveryQuery,
    triageFn = triageCandidates,
    qaFn = null,
  } = opts;

  const usingDefaultSearch = searchFn === runDiscoveryQuery;
  const discoveryEnabled = process.env.WEB_DISCOVERY_ENABLED !== "0";

  // Hard short-circuit: default network path with no key or disabled → empty result.
  if (usingDefaultSearch && (!discoveryEnabled || !process.env.ANTHROPIC_API_KEY)) {
    return emptyResult(missions, !discoveryEnabled ? "web_discovery_disabled" : "no_anthropic_key");
  }
  ...
  let triaged = await triageFn(clustered, { skipLlm });
