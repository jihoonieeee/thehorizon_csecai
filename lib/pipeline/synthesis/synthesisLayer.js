...
export async function runSynthesisLayer(sources, opts = {}) {
  ...
  const externalVisualEvidence = evidenceSearchResult.external_visual_evidence || [];
  ...
  // Enrich corpus viz specs with external evidence references, then append:
  // 1. Charts re-drawn from web text-evidence data series
  // 2. Visual evidence specs (diagrams, figures, framework maps, benchmark charts)
  ...
  process.stdout.write(
    `    external-evidence charts: ${externalVizSpecs.length} | ` +
    `external-visual specs: ${externalVisualSpecs.length}\n`,
  );

  // ── L5C — Web Evidence branch (gap-driven; off unless WEB_EVIDENCE_ENABLED) ──
  // Runs after rawfacts + analytics so it can target evidence gaps. Returns an
  // empty result when disabled or no provider key is set — never blocks the run.
  let webEvidence = null;
  const webEvidenceConfig = getWebEvidenceConfig();
  if (webEvidenceConfig.enabled) {
    process.stdout.write("  [L5C-web-evidence] branch starting (gap-driven web evidence + visuals)...\n");
    try {
  ...
  const cross_category_synthesis = await runCrossCategorySynthesis(
    category_analyses, derived_metrics, visualization_specs, {
      skipLlm,
  ...
  return {
    ...
    // Combined visual evidence: text-evidence items flagged is_visual + full visual evidence objects
    visual_evidence:           [
      ...externalEvidence.filter((e) => e && (e.is_visual || e.chart_data)),
