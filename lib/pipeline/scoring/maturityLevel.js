...
export async function classifyMaturityLevel(source, callFn) {
  ...
  try {
    ...
    // The prompt writes level names in caps (RESEARCH, VALIDATED, …) and models
    // routinely echo that casing back. Normalise before validating — a
    // case-sensitive check here silently dumped those sources onto the
    // deterministic fallback.
    const level      = String(raw?.level      ?? "").trim().toLowerCase();
    const confidence = String(raw?.confidence ?? "").trim().toLowerCase();
    if (!MATURITY_LEVELS.includes(level)) throw new Error(`Invalid level: ${raw?.level}`);
    return {
      level,
    ...
    // Fall back, but say WHY. A bare `catch {}` here previously made a broken
    // LLM config look like a successful run of low-confidence classifications.
    const fb = deterministicMaturity(source);
    return { ...fb, reason: `${fb.reason} (LLM failed: ${err.message})`, error: err.message };
  }
}
