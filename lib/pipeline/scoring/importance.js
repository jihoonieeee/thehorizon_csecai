 ...
 *   reference — authoritative advisory: governance_signal; also vulnerability reports from
 *               primary/high-trust publishers (floor — prevents noise when L3 misclassifies
 *               an exploit-chain disclosure as "vulnerability")
...
export function computeImportance(source) {
  ...
  if (reality === "vulnerability") {
    ...
    // Authoritative/established publishers (Microsoft, Google, Anthropic, CISA, etc.) filing
    // vulnerability reports retain reference value even without in-wild confirmation.
    // This is a floor — it prevents primary publishers from disappearing into noise when
    // the L3 LLM mislabels a full exploit-chain disclosure as "vulnerability".
    if (AUTHORITATIVE_TIERS.has(trustTier) || provenance === "established") {
      return { tier: "reference", reality: "advisory", posture, provenance, rules_version: RULES_VERSION };
    }
