 ...
 *   reading_value   — audience fit: essential > recommended > informative > background
 *                     (set by Layer 3 LLM; answers "should leadership / team read this?")
 *   maturity_level  — threat lifecycle: operational > observed > validated > research
...
const READING_VALUES = new Set(["essential", "recommended", "informative", "background"]);

// "analyst" was renamed to "informative" (Sept 2026). Rows written before
// scripts/relabelReadingValues.js ran still carry the old value — normalise on read so a
// partially-migrated corpus never drops out of ranking. Safe to delete once the relabel
// has covered the corpus and no row has reading_value='analyst'.
const LEGACY_READING = { analyst: "informative" };
...
export function readingValueOf(s = {}) {
  ...
  const raw = s.reading_value ?? s.intelligence?.reading_value ?? null;
  const v = LEGACY_READING[raw] ?? raw;
  return READING_VALUES.has(v) ? v : null;
}

export const READING_RANK = { essential: 3, recommended: 2, informative: 1, background: 0 };
...
export function isNoiseSource(s = {}) {
  ...
  if (rv && rv !== "background") return false;  // informative/recommended/essential always signal
