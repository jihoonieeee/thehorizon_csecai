...
export async function runPipeline(sources, opts = {}) {
  ...
  // ── Step 4: L6 Analysis ────────────────────────────────────────────────────
  // Select top sources per category, build dossiers, run LLM analysis,
  // QA citations. Output: CategoryAnalysis[] with insights + cited sources.
  log("L6", "Analysing categories...");
  const windowInfo = {
    type:      "pipeline_run",
    key:       run_id,
    label:     corpus_summary.date_range || run_id,
    date_from: corpus_summary.date_range?.split(" – ")?.[0] || run_date.slice(0, 10),
    date_to:   corpus_summary.date_range?.split(" – ")?.[1] || run_date.slice(0, 10),
  };
  const category_analyses = await runAnalysis(
    allRelevantQA, evidenceItems, corpus_summary, windowInfo,
    { skipLlm, supabase: null }  // pipeline runs don't persist to category_insights
  );
  const totalInsights = category_analyses.reduce((n, ca) =>
    n + (ca.insights || []).filter(i => !i.blocked).length, 0);
  log("L6", `${totalInsights} insights across ${category_analyses.length} categories`);

  await onCheckpoint("analysis", {
    run_id,
    total_insights: totalInsights,
    category_summary: category_analyses.map(ca => ({
      category:        ca.category,
      status:          ca.assessment_status,
      approved:        (ca.insights || []).filter(i => !i.blocked).length,
      blocked:         (ca.insights || []).filter(i => i.blocked).length,
      source_count:    ca.selected_source_ids.length,
    })),
  });

  // ── Build dashboard state (no LLM) ─────────────────────────────────────────
  const dashboard_state = buildDashboardState({
    run_id, run_date, category_analyses, evidence_items: evidenceItems,
    corpus_summary, cross_category: { patterns: [] },
  });

  // ── Step 5: Build presentation (optional) ──────────────────────────────────
  // Slide-specific steps (cross-category, case studies, outlooks, planning)
  // are orchestrated inside buildPresentation to keep them out of L6.
  let deck = null;
  if (!skipSlides) {
    ...
    deck = await buildPresentation(category_analyses, null, evidenceItems, {
      skipLlm,
    ...
    await onCheckpoint("presentation", {
      ...
      evidence_callouts:   deck.counts?.evidence_callouts || 0,
    });
  }
