 ...
 * @returns {Promise<PipelineV2Result>}
 */
// Max sources per offensive category loaded for slide generation.
// Keeps synthesis dossiers focused and cuts LLM token spend significantly.
const SOURCES_PER_CATEGORY = 40;

export async function runPipelineFromDB(supabase, opts = {}) {
  const { days = 30, limit = 200, category } = opts;
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);

  // Load top SOURCES_PER_CATEGORY sources per offensive category rather than
  // a flat limit across all categories. Filters to:
  //   - already classified (main_category set, not unclear_or_adjacent)
  //   - validation pass
  //   - ai_specificity_score >= 30 (meaningfully AI-specific)
  // Sources from each category are sorted by date so the most recent work
  // surfaces first; importance_tier (if set) is a secondary signal via
  // the intelligence jsonb but we keep the query simple.
  if (!category) {
    ...
    const perCatSources = await Promise.all(
      OFFENSIVE_CATS.map(async (cat) => {
        const { data, error } = await supabase
          ...
          .gte("ai_specificity_score", 30)
          .order("date_published", { ascending: false })
          .limit(SOURCES_PER_CATEGORY);
        if (error) throw new Error(`DB load failed for ${cat}: ${error.message}`);
        return data || [];
      })
    );
    const data = perCatSources.flat();
    if (!data.length) throw new Error("No classified sources found in the specified window");
    console.log(`  Loaded ${data.length} sources from Supabase (last ${days} days, ${SOURCES_PER_CATEGORY}/category)`);
    // Pass supabase so understandAllSources skips already-classified sources (fromDbRow path)
