...
import { extractAllEvidence }      from "./analysis/extractEvidence.js";
...
/**
 * Fetch sources for one category, tiered by importance label.
 * Priority order: critical → important → supporting.
 * Starred sources are always included (up to 10 extra, deduped).
 * Falls back to ai_specificity_score >= 30 if source_label column is absent.
 */
async function fetchCategorySources(supabase, cat, since) {
  ...
  // Fetch each label tier and starred in parallel
  const [criticalRes, importantRes, supportingRes, starredRes] = await Promise.all([
    base.eq("source_label", "critical").limit(CAP.critical),
    base.eq("source_label", "important").limit(CAP.important),
    base.eq("source_label", "supporting").limit(CAP.supporting),
    // Always include starred sources regardless of label
  ...
  for (const res of [criticalRes, importantRes, supportingRes, starredRes]) {
  ...
  // If the source_label column is not yet populated (new deployment, backfill
  // pending), fall back to fetching by ai_specificity_score so the pipeline
  // still works. Check: if no labelled sources found, use the fallback query.
  if (sources.length === 0) {
...
export async function runPipelineFromDB(supabase, opts = {}) {
  ...
  if (!category) {
    ...
    const byLabel = { critical: 0, important: 0, supporting: 0, other: 0 };
    for (const s of data) byLabel[s.source_label] = (byLabel[s.source_label] || 0) + 1;
    console.log(
      `  Loaded ${data.length} sources (last ${days}d) — ` +
      `critical:${byLabel.critical} important:${byLabel.important} ` +
      `supporting:${byLabel.supporting} unlabelled:${byLabel.other}`
    );
