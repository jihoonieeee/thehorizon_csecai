...
import { VALID_PRIMARY_TAGS, VALID_SECONDARY_DIMENSIONS } from "../../config/taxonomyRegistry.js";
...
function buildDeterministicTaxonomy(source) {
  ...
  // Infer rawfact_tags from existing tags + validated taxonomy fields
  const rawfactTagSet = new Set(source.tags || []);
  for (const t of source.understanding?.primary_threat_tags || []) {
    if (t.tag) rawfactTagSet.add(t.tag);
  }
  for (const d of source.understanding?.secondary_dimensions || []) {
    if (d) rawfactTagSet.add(typeof d === "string" ? d : d.tag);
  }
...
function buildUserPrompt(source) {
  ...
  const frameworkTagNames = [
    ...(u.primary_threat_tags || []).map((t) => `${t.tag}(${t.domain || "?"})`),
    ...(u.secondary_dimensions || []).map((d) => (typeof d === "string" ? d : d.tag)),
  ].filter(Boolean).join(", ");
...
function validateOutput(raw) {
  ...
  // Validate rawfact_tags against the canonical taxonomy registry.
  // Accept only known primary threat tags and secondary dimensions; drop invented strings.
  const validatedRawfactTags = ensureArray(out.rawfact_tags)
    .filter((t) => typeof t === "string" && (VALID_PRIMARY_TAGS.has(t) || VALID_SECONDARY_DIMENSIONS.has(t)))
    .slice(0, 10);
