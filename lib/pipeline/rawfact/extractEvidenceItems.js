  ...
  properties: {
    items: {
      ...
      items: {
        type: "object",
        required: [
          "evidence_type", "fact", "display_label", "source_quote",
          "type_justification", "entities", "category_hint", "evidence_confidence",
          "best_used_for",
        ],
        properties: {
          ...
          // type_justification: one sentence explaining why this fact qualifies for its evidence_type
          type_justification: { type: "string" },
          entities:           { type: "array", items: { type: "string" } },
          event_date:         { type: ["string", "null"], description: "YYYY-MM-DD if the fact describes a dated event, else null" },
          date_range: {
            type: ["object", "null"],
            properties: {
              start: { type: "string" },
          ...
          metric: {
            type: ["object", "null"],
            description: "Structured quantitative measurement if present in the fact",
            properties: {
              name:    { type: "string", description: "e.g. attack_success_rate, cve_count, affected_systems" },
              value:   { type: ["string", "number"] },
              unit:    { type: "string", description: "e.g. %, count, USD, days" },
              context: { type: "string", description: "e.g. against GPT-4 in zero-shot setting" },
            },
          },
...
A rawfact is NOT a summary. It is one precise, verifiable claim copied from a specific statement in the source text.
...
BAD (DO NOT extract):
  ...
  "Attackers can use AI to improve phishing." (vague prediction)

## GROUNDING — NON-NEGOTIABLE
Every item MUST include source_quote: a VERBATIM span copied EXACTLY from the SOURCE TEXT.
- Copy the words as they appear. Do NOT paraphrase, summarise, or fix grammar.
- The source_quote must contain the evidence for the fact.
...
1. Is this happening in the real world?       2. Is this newly possible (with explicit comparison)?
3. Does it affect many systems/sectors/orgs?  4. Does it change attacker operational capability?
5. Does it change defender priorities?        6. Is there direct evidence adversaries are using this?
7. Does it create a concrete exploitable dependency?  8. Is there a mandate, prohibition, or measurable control?

${UNIVERSAL_EXTRACTION_RULES}

## FIELDS

evidence_type       One value from allowed_evidence_types in the extraction profile. See type-specific rules above.

fact                ONE atomic claim. ≤25 words. Specific subject + specific assertion. THIS is the evidence.

display_label       DISPLAY-ONLY slide label, ≤8 words. NEVER used as evidence.

source_quote        VERBATIM span from SOURCE TEXT (≤200 chars). Copy exactly. This grounds the fact.

type_justification  ONE sentence explaining why this fact qualifies for the chosen evidence_type and why it is not a different type. Required.

entities            Named orgs, people, systems, CVEs, threat actors, products referenced in this fact.

event_date          YYYY-MM-DD if the fact describes a specific dated event, else null.

date_range          { start, end } if the fact covers a period (e.g. a campaign), else null.

metric              If the fact contains a specific quantitative measurement, extract as:
                    { name: "attack_success_rate", value: "87", unit: "%", context: "against GPT-4" }
                    Otherwise null. Do NOT use metric for vague estimates.
...
evidence_confidence
  ...
  low    — fact is inferred, or only a summary was available
...
function buildUserPrompt(source) {
  ...
  return [
    ...
    ``,
    `Extract up to ${maxItems} atomic, source-grounded claims. Each needs: fact, source_quote (verbatim), type_justification, and evidence_type from the allowed list.`,
  ].filter((l) => l !== "").join("\n");
}
...
function validateItem(raw, source, index) {
  ...
  return {
    ...
    fact,
    display_label:       typeof raw.display_label === "string" ? raw.display_label.trim().slice(0, 100) : fact.slice(0, 60),
    source_quote,
