...
export async function extractEvidenceItems(sources, opts = {}) {
  ...
  // Skip extraction for sources that were already processed and have rawfact_evidence
  // persisted to the DB (loaded back as source.rawfact_evidence). Reuse the stored
  // items as evidence_items_raw so the rest of the branch (scoring, triage, etc.) runs.
  let alreadyDone = 0;
  const toProcess = [];
  ...
  for (let i = 0; i < sources.length; i++) {
    const s = sources[i];
    const stored = s.rawfact_evidence;
    if (Array.isArray(stored) && stored.length > 0) {
      results[i] = { ...s, evidence_items_raw: stored };
      alreadyDone++;
    } else {
      toProcess.push({ index: i, source: s });
    }
  }

  if (alreadyDone > 0) {
    process.stdout.write(
      `  [L5A-extract] ${alreadyDone} source(s) skipped — rawfact_evidence already in DB\n`
    );
  }

  for (let b = 0; b < toProcess.length; b += concurrency) {
    const batch = toProcess.slice(b, b + concurrency);
    const batchResult = await Promise.all(batch.map(({ source }) => processOne(source, hasLlm)));
    for (let j = 0; j < batch.length; j++) {
      results[toProcess[b + j].index] = batchResult[j];
    }
  }
