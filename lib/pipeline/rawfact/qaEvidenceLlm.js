...
const BANDS = ["critical", "high", "medium", "low", "archive_only"];

function downgrade(item, steps, flag) {
  const cur = item.score_data?.evidence_priority || "medium";
  const idx = BANDS.indexOf(cur);
  const next = BANDS[Math.min(idx + steps, BANDS.length - 1)];
  return {
    ...item,
    score_data: {
      ...item.score_data,
      evidence_priority: next,
      evidence_score: Math.max(0, (item.score_data?.evidence_score ?? 50) - steps * 12),
    },
...
export async function runSecondModelEvidenceQa(sources, opts = {}) {
  ...
  for (let si = 0; si < sources.length && budget > 0; si++) {
    const items = (sources[si].evidence_items || []).filter((it) =>
      ["critical", "high"].includes(it.score_data?.evidence_priority)
    );
  ...
  async function verifyOne({ sourceIdx, items }) {
    ...
    try {
      ...
      for (const it of items) {
        ...
        if (after.score_data?.evidence_priority !== before.score_data?.evidence_priority) report.downgraded++;
