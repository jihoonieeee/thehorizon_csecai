...
// ── Strength resolver (categorical rank for representative pick; not a score) ──

import { strengthRank } from "../evidenceTriage/evidenceTriageVocab.js";
import { createHash } from "crypto";

function resolveItemScore(item) {
  return strengthRank(item.triage_data?.evidence_strength);
}

// ── Trust tier order for primary_origin selection ─────────────────────────────

const TRUST_TIER_ORDER = { primary: 5, curated: 4, high: 3, medium: 2, low: 1, unknown: 0 };
function trustRank(tier) {
  return TRUST_TIER_ORDER[tier] ?? 0;
}

// ── Event fingerprint for deduplication ──────────────────────────────────────

/**
 * Compute a fingerprint for an evidence item based on CVE ID,
 * incident date + victim, or attack type.
 * Items with the same fingerprint are de facto the same event.
 */
function computeEventFingerprint(item) {
  // CVE-based fingerprint
  const cveMatch = (item.fact || "").match(/CVE-\d{4}-\d{4,}/i);
  if (cveMatch) {
    return createHash("sha256").update(cveMatch[0].toUpperCase()).digest("hex").slice(0, 16);
  }

  // Date + first named entity
  const dateMatch = (item.event_date || item.date || "");
  const firstEntity = (item.entities || [])[0] || "";
  const attackType  = item.evidence_type || "";

  if (dateMatch && firstEntity) {
    const key = `${dateMatch}|${firstEntity.toLowerCase()}|${attackType}`;
...
export function clusterEvidenceItems(sources) {
  ...
  // Assign representative within each cluster (by trust tier first, then strength)
  for (const [cid, meta] of Object.entries(clusterMeta)) {
    const scored = meta.item_ids
      .map((i) => ({
        i,
        score:    resolveItemScore(allItems[i].item),
        trust:    trustRank(allItems[i].item._trust_tier || "unknown"),
      }))
      .sort((a, b) => (b.trust !== a.trust ? b.trust - a.trust : b.score - a.score));
    ...
    meta.rep_publisher_class = allItems[scored[0]?.i]?.item.publisher_class || null;
  }
  ...
  // Attach evidence_cluster to each item, including provenance dedup fields
  const updatedAllItems = allItems.map((entry, i) => {
    ...
    // Compute event_fingerprint for cross-source deduplication
    const event_fingerprint = computeEventFingerprint(entry.item);

    // primary_origin = highest-trust source in its cluster
    const primary_origin = is_rep;

    // independent_origin = source publisher_class differs from cluster rep's publisher_class
    let independent_origin = true;
    if (is_multi && !is_rep && meta) {
      const repEntry = meta.rep_id !== null ? allItems[meta.rep_id] : null;
      const repPubClass = repEntry?.item.publisher_class || meta.rep_publisher_class;
      const myPubClass  = entry.item.publisher_class;
      if (repPubClass && myPubClass && repPubClass === myPubClass) {
        independent_origin = false;
      }
    }

    // duplicate_reporting = same event_fingerprint, not primary_origin
    const duplicate_reporting = is_multi && !is_rep && !!event_fingerprint;
