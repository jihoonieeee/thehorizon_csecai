 ...
 *         relevance_tier, ai_specificity_score, main_category)
...
export function assessEvidenceEligibility(source) {
  ...
  const aiScore     = source.ai_specificity_score ?? 100;
  const mainCat     = source.main_category;

  // Hard discard: off-topic relevance
  ...
  // Low AI specificity, not protected → analytics only
  if (aiScore < 10 && !PROTECTED_TIERS.has(tier)) {
    return attach(source, "analytics_only", `ai_specificity_score=${aiScore} below threshold`, st);
  }
