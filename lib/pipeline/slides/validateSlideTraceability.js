...
function collectSlideVisualIds(slide) {
  ...
  for (const vizId of (slide.visualization_ids || [])) {
    ...
    if (vizId && !vizId.startsWith("fig_") && !vizId.startsWith("diagram_")) continue;
    visIds.add(vizId);
  }
...
function validateSlide(slide, registry) {
  ...
  // 7. AI-generated diagrams: must have source_evidence_id traceability and footnote
  if (slide.ai_diagram_spec) {
    const diag = slide.ai_diagram_spec;
    if (!diag.source_evidence_id) {
      errors.push(`${slot} AI diagram ${diag.visualization_id} has no source_evidence_id — provenance unverifiable`);
    }
    if (!diag.render_url && !diag.image_url) {
      errors.push(`${slot} AI diagram ${diag.visualization_id} has no render_url — cannot embed in slide`);
    }
    if (!diag.footnote) {
      errors.push(`${slot} AI diagram ${diag.visualization_id} is missing required footnote`);
    }
    if (!diag.ai_generated) {
      errors.push(`${slot} diagram ${diag.visualization_id} must be flagged ai_generated=true`);
    }
    if (diag.mermaid_dsl?.length < 10) {
      warnings.push(`${slot} AI diagram ${diag.visualization_id} has suspiciously short mermaid_dsl`);
    }
  }

  // 8. Analytics slides must have computation_method on their packets
  if (slide.slide_type === "analytics_pattern") {
    for (const ae of (slide.analytics_evidence || [])) {
      ...
      // Every analytics chart on a main slide must have input_evidence_ids (Rule 9 new)
      const inputIds = ae?.input_evidence_ids || ae?.source_evidence_ids || [];
      if (inputIds.length === 0 && slide.claim_id) {
        errors.push(`${slot} analytics chart ${id || "(unknown)"} has no input_evidence_ids — cannot trace to source evidence`);
      }

      // Analytics chart must be direct_support for main claim slides (Rule 9)
      const visualSupport = ae?.analytics_visual_support;
      if (visualSupport === "not_supporting" && slide.claim_id) {
        errors.push(`${slot} analytics chart ${id || "(unknown)"} is not_supporting — move to appendix or dashboard`);
      }
    }
  }

  // 9. Bullet role validation on analytical slides
  const ANALYTICAL_SLIDE_TYPES = new Set([
    "critical_claim", "evidence_support", "trend_claim", "outlook_6month",
    "recommendation", "case_study", "analytics_pattern", "evidence_gap",
  ]);
  if (ANALYTICAL_SLIDE_TYPES.has(slide.slide_type)) {
    for (const bullet of (slide.bullets || [])) {
      ...
      if (role && !VALID_ROLES.has(role)) {
        errors.push(`${slot} bullet has invalid bullet_role "${role}"`);
      }
      // finding/evidence bullets require supporting_evidence_id
      if ((role === "finding" || role === "evidence") && !bullet.supporting_evidence_id) {
        warnings.push(`${slot} ${role} bullet "${(bullet.text || "").slice(0, 50)}" missing supporting_evidence_id`);
      }
    }
  }

  // 10. Main analytical slides must have at least one EvidencePacket (unless evidence_gap)
  const REQUIRES_EVIDENCE = new Set(["critical_claim", "evidence_support", "trend_claim", "analytics_pattern"]);
  if (REQUIRES_EVIDENCE.has(slide.slide_type) && slide.claim_id) {
    const allEvidenceIds = collectSlideEvidenceIds(slide);
    if (allEvidenceIds.length === 0) {
      errors.push(`${slot} analytical slide type "${slide.slide_type}" has no supporting evidence packets — every analytical slide must cite at least one EvidencePacket`);
    }
  }

  // 11. External figures must have source_url (already checked in rule 6, consolidating)
  for (const vc of (slide.external_visual_callouts || [])) {
    if (!vc?.source_url) {
      errors.push(`${slot} external figure ${vc?.visualization_id} has no source_url — cannot prove provenance`);
    }
  }
