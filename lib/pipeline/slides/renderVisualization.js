...
export function renderVisualizationSpec(pptxSlide, spec, x, y, w, h, theme) {
  ...
  // AI-described placeholder (from visual planning pass — no render URL available).
  // Renders as a bordered box with the visual type label, description, and mandatory footnote.
  if (spec.ai_generated && !spec.render_url && !spec.image_url) {
    const boxH = Math.max(0.6, h - 0.30);
    pptxSlide.addShape("rect", {
      x, y, w, h: boxH,
      fill: { color: "F0F4F8" },
      line: { color: "CBD5E1", pt: 1, dashType: "dash" },
    });
    const typeLabel = (spec.visualization_type || "visual").toUpperCase().replace(/_/g, " ");
    pptxSlide.addText(typeLabel, {
      x: x + 0.08, y: y + 0.06, w: w - 0.16, h: 0.22,
      fontSize: 8, bold: true, color: theme.navy || "#1e3a5f",
      fontFace: theme.fontBody, charSpacing: 1,
    });
    if (spec.title) {
      pptxSlide.addText(spec.title.slice(0, 80), {
        x: x + 0.08, y: y + 0.30, w: w - 0.16, h: 0.26,
        fontSize: 10, bold: true, color: theme.dark || "#1a1a2e",
        fontFace: theme.fontTitle, wrap: true,
      });
    }
    if (spec.description) {
      pptxSlide.addText(spec.description.slice(0, 280), {
        x: x + 0.08, y: y + 0.60, w: w - 0.16, h: boxH - 0.80,
        fontSize: 9, color: "#475569", fontFace: theme.fontBody,
        wrap: true, valign: "top",
      });
    }
    const footnote = spec.footnote || "⚠ AI-described visual — illustrative only. Verify against cited evidence.";
    pptxSlide.addText(footnote, {
      x, y: y + boxH + 0.04, w, h: 0.20,
      fontSize: 7, italic: true, color: "#f59e0b", fontFace: theme.fontBody, wrap: true,
    });
    return true;
  }
