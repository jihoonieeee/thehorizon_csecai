...
function qaSlide(slide, opts = {}) {
  ...
  if (slide.headline) {
    ...
    if (strict) {
      const sanitized = sanitizeText(slide.headline, headlineIssues);
      // Headline can't be dropped — fall back to core_message when stat is unverified
      cleanedHeadline = sanitized !== null ? sanitized : (slide.core_message || slide.headline);
    }
  }
  ...
  for (const bullet of (slide.bullets || [])) {
    ...
    if (strict) {
      const cleaned = sanitizeText(bulletText, bIssues);
