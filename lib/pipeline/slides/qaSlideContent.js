 ...
 * When a conflict is detected, auto-reconcile by keeping the value from the
 * higher-trust slide (more evidence callouts → higher trust; tie-break by
 * claim_priority then slide_number). The lower-trust slide's key_fact is
 * rewritten to use the canonical value so the deck stays internally consistent.
 *
 * Only checks numbers ≥10 in evidence_callout key_facts (not bullet text).
 */
function checkCrossSlideStatConsistency(slides) {
  const issues = [];

  // Build map: context_key → [ { slide_number, value, text, callout, slide } ]
  ...
  for (const slide of slides) {
    for (const callout of (slide.evidence_callouts || [])) {
      ...
      for (const num of nums) {
        ...
        statsByContext.get(topicKey).push({ slide_number: slide.slide_number, value: n, text: text.slice(0, 80), num, callout, slide });
      }
    }
  }
  ...
  for (const [topic, entries] of statsByContext) {
    ...
    // Pick the canonical (highest-trust) entry: most evidence callouts → highest
    // claim priority → lowest slide number (most prominent).
    const ranked = [...entries].sort((a, b) => {
      const aCallouts = (a.slide.evidence_callouts || []).length;
      const bCallouts = (b.slide.evidence_callouts || []).length;
      if (bCallouts !== aCallouts) return bCallouts - aCallouts;
      const aPrio = PRIORITY_RANK[a.slide.claim_priority] || 0;
      const bPrio = PRIORITY_RANK[b.slide.claim_priority] || 0;
    ...
    // Rewrite non-canonical slides' key_fact to use the canonical numeric value
    for (const entry of ranked.slice(1)) {
      if (entry.value !== canonical.value && entry.callout) {
        const original = entry.callout.key_fact || "";
        entry.callout.key_fact = original.replace(
          new RegExp(`\\b${entry.num.replace(/[+%]/g, "\\$&")}\\b`, "g"),
          canonical.num
        );
      }
    }
    ...
    issues.push({
      ...
      severity:      "warning",
      topic,
      canonical_value: canonical.num,
      canonical_slide: canonical.slide_number,
      values:        entries.map((e) => ({ slide: e.slide_number, value: e.num })),
      label:         `Same topic "${topic.replace("|", " … ")}" had conflicting values across slides ${slideNums.join(", ")} — auto-reconciled to "${canonical.num}" from slide ${canonical.slide_number}`,
      reconciled:    true,
    });
  }
  return issues;
...
function checkBulletRoles(slide) {
  ...
  for (const bullet of (slide.bullets || [])) {
    ...
    // Blocking only on claim-anchored slides (where the LLM was instructed to
    // cite evidence for a specific judgment). On non-claim slides the bullets
    // come from analysis metadata, not LLM fabrication — warn only.
    if ((role === "finding" || role === "evidence") && !bullet.supporting_evidence_id) {
      issues.push({
        issue:    "finding_bullet_missing_evidence_id",
        severity: slide.claim_id ? "blocking" : "warning",
        label:    `Bullet (role=${role}) "${(bullet.text || "").slice(0, 60)}" must have supporting_evidence_id — ungrounded finding`,
        original_text: bullet.text || "",
      });
    }
...
export function checkBulletGrounding(slide) {
  ...
  for (const bullet of (slide.bullets || [])) {
    ...
    // Skip when the bullet cites a valid supporting_evidence_id that resolves — the
    // citation is the ground truth. Low overlap on a cited bullet means the LLM is
    // explaining a mechanism rather than repeating the metric verbatim, which is fine.
    // Only flag uncited bullets (evId present but doesn't resolve in evidenceById),
    // which indicates a fabricated or misattributed evidence reference.
    const overlap = contentWordOverlap(text, evidenceText);
    if (overlap < 0.15 && text.length > 30 && !evidenceById.has(evId)) {
      results.push({
        bullet,
        support_status: "unsupported",
        reason: `evidence_id "${evId}" not found in slide callouts — bullet may be fabricated or misattributed`,
