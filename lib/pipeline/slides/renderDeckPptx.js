...
function buildContentSlide(pptx, slide, pageNum, total) {
  ...
  if (showDiagram) {
    // Stacked: compact bullets at top, diagram full-width below — horizontal
    // diagrams (flowchart LR) need the full slide width to be legible.
...
function buildCaseStudySlide(pptx, slide, pageNum, total) {
  ...
  const bulletsH   = 1.55;    // was 2.05 — give the diagram more vertical real-estate
  ...
  if (slide.diagram_spec) {
    ...
    addBullets(s, bullets, MARGIN, bulletsTop, FULL_W, bulletsH, { fontSize: 12, spaceAfter: 4, max: 5 });
  } else {
