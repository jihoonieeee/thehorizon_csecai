...
function citationRuns(citeNums, footnoteMap, paraSpaceAfter) {
  ...
  return citeNums.flatMap((n, i) => {
    ...
    const pub = fn?.publisher ? clamp(fn.publisher, 20) : null;
...
function addFootnoteStrip(slide, footnotes, pageW) {
  ...
  footnotes.slice(0, 5).forEach((f, i) => {
    ...
    runs.push({ text: `${sep}[${f.num}] ${clamp(pub, 22)}`, options: { color: T.grey, fontSize: 8 } });
    if (url) {
      const domain = (() => { try { return new URL(url).hostname.replace(/^www\./, ""); } catch { return clamp(url, 30); } })();
      runs.push({ text: ` ${domain}`, options: { color: T.blue, fontSize: 8, hyperlink: { url }, italic: true } });
    }
  });
  slide.addText(runs, {
...
function buildContentSlide(pptx, slide, pageNum, total) {
  ...
  // outlook_structured removed from CENTERED — 5–8 bullets at variable length
  // look better top-aligned than compressed into a vertically centred block.
...
function buildCaseStudySlide(pptx, slide, pageNum, total) {
  ...
  if (slide.diagram_spec) {
    ...
    addBullets(s, bullets, MARGIN, bulletsTop, FULL_W, bulletsH, { fontSize: 12, spaceAfter: 4, max: 5, footnoteMap });
  } else {
    addBullets(s, bullets, MARGIN, topY, FULL_W, FOOTER_Y - topY - 0.08, { fontSize: 15, spaceAfter: 8, max: 5, footnoteMap });
  }
...
/** Section summary: dark navy divider listing the shift headlines for the category. */
function buildSectionSummary(pptx, slide) {
  const s = pptx.addSlide({ masterName: "DIVIDER" });
  const acc = T.accent;
  s.addShape("rect", { x: 0, y: 0, w: 0.22, h: H, fill: { color: acc }, line: { color: acc } });
  ...
  s.addText(clamp(slide.headline || "", 60), {
    x: 0.62, y: 2.2, w: W - 1.4, h: 1.0,
    fontSize: 34, bold: true, color: T.white, fontFace: T.fontTitle, align: "left", valign: "middle", wrap: true,
  });
  s.addShape("rect", { x: 0.64, y: 3.28, w: 2.6, h: 0.06, fill: { color: acc }, line: { color: acc } });

  // Shift headlines listed below — 0.44" gap after the accent bar to avoid crowding
  const headlines = (slide.shift_headlines || []).slice(0, 3);
  if (headlines.length) {
    const runs = [];
    headlines.forEach(h => {
      runs.push({ text: h, options: { color: "C7D6E5", bullet: { code: "2022", indent: 14 }, breakLine: true, paraSpaceAfter: 10 } });
    });
    s.addText(runs, {
      x: 0.64, y: 3.74, w: W - 1.4, h: 2.4,
      fontSize: 14, fontFace: T.fontBody, valign: "top", wrap: true,
    });
  }
...
function buildOverviewSlide(pptx, slide, pageNum, total) {
  ...
  addBullets(s, slide.bullets || [], MARGIN, top, FULL_W, contentH, {
    numbered: true, max: 5, fontSize: 19, spaceAfter: 22, valign: "middle",
  });
...
function buildStrategicShiftSlide(pptx, slide, pageNum, total) {
  ...
  // Maturity + confidence chip (top-right, safely inside the logo boundary)
  if (slide.maturity || slide.confidence) {
    ...
    s.addText(chip.toUpperCase(), {
      x: LOGO_SAFE_X - 2.6, y: TITLE_Y - 0.02, w: 2.4, h: 0.24,
      fontSize: 8, bold: true, color: T.grey, fontFace: T.fontBody, align: "right", charSpacing: 1,
    });
  }
  ...
  // IMPL_H increased to 0.72 to fit 2 lines at 13pt without clipping
  const IMPL_H = 0.72;
  ...
  if (slide.implication?.trim()) {
    ...
    // Label and text share a single text box to avoid overlap
    s.addText([
      ...
      x: MARGIN + 0.16, y: implY + 0.02, w: FULL_W - 0.22, h: IMPL_H - 0.06,
      fontFace: T.fontBody, valign: "middle", wrap: true,
    });
  }

  // Takeaway box below the title — taller to accommodate 3-line takeaways at 14pt
  ...
  if (slide.takeaway?.trim()) {
    ...
    s.addText(clamp(slide.takeaway, 300), {
      x: MARGIN + 0.18, y: y + 0.06, w: FULL_W - 0.26, h: TAKEAWAY_H - 0.12,
      fontSize: 14, color: T.navy, fontFace: T.fontBody, bold: false, valign: "middle", wrap: true,
    });
    y += TAKEAWAY_H + 0.16;
  }

  // Evidence bullets between takeaway and implication — top-aligned with spacing
  // (valign: "middle" produces a single floating bullet when only 1 fact exists)
  ...
  for (const slide of slides) {
    ...
    switch (type) {
      ...
      case "section_summary":
        slide.shift_headlines ? buildSectionSummary(pptx, slide) : buildSectionIntro(pptx, slide);
        break;
      case "overview":
