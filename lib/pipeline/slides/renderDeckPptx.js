...
import { drawAttackChain } from "../../slides/drawAttackChain.js";
...
const LEFT_W     = 6.40;        // narrowed to give the diagram panel more room
const RIGHT_X    = MARGIN + LEFT_W + 0.30;
const RIGHT_W    = W - RIGHT_X - MARGIN;  // ~5.5 in (was ~5.0)
...
// Silver line colour for separators
T.silver = "CBD5E1";
...
function addChrome(slide, pageNum, totalPages) {
  ...
  if (pageNum) {
    slide.addText(`${pageNum}${totalPages ? ` / ${totalPages}` : ""}`, {
      ...
      fontSize: FOOTNOTE_PT, color: T.grey, fontFace: T.fontBody, align: "right",
    });
  }
}
...
const TITLE_PT      = 26;
const TITLE_LINE_H  = 0.48;   // rendered line height for a 26pt bold headline

// ── Typography scale ─────────────────────────────────────────────────────────
// Single source of truth for font sizes. All slide builders reference these —
// never hardcode a body/kicker/chip size directly in a builder function.
const SUBHEAD_PT  = 19;   // takeaway box, overview statements
const BODY_PT     = 17;   // evidence bullets, implication text, case study body, outlook watch items
const KICKER_PT   = 11;   // category kicker line above title, implication label
const CHIP_PT     = 9;    // maturity/confidence chips, inline citation labels
const FOOTNOTE_PT = 8;    // footer strip, RESTRICTED marking, page numbers
...
function addTitle(slide, headline, kicker) {
  ...
  if (kicker) {
    slide.addText(kicker.toUpperCase(), {
      ...
      fontSize: KICKER_PT, bold: true, color: T.accent, fontFace: T.fontBody, charSpacing: 2,
    });
  }
...
// Build a set of citation runs for a bullet: one " [n](URL)" run per cited source,
// rendered as a small clickable hyperlink. PptxGenJS allows hyperlinks on individual
// text runs within the same addText call.
function citationRuns(citeNums, footnoteMap, paraSpaceAfter) {
  if (!citeNums?.length) return [];
  return citeNums.flatMap((n, i) => {
    const fn  = footnoteMap?.[n];
    const url = fn?.url || null;
    const pub = fn?.publisher ? clamp(fn.publisher, 32) : null;
    // Show  " [n]"  (no URL fallback) or  " [Publisher ↗]"  (hyperlinked)
    const label = pub ? ` [${pub}]` : ` [${n}]`;
    const run = {
      text:    label,
      options: {
        fontSize:  CHIP_PT,
...
function addBullets(slide, bullets, x, y, w, h, opts = {}) {
  ...
  // Build a num→footnote lookup from the slide's _footnotes array (if available
  // via opts.footnoteMap) so citation runs can show the publisher + hyperlink.
  const fmap = opts.footnoteMap || {};

  const runs = [];
  items.forEach((b, i) => {
    const cites     = (typeof b === "object" && Array.isArray(b?.cite_nums)) ? b.cite_nums : [];
    const lead      = opts.numbered ? `${i + 1}.  ` : "";
    const bodyText  = lead + bulletText(b);
    // Main bullet run — carries the bullet glyph via breakLine on the LAST run of
    // this paragraph (the citation run, or the body run when no citations).
    ...
    runs.push({
      ...
      options: {
        ...
        ...(!hasCites ? { breakLine: true, paraSpaceAfter: opts.spaceAfter ?? 10 } : {}),
      },
    });
    if (hasCites) {
      const cRuns = citationRuns(cites, fmap, opts.spaceAfter ?? 10);
      cRuns.forEach((r, ri) => {
        const isLast = ri === cRuns.length - 1;
        runs.push({ ...r, options: { ...r.options, ...(isLast ? { breakLine: true, paraSpaceAfter: opts.spaceAfter ?? 10 } : {}) } });
      });
    }
  });
...
/** Per-slide source strip — renders at the very bottom of a content slide
 *  (just above the chrome footer line) as small greyed source labels.
 *  Only shown when the slide carries _footnotes and at least one has a publisher.
 *  Returns the Y reserved for the strip (caller shrinks contentH to match). */
function addFootnoteStrip(slide, footnotes, pageW) {
  if (!footnotes?.length) return 0;
  const STRIP_H = 0.22;
  const STRIP_Y = FOOTER_Y - STRIP_H - 0.04;
  // Each footnote: "[n] Publisher — url" as a separate hyperlinked run.
  const runs = [];
  footnotes.slice(0, 5).forEach((f, i) => {
    const pub  = f.publisher || f.title || "Source";
    const url  = f.url || null;
    const sep  = i > 0 ? "     " : "";
    // Label run (not hyperlinked)
    runs.push({ text: `${sep}[${f.num}] ${clamp(pub, 32)}`, options: { color: T.grey, fontSize: FOOTNOTE_PT } });
    if (url) {
      const domain = (() => { try { return new URL(url).hostname.replace(/^www\./, ""); } catch { return clamp(url, 30); } })();
      runs.push({ text: ` ${domain}`, options: { color: T.blue, fontSize: FOOTNOTE_PT, hyperlink: { url }, italic: true } });
    }
  });
  slide.addText(runs, {
    x: MARGIN, y: STRIP_Y, w: pageW - MARGIN * 2 - 1.0, h: STRIP_H,
    fontFace: T.fontBody, valign: "middle", wrap: false,
  });
  return STRIP_H + 0.04;
}
...
function renderDiagram(slide, diag, x, y, w, h) {
  if (diag.diagram_type === "attack_chain") {
    drawAttackChain(slide, diag, x, y, w, h);
    return true;
  }
...
function buildSectionIntro(pptx, slide) {
  ...
  s.addText(slide.headline || slide.argument || "", {
...
// References per column/page. 7 keeps entries (title wraps to ~2 lines + domain
// + spacing) within the column height; 9 overflowed into the footer. Shared with
// the page-total calc in renderDeckPptx so numbering stays consistent.
const REF_PER_COL  = 7;
const REF_PER_PAGE = REF_PER_COL * 2;

function buildReferencesSlide(pptx, slide, pageNum, total) {
  const refs = (slide.bullets || []);
  const PER_COL  = REF_PER_COL;
  const PER_PAGE = REF_PER_PAGE;
  const pages = Math.max(1, Math.ceil(refs.length / PER_PAGE));
  ...
  // Extract domain from URL for a clean one-line source credit.
  const domain = url => { try { return new URL(url).hostname.replace(/^www\./, ""); } catch { return ""; } };

  function buildRuns(refList) {
    const runs = [];
    refList.forEach(b => {
      const num  = b.ref_num != null ? `[${b.ref_num}]  ` : "";
      const head = clamp(
        `${b.publisher || ""}${b.publisher && b.title ? " — " : ""}${b.title || bulletText(b)}`,
        120,
      );
      const dom  = b.url ? domain(b.url) : "";
      // Number bold navy + title regular dark — same paragraph, breakLine on title
      runs.push({ text: num,  options: { bold: true, color: T.navy, fontSize: 11 } });
      runs.push({ text: head, options: { color: T.dark, fontSize: 11, breakLine: true } });
      // Domain small blue italic, hyperlinked, spacing after entry
      if (dom) {
        runs.push({ text: dom, options: {
          color: T.blue, fontSize: 9, italic: true,
          ...(b.url ? { hyperlink: { url: b.url } } : {}),
  ...
  for (let p = 0; p < pages; p++) {
    ...
    const top = addTitle(s, pages > 1 ? `Source References (${p + 1}/${pages})` : "Source References");
    const h   = FOOTER_Y - top - 0.10;
    const chunk = refs.slice(p * PER_PAGE, (p + 1) * PER_PAGE);

    const leftRefs  = chunk.slice(0, PER_COL);
    const rightRefs = chunk.slice(PER_COL);

    // Thin divider between columns
    s.addShape("rect", {
      x: leftX + COL_W + COL_GAP / 2 - 0.005, y: top, w: 0.01, h: h,
      fill: { color: T.silver }, line: { color: T.silver },
    });

    if (leftRefs.length) {
      s.addText(buildRuns(leftRefs), {
        x: leftX, y: top, w: COL_W, h,
        fontFace: T.fontBody, valign: "top", wrap: true,
      });
    }
    if (rightRefs.length) {
      s.addText(buildRuns(rightRefs), {
        x: rightX, y: top, w: COL_W, h,
        fontFace: T.fontBody, valign: "top", wrap: true,
      });
    }
...
const ROLE_KICKER = {
  ...
  overview:                "Threat Landscape Overview",
};
...
function buildContentSlide(pptx, slide, pageNum, total) {
  ...
  // Build footnote lookup: num → { publisher, url }
  const footnoteMap  = Object.fromEntries((slide._footnotes || []).map(f => [f.num, f]));
  ...
  // Reserve bottom strip for per-slide source footnotes before computing contentH.
  const footnoteH    = addFootnoteStrip(s, slide._footnotes, W);
  const contentH     = FOOTER_Y - top - 0.12 - footnoteH;
  ...
  // outlook_structured has its own dedicated builder (buildOutlookSlide); it never reaches here.
  const CENTERED = new Set(["scope_methodology", "evidence_snapshot", "early_signals_watchlist"]);
  ...
  const bulletOpts = slide.type === "category_insights"
    ? { numbered: true, max: 3, fontSize: 20, spaceAfter: 26, valign: "middle", footnoteMap }
    : { ...(CENTERED.has(slide.type) ? { valign: "middle", spaceAfter: 12 } : {}), footnoteMap };
  if (showDiagram) {
    const bulletsH = Math.min(contentH * 0.36, 2.0);
    const gapY     = top + bulletsH + 0.12;
    const diagH    = FOOTER_Y - gapY - 0.10;
    addBullets(s, slide.bullets, MARGIN, top, FULL_W, bulletsH, { ...bulletOpts, max: 4 });
    s.addShape("rect", { x: MARGIN, y: gapY - 0.08, w: FULL_W, h: 0.02,
      fill: { color: T.silver }, line: { color: T.silver } });
    renderDiagram(s, slide.diagram_spec, MARGIN, gapY, FULL_W, diagH);
  } else {
...
function buildCaseStudySlide(pptx, slide, pageNum, total) {
  ...
  if (slide.named_entity) {
    ...
    s.addText(clamp(slide.named_entity, 100), { x: MARGIN + 0.12, y: topY, w: TITLE_W, h: 0.34, fontSize: 12, bold: true, color: T.white, fontFace: T.fontBody, valign: "middle" });
    topY += 0.46;
  }

  // Per-slide source footnote strip — drawn before computing bullet/diagram heights.
  const footnoteH = addFootnoteStrip(s, slide._footnotes, W);

  // Reserve a compact bullet strip at the bottom; the attack-chain diagram fills
  // the majority of the slide above it full-width — bigger = more readable.
  const bullets    = (slide.bullets || []).slice(0, 5);
  const bulletsH   = 1.55;
  const bulletsTop = FOOTER_Y - bulletsH - 0.05 - footnoteH;

  // The named-entity chip above already labels the case study. When the diagram's
  // caption repeats that same label, drop it so it isn't rendered twice.
  let diagramSpec = slide.diagram_spec;
  if (diagramSpec?.caption && slide.named_entity) {
    const normLabel = (v) => String(v || "").trim().replace(/\s+/g, " ").toLowerCase();
    if (normLabel(diagramSpec.caption) === normLabel(slide.named_entity)) {
      diagramSpec = { ...diagramSpec, caption: null };
    }
  }

  if (diagramSpec) {
    const diagH = Math.max(1.6, bulletsTop - topY - 0.12);
    renderDiagram(s, diagramSpec, MARGIN, topY, FULL_W, diagH);
    addBullets(s, bullets, MARGIN, bulletsTop, FULL_W, bulletsH, { fontSize: 14, spaceAfter: 6, max: 5, footnoteMap });
  } else {
    addBullets(s, bullets, MARGIN, topY, FULL_W, FOOTER_Y - topY - 0.08, { fontSize: BODY_PT, spaceAfter: 10, max: 5, footnoteMap });
  }

  addChrome(s, pageNum, total);
}

/** Section summary: clean white category divider — large name, thick accent underline,
 *  white space above and below so the slide breathes rather than looking crowded. */
function buildSectionSummary(pptx, slide, pageNum, total) {
  const s = pptx.addSlide({ masterName: "CONTENT" });

  const BAND_H = 2.72;
  const cx = MARGIN;
  const cw = LOGO_SAFE_X - MARGIN;

  // ── Full-width navy band (top 36% of slide) ────────────────────────────────
  s.addShape("rect", {
    x: 0, y: 0, w: W, h: BAND_H,
    fill: { color: T.navy }, line: { color: T.navy },
  });

  // Category name — white, anchored to the bottom of the band
  s.addText(slide.headline || "", {
    x: cx, y: 1.30, w: cw, h: 1.24,
    fontSize: 44, bold: true, color: T.white,
    fontFace: T.fontTitle, align: "left", valign: "bottom", wrap: true,
  });

  // ── White area below band ──────────────────────────────────────────────────

  // "IN THIS SECTION" label
  const headlines = (slide.shift_headlines || []).slice(0, 2);
  if (headlines.length) {
    s.addText("IN THIS SECTION", {
      x: cx, y: 3.18, w: cw, h: 0.32,
      fontSize: 13, bold: true, color: T.grey,
      fontFace: T.fontTitle, charSpacing: 3, align: "left",
    });

    // Shift headlines — bullet point form
    const runs = [];
    headlines.forEach((h, i) => {
      runs.push({
        ...
        options: {
          color: T.dark,
          bullet: { code: "2022", color: T.dark, indent: 18 },
          breakLine: true,
          paraSpaceAfter: i < headlines.length - 1 ? 22 : 0,
        },
      });
    });
    s.addText(runs, {
      x: cx, y: 3.58, w: cw, h: 2.60,
      fontSize: 20, fontFace: T.fontTitle, valign: "top", wrap: true,
    });
  }
...
/** Overview slide: opening landscape summary, numbered statements, no citations. */
function buildOverviewSlide(pptx, slide, pageNum, total) {
  const s = pptx.addSlide({ masterName: "CONTENT" });
  // No kicker on this slide — headline already identifies it as the period overview
  const top = addTitle(s, slide.headline || "Threat Landscape Overview");
  const contentH  = FOOTER_Y - top - 0.12;

  addBullets(s, slide.bullets || [], MARGIN, top, FULL_W, contentH, {
    numbered: true, max: 5, fontSize: SUBHEAD_PT, spaceAfter: 22, valign: "middle",
  });

  addChrome(s, pageNum, total);
}

/** Strategic shift slide: headline + takeaway box + evidence bullets + implication bar + maturity/confidence label. */
function buildStrategicShiftSlide(pptx, slide, pageNum, total) {
  const s = pptx.addSlide({ masterName: "CONTENT" });
  const cat    = slide.category ? (CATEGORY_LABELS[slide.category] || titleCase(slide.category)) : "";
  const kicker = cat ? `${cat}  ·  Strategic Shift` : "Strategic Shift";
  const footnoteMap = Object.fromEntries((slide._footnotes || []).map(f => [f.num, f]));

  let y = addTitle(s, slide.headline || "", kicker);

  // Maturity + confidence chip — bottom-right corner, next to page number
  if (slide.maturity || slide.confidence) {
    const chip = [titleCase(slide.maturity || ""), slide.confidence].filter(Boolean).join("  ·  ");
    s.addText(chip, {
      x: MARGIN, y: FOOTER_Y, w: W - MARGIN - 1.7, h: 0.24,
      fontSize: CHIP_PT, bold: true, color: T.grey, fontFace: T.fontBody, align: "right", charSpacing: 0,
    });
  }

  const footnoteH = addFootnoteStrip(s, slide._footnotes, W);
  const bottomY   = FOOTER_Y - footnoteH - 0.08;

  // Implication bar at the bottom (reserve space first).
  // Label and body text are the same font size so the run reads as one unit.
  const IMPL_H = 1.00;
  const implY  = bottomY - IMPL_H;
  if (slide.implication?.trim()) {
    s.addShape("rect", { x: MARGIN, y: implY - 0.06, w: FULL_W, h: 0.02, fill: { color: T.silver }, line: { color: T.silver } });
    s.addShape("rect", { x: MARGIN, y: implY, w: 0.07, h: IMPL_H - 0.04, fill: { color: T.amber }, line: { color: T.amber } });
    s.addText([
      { text: "Implication  ", options: { bold: true, color: T.amber, fontSize: BODY_PT, charSpacing: 0 } },
      { text: slide.implication, options: { color: T.dark, fontSize: BODY_PT } },
    ], {
      x: MARGIN + 0.16, y: implY + 0.04, w: FULL_W - 0.22, h: IMPL_H - 0.08,
      fontFace: T.fontBody, valign: "middle", wrap: true,
    });
  }

  // Takeaway box below the title
  const TAKEAWAY_H = 1.20;
  if (slide.takeaway?.trim()) {
    s.addShape("roundRect", {
      x: MARGIN, y: y, w: FULL_W, h: TAKEAWAY_H, rectRadius: 0.06,
      fill: { color: "EFF6FF" }, line: { color: "BFDBFE", pt: 1 },
    });
    s.addShape("rect", {
      x: MARGIN, y: y, w: 0.07, h: TAKEAWAY_H,
      fill: { color: T.blue }, line: { color: T.blue },
    });
    s.addText(slide.takeaway || "", {
      x: MARGIN + 0.18, y: y + 0.08, w: FULL_W - 0.26, h: TAKEAWAY_H - 0.16,
      fontSize: SUBHEAD_PT, color: T.navy, fontFace: T.fontBody, bold: false, valign: "middle", wrap: true,
    });
    y += TAKEAWAY_H + 0.18;
  }

  // Evidence bullets between takeaway and implication — top-aligned with spacing
  const evidenceH = Math.max(0.4, implY - y - 0.10);
  addBullets(s, slide.bullets || [], MARGIN, y, FULL_W, evidenceH, {
    max: 3, fontSize: BODY_PT, spaceAfter: 20, valign: "top", footnoteMap,
  });

  addChrome(s, pageNum, total);
}

/** Outlook slide: 3 watch-item cards with current signal and watch_for detail. */
function buildOutlookSlide(pptx, slide, pageNum, total) {
  ...
  const available = FOOTER_Y - top - 0.16;
  const cardH = Math.min(1.50, (available - (items.length - 1) * 0.14) / Math.max(items.length, 1));

  items.forEach((item, i) => {
    const cy = top + 0.08 + i * (cardH + 0.14);
    ...
    // Main watch text (upper portion of card). When a confidence chip sits in
    // the top-right, reserve its width so the title doesn't run under it.
    const textW = item.confidence ? FULL_W - 1.50 : FULL_W - 0.36;
    s.addText(item.text || "", {
      x: MARGIN + 0.18, y: cy + 0.08, w: textW, h: cardH * 0.44,
      fontSize: BODY_PT, color: T.navy, fontFace: T.fontBody, wrap: true, valign: "top",
    });

    // Signal + watch_for detail (lower portion, smaller text). Each on its own
    // line — bolded label then the text — so the two are visually distinct.
    const signalRuns = [];
    if (item.current_signal) {
      signalRuns.push({ text: "Signal: ",   options: { bold: true } });
      signalRuns.push({ text: item.current_signal, options: { breakLine: true } });
    }
    if (item.watch_for) {
      signalRuns.push({ text: "Watch for: ", options: { bold: true } });
      signalRuns.push({ text: item.watch_for, options: { breakLine: true } });
    }
    if (signalRuns.length) {
      s.addText(signalRuns, {
        x: MARGIN + 0.18, y: cy + cardH * 0.50, w: FULL_W - 1.50, h: cardH * 0.44,
        fontSize: KICKER_PT, color: T.grey, fontFace: T.fontBody, wrap: true, valign: "top",
      });
    }

    // Confidence chip — top-right of card
    if (item.confidence) {
      s.addText(item.confidence.toUpperCase(), {
        x: W - MARGIN - 1.20, y: cy + 0.10, w: 1.10, h: 0.24,
        fontSize: CHIP_PT, bold: true, color: T.accent, fontFace: T.fontBody, align: "right",
      });
    }
  });

  // Caveat note below cards
  if (slide.caveat) {
    const caveY = top + 0.08 + items.length * (cardH + 0.14) + 0.06;
    if (caveY < FOOTER_Y - 0.30) {
      s.addText(`Note: ${slide.caveat}`, {
        x: MARGIN, y: caveY, w: FULL_W, h: 0.28,
        fontSize: 11, italic: true, color: T.grey, fontFace: T.fontBody, wrap: true,
      });
    }
  }
...
export async function renderDeckPptx(deck, outputPath, opts = {}) {
  ...
  // Real page total must account for the references slide paginating.
  const refSlide = slides.find(s => (s.type || s.slide_type) === "references");
  const refExtra = refSlide ? Math.max(0, Math.ceil((refSlide.bullets || []).length / REF_PER_PAGE) - 1) : 0;
  ...
  for (const slide of slides) {
    ...
    switch (type) {
      ...
      case "section_summary":
        slide.shift_headlines
          ? buildSectionSummary(pptx, slide, page + 1, total)
          : buildSectionIntro(pptx, slide);
        break;
      case "overview":
        buildOverviewSlide(pptx, slide, page + 1, total);
        break;
      case "strategic_shift":
        buildStrategicShiftSlide(pptx, slide, page + 1, total);
        break;
      case "references": {
      ...
      case "outlook_structured":
        buildOutlookSlide(pptx, slide, page + 1, total);
        break;
      default:
