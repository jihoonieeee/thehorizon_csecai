...
function buildStrategicShiftSlide(pptx, slide, pageNum, total) {
  ...
  if (slide.takeaway?.trim()) {
    ...
    s.addText(clamp(slide.takeaway, 300), {
      ...
      fontSize: 15, color: T.navy, fontFace: T.fontBody, bold: false, valign: "middle", wrap: true,
    });
    y += TAKEAWAY_H + 0.18;
  }

  // Evidence bullets between takeaway and implication — vertically centred
  const evidenceH = Math.max(0.4, implY - y - 0.12);
  addBullets(s, slide.bullets || [], MARGIN, y, FULL_W, evidenceH, {
    max: 3, fontSize: 14, spaceAfter: 10, valign: "middle", footnoteMap,
  });
