...
function buildSectionSummary(pptx, slide) {
  ...
  // Shift headlines listed below
  const headlines = (slide.shift_headlines || []).slice(0, 3);
  if (headlines.length) {
    const runs = [];
    headlines.forEach((h, i) => {
      runs.push({ text: `${h}`, options: { color: "C7D6E5", bullet: { code: "2022", indent: 14 }, breakLine: true, paraSpaceAfter: 8 } });
    });
    s.addText(runs, {
      x: 0.64, y: 3.50, w: W - 1.4, h: 2.6,
...
function buildOverviewSlide(pptx, slide, pageNum, total) {
  ...
  const footnoteH = addFootnoteStrip(s, [], W);
  const contentH  = FOOTER_Y - top - 0.12 - footnoteH;
...
function buildStrategicShiftSlide(pptx, slide, pageNum, total) {
  ...
  // Maturity + confidence chip (top-right, above content)
  if (slide.maturity || slide.confidence) {
    ...
    s.addText(chip.toUpperCase(), {
      x: LOGO_SAFE_X - 2.2, y: TITLE_Y - 0.02, w: 2.6, h: 0.24,
