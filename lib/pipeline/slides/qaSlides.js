...
const QA_TYPES = new Set([
  // Legacy types (kept for backward compat with existing decks)
  ...
  // New strategic intelligence layer types
  "category_development", "category_insight",
  "overall_developments", "overall_insights",
  "outlook_tiered",
]);
...
export function qaSlides(slides, evidenceIndex = {}) {
  ...
  for (const slide of slides) {
    ...
    for (let bi = 0; bi < bullets.length; bi++) {
      ...
      // 5. Over-claim — a strong verb on a claim/data_point/mechanism bullet that
      //    the CITED evidence never uses. Only checked when the bullet resolves to
      //    specific evidence, so we compare against the right source text.
      //    Mechanism bullets are included: a walkthrough that says "the attacker
      //    compromised X" when the evidence only says "demonstrated" or "could" is
      //    still a hallucinated claim, regardless of bullet type.
      if (["claim", "data_point", "mechanism"].includes(bullet_type) && evidence_id && evidenceIndex[evidence_id]) {
        ...
        for (const { verb, roots } of ESCALATION) {
          if (verb.test(text) && !roots.some(r => evText.includes(r))) {
            issues.push({
              ...
              slide_type:   slide.type,
              type:         "overclaim",
              severity:     HEDGE_RE.test(evText) ? "fail" : "warning",
      ...
      // 6. Mechanism bullet with a numeric stat not grounded in any evidence.
      //    A walkthrough like "the attacker sent 50,000 phishing emails" where
      //    that number doesn't appear anywhere in the evidence corpus is a fabrication.
      if (bullet_type === "mechanism") {
        const stats = [...(text.matchAll(STAT_RE))].map(m => m[0]);
        for (const stat of stats) {
          const num = stat.match(/[\d.]+/)?.[0];
          if (num && !allEvidenceFacts.includes(num)) {
