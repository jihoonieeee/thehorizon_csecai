...
function slideTitle(slide, title, T, opts = {}) {
  slide.addText(title || "", {
    x: 0.55, y: opts.y ?? 0.42, w: LOGO_SAFE_X - 0.55, h: opts.h ?? 0.9,
    fontSize: opts.fontSize ?? 26, bold: true, color: opts.color || T.navyDk,
...
function slideHeadline(slide, text, T, opts = {}) {
  if (!text) return;
  slide.addText(text, {
    x: 0.55, y: opts.y ?? 1.32, w: W - 1.1, h: opts.h ?? 0.5,
    fontSize: opts.fontSize ?? 14, bold: true, color: opts.color || T.blue,
...
function pickViz(ids, vizSpecs, usedViz, opts = {}) {
  ...
  const renderable = (id) => vizSpecs.find((v) => v.visualization_id === id && !v.insufficient_data);
  for (const id of ids) {
...
function buildSectionDivider(pptx, slide, T) {
  ...
  s.addText(slide.title || "", {
    x: 0.75, y: 1.9, w: W - 1.5, h: 2.1,
    fontSize: 40, bold: true, color: T.white, fontFace: T.fontTitle, valign: "middle", wrap: true,
  });
  // Small category-colour rule under the title.
  s.addShape("rect", { x: 0.78, y: 4.05, w: 2.4, h: 0.06, fill: { color: acc }, line: { color: acc } });
  if (slide.core_message) {
    s.addText(slide.core_message, {
      x: 0.78, y: 4.3, w: W - 1.6, h: 1.2,
      fontSize: 15, color: "C7D6E5", fontFace: T.fontBody, wrap: true,
    });
  }
...
async function buildContentSlide(pptx, slide, T, vizSpecs, usedViz, evidenceByCat = {}, usedEvidenceIds = new Set()) {
  ...
  slideTitle(s, slide.title || "", T, { fontSize: 24 });
  if (slide.headline) slideHeadline(s, slide.headline, T);

  // Prefer a validated web figure on category slides; only fall back to a corpus
  // chart when no web figure exists or the corpus sample is large enough to headline.
  let vizSpec = null;
  if (slide.category && slide.category !== "cross_category") {
    ...
    if (!corpus || isLowSignalCorpusViz(corpus)) {
      vizSpec = pickCategoryFigure(slide.category, vizSpecs, usedViz) || corpus;
    } else {
      vizSpec = corpus;
    }
  } else {
  ...
  const bulletY     = slide.headline ? 2.05 : 1.55;
  const bulletH     = FOOTER_Y - bulletY - 0.1;

  if (slide.bullets?.length) {
    const items = slide.bullets.slice(0, 5).map((b) => ({
      text: b,
      options: { fontSize: 13, color: T.dark, bullet: { color: T.blue }, breakLine: true },
    }));
    s.addText(items, {
...
function buildExecOverviewSlide(pptx, slide, T, aggregates) {
  ...
  if (slide.bullets?.length) {
    const items = slide.bullets.slice(0, 4).map((b) => ({
      text: b, options: { fontSize: 11.5, color: T.dark, bullet: { color: T.navy }, breakLine: true },
    }));
    s.addText(items, {
...
async function buildLandscapeSlide(pptx, slide, T, vizSpecs, aggregates, usedViz) {
  ...
  const vizSpec = pickViz(slide.visualization_ids, vizSpecs, usedViz, { allowReuse: true })
               || (vizSpecs || []).find((v) => v.visualization_id === "category_distribution");
  ...
  if (slide.bullets?.length) {
    ...
    const items = slide.bullets.slice(0, 5).map((b) => ({
      text: b, options: { fontSize: 13, color: T.dark, bullet: { color: T.blue }, breakLine: true },
    }));
    s.addText(items, {
...
function buildConclusionSlide(pptx, slide, T) {
  ...
  s.addText(slide.title || "Key Takeaways", {
    x: 0.6, y: 0.5, w: W - 1.2, h: 0.88,
    fontSize: 28, bold: true, color: T.white, fontFace: T.fontTitle,
  });
  s.addShape("rect", { x: 0.6, y: 1.32, w: W - 1.2, h: 0.05, fill: { color: T.blue }, line: { color: T.blue } });
  ...
  (slide.bullets || []).slice(0, 5).forEach((b, i) => {
    ...
    s.addText(b, {
      x: 0.92, y, w: W - 1.5, h: 0.8, fontSize: 12, color: T.white, fontFace: T.fontBody, valign: "middle", wrap: true,
    });
  });
