...
function isRenderableSpec(spec) {
  ...
  if (spec.source === "external_web" || spec.visualization_type === "ai_diagram") return true;
  // Internal analytics specs: renderable when they have non-empty numeric chart_data
  const cd = spec.chart_data || {};
  return !!(
    (Array.isArray(cd.items)   && cd.items.length > 0)   ||
    (Array.isArray(cd.stacks)  && cd.stacks.length > 0)  ||
    (Array.isArray(cd.series)  && cd.series.length > 0)  ||
    (Array.isArray(cd.axes)    && cd.axes.length > 0)    ||
    (Array.isArray(cd.rows)    && cd.rows.length > 0)    ||
    (Array.isArray(cd.events)  && cd.events.length > 0)  ||
    (Array.isArray(cd.gauges)  && cd.gauges.length > 0)
  );
}
...
async function buildContentSlide(pptx, slide, T, vizSpecs, usedViz, evidenceByCat = {}, usedEvidenceIds = new Set()) {
  ...
  // Fallback: use visual_requirement from inline visual planning or generateVisualPlanning.
  // When render_url is present (inline Mermaid diagram) → render as ai_diagram image.
  // When no render_url → render as labelled placeholder box with AI description.
  if (!vizSpec && slide.visual_requirement?.visual_type && slide.visual_requirement.visual_type !== "none") {
    ...
    if (vr.render_url) {
      vizSpec = {
        ...
        title:              vr.title || slide.title,
        caption:            vr.description || "",
        render_url:         vr.render_url,
        ai_generated:       true,
        footnote:           vr.footnote || "⚠ AI-generated diagram — illustrative only. Verify against cited evidence.",
      };
    } else {
      vizSpec = {
        visualization_type: vr.visual_type,
        title:              vr.title || slide.title,
        description:        vr.description || "",
        ai_generated:       true,
        footnote:           vr.footnote || "⚠ AI-generated visual — illustrative only. Verify against cited evidence.",
      };
    }
  }
