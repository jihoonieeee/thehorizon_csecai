...
export async function extractStrategicThemes(
  ...
  try {
    ...
    // Log the field keys of the first returned theme so schema drift is immediately visible
    if (themes.length > 0) {
      process.stdout.write(`  [L7-strategic-themes] first theme keys: ${Object.keys(themes[0]).join(", ")}\n`);
    }

    // Normalise common LLM field deviations before validating.
    // Anthropic returns title/description/categories_spanned rather than the
    // schema names theme_title/theme_summary/categories_involved.
    const toArray = (v) => {
      if (Array.isArray(v)) return v;
      if (typeof v === "string" && v) return v.split(/[,;]+/).map((s) => s.trim()).filter(Boolean);
      return [];
    };
    const normalised = themes.map((t) => ({
      ...t,
      theme_id: t.theme_id || t.id || `theme_${Math.random().toString(36).slice(2, 8)}`,
      theme_title:   t.theme_title   || t.title       || "",
      theme_summary: t.theme_summary || t.summary      || t.description || "",
      categories_involved: (() => {
        for (const v of [t.categories_involved, t.categories, t.categories_spanned, t.involved_categories]) {
          const arr = toArray(v);
    ...
    // Drop themes that are still missing core fields after normalisation
    const valid = normalised.filter((t) =>
      t.theme_id && t.theme_title && t.theme_summary &&
      t.categories_involved.length > 0 && t.confidence
    );
    ...
    if (valid.length < themes.length) {
      ...
      for (const t of normalised.filter((t) =>
        !t.theme_id || !t.theme_title || !t.theme_summary ||
        t.categories_involved.length === 0 || !t.confidence
      )) {
        const missing = [
          !t.theme_id      && "theme_id",
          !t.theme_title   && "theme_title",
          !t.theme_summary && "theme_summary",
          t.categories_involved.length === 0 && "categories_involved",
          !t.confidence    && "confidence",
        ].filter(Boolean);
        process.stdout.write(`    dropped: "${(t.theme_title || t.id || "(no title)").slice(0, 50)}" — missing: ${missing.join(", ")}\n`);
      }
    }
