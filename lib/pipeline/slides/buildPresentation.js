...
export async function buildPresentation(categoryAnalysesRaw, crossCategory, evidenceItemsRaw, opts = {}, externalSlidePlan = null) {
  // ── Normalise: accept new insight shape OR legacy judgment shape ──────────────
  // New shape (from runAnalysis): ca.insights[]
  // Legacy shape (from old synthesizeAllCategories): ca.judgments[]
  const categoryAnalyses = (categoryAnalysesRaw || []).map(ca => {
    if (ca.judgments?.length || !ca.insights?.length) return ca; // already legacy shape
    // Map new insight fields → judgment fields so the rest of buildPresentation
    // works unchanged. The new shape is the canonical one going forward.
    const judgments = (ca.insights || []).map(ins => ({
      judgment_id:       ins.insight_id,
      judgment:          ins.title,
      short_takeaway:    ins.title,
      what_changed:      ins.what_changed,
      causal_mechanism:  ins.mechanism,
      why_this_matters:  ins.implication,
      evidence_maturity: ins.evidence_maturity,
      confidence:        ins.confidence,
      evidence_for:      ins.evidence_item_ids || [],
      evidence_against:  [],
      caveats:           ins.caveats || [],
      technique_focus:   ins.technique_tags || [],
      monitoring_signals: ins.monitoring_signal
        ? [{ signal: ins.monitoring_signal, why_it_matters: "", escalation_trigger: "" }]
        : [],
      recommended_action: "",
      blocked:            ins.blocked || false,
      qa_issues:          ins.qa_issues || [],
    }));
    return {
      ...ca,
      judgments,
      approved_judgment_count: judgments.filter(j => !j.blocked).length,
      blocked_judgment_count:  judgments.filter(j => j.blocked).length,
      evidence_ids: ca.insights.flatMap(i => i.evidence_item_ids || []),
    };
  });
  ...
  // Evidence packs keyed by category
  const evidenceByCategory = {};
  for (const ca of categoryAnalyses) {
    const ids = new Set(ca.evidence_ids || []);
    const items = evidenceItems.filter(ei => ids.has(ei.evidence_id) || ei.category === ca.category);
