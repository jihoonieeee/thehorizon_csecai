...
function buildCatInsightsUser(catLabel, judgments, evidenceIndex) {
  const lines = [
    `CATEGORY: ${catLabel}`,
    ``,
    `TOP INSIGHTS AND HAPPENINGS THIS PERIOD (${judgments.length} total — distil into 3–5 bullets):`,
  ];
  for (const j of judgments) {
    ...
    const evSample = (j.evidence_for || []).slice(0, 3).map(id => evidenceIndex[id]).filter(Boolean);
    for (const ev of evSample) {
      lines.push(`  Evidence [${ev.evidence_id}]: ${ev.fact.slice(0, 200)}`);
    }
  }
  lines.push(`\nWrite ONE overview slide with 3–5 bullets. Most impactful insight first.`);
  lines.push(`Return JSON: { "headline": "...", "bullets": [...], "speaker_notes": "...", "visual_suggestion": "..." }`);
  return lines.join("\n");
}
    ...
    const generated = await Promise.all(batch.map(async plan => {
      ...
      if (plan.type === "category_insights" && !opts.skipLlm) {
        ...
        try {
          ...
          const usr = buildCatInsightsUser(catLabel, catJudgments, evidenceIndex);
          try {
            const { result } = await routedLLM(sys, usr, { task: "category_insights", requires_json: true, schema: SLIDE_SCHEMA });
            raw = typeof result === "string" ? JSON.parse(result) : result;
          } catch {
