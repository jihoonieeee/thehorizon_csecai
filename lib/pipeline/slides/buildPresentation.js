...
// no version strings / CVE IDs, and a ≤16-word cap truncated at a word boundary.
const VERSION_RE = /\bv?\d+\.\d+(?:\.\d+)?(?:[–-]\d+\.\d+(?:\.\d+)?)?\b/g;   // 1.139.0, v2.9, 1.82.7-1.82.8
...
A case study tells ONE concrete attack story anchored on ONE named entity. The evidence you
are given all comes from a SINGLE incident — tell THAT one story start to finish. Do NOT turn
it into a list of several unrelated incidents; if the evidence hints at multiple events, pick
the ONE with the clearest chain and stick to it.
  named_entity : the specific product, victim org, malware family, or threat actor this ONE case is about.
  headline     : ≤10 words, plain English, naming what happened to whom. e.g. "Booby-trapped AI plugin reached 26,000 company accounts".

PLAIN-LANGUAGE RULES (critical for this audience):
  - Translate every technical term. "ASGI middleware trusted upstream headers" → "the system trusted a forged ID and let attackers in without a password".
  - NO CVE numbers, NO version strings, NO acronyms unless universally known; gloss anything technical in plain words.
  - Lead with impact and stakes. A non-technical reader must grasp WHY THEY SHOULD CARE from each bullet.

bullets (3-4) — walk ONE story: what the attacker did → how it worked → how bad it got → what it means:
  - "claim" / "data_point" = one concrete PLAIN-LANGUAGE step or fact from the story (what happened, the scale, the impact). MUST cite an evidence_id.
  - "implication" = what this means for the business (which protection failed, what's now exposed) — in business terms. Cite the evidence_id that drives this implication. The LAST bullet.
  Do NOT write a recommendation/action bullet.

ONE BULLET = ONE FACT (critical): each Evidence bullet states a fact from EXACTLY ONE evidence
item and cites THAT item. NEVER stitch an actor from one item to a number or victim from another.
Rules: ≤20 words/bullet, plain English, no jargon, no source names/IDs in text (citations are added automatically). Evidence bullets MUST set evidence_id. NEVER start bullet text with a dash or bullet character.
...
speaker_notes: 2-3 sentences of nuance (confidence, what to watch). No restating bullets.
...
A slide has a STRUCTURE the reader must grasp in one glance:
  ...
  • BULLETS   = the support, where EACH bullet is exactly ONE of:
      ...
      Mechanism      — HOW the attack works, in 1 plain-English sentence (e.g. "Attacker embeds a hidden instruction in a calendar invite; the AI assistant reads it, silently exfiltrates the thread, and replies as the attacker").
...
NEVER write a Recommendation, action, or "defenders should…" bullet — this briefing gives NO recommendations.
...
════ HEADLINE ════
One declarative sentence ≤12 words. It must NAME A SPECIFIC ENTITY — a product, technique,
actor group, CVE, measurement, or named tool — and state the CONSEQUENCE.

REQUIRED: your headline must answer "WHAT specifically does WHAT to WHOM?"
  BAD:  "AI Security Challenges Are Growing" — names nothing, proves nothing
  BAD:  "Researchers Discover New Vulnerabilities" — generic, could apply to any domain
  BAD:  "Agentic AI Threats Are Evolving" — pure topic label, zero analytical content
  GOOD: "Many-shot jailbreaking bypasses Claude 3 Opus and GPT-4 safety at 80%+"
  GOOD: "LangChain MCP versions below 0.3.15 allow credential theft via tool-call hijack"
  GOOD: "WormGPT successors cut CVE-to-exploit time below 48 hours for criminal groups"

HEADLINE CALIBRATION — the verb must match the evidence maturity:
  research_demonstration  → "demonstrated", "shown in lab", "achieves in controlled test"
  disclosed_vulnerability → "enables", "a CVE confirms", "patched in [version]"
  observed_exploitation   → "exploited in the wild", "observed in [N] incidents"
  adversary_adoption      → "[named actor] is using", "adopted by [group]"
  operational_campaign    → "sustained campaign", "confirmed at scale"
  NEVER escalate: one research paper ≠ "confirmed in the wild", one CVE ≠ "being exploited"

════ BULLETS — MANDATORY 4-BULLET STRUCTURE ════
Every KEY DEVELOPMENT slide has EXACTLY this sequence — no exceptions, no substitutions:
  ...
  Bullet 1 — Evidence  (bullet_type: "claim" or "data_point")
    ...
    MUST cite evidence_id. ≤20 words. Active voice.
    BAD:  "Research shows AI-assisted phishing is more effective." (names nothing)
    GOOD: "GPT-4o personalised phishing emails achieved 54% click rate vs 12% for templates."

  Bullet 2 — Mechanism  (bullet_type: "mechanism")
    HOW the attack works, as a causal chain starting with the attacker's action.
    Cite the evidence_id this mechanism derives from (same item as Bullet 1 is fine). ≤20 words.
    BAD:  "This is possible because LLMs can generate personalised content." (describes, not chains)
    GOOD: "Attacker feeds LinkedIn profile to GPT-4o; model writes unique, role-accurate lure per target."

  Bullet 3 — Implication  (bullet_type: "implication")
    WHICH specific defender control or assumption now fails, and why.
    Cite the evidence_id that drives this implication (same item as Bullet 1 is fine). ≤20 words.
    BAD:  "Defenders should be aware of this threat." (no specific control named)
    GOOD: "Signature-based email filters fail — every email is unique, so no shared pattern to block."

Do NOT write a recommendation/action bullet — this briefing does not give recommendations.
Use 3 bullets: Evidence → Mechanism → Implication.

FORBIDDEN patterns (will be flagged as failures):
  ✗ Three evidence bullets in a row (you enumerated facts, not an analysis)
  ✗ "Researchers found that..." as bullet 2 or beyond (that's more evidence, not mechanism)
  ✗ A recommendation or "defenders should…" bullet
  ✗ CVE numbers or version strings in any bullet
  ✗ Any bullet without clear type identity

Each bullet: text ≤ 20 words, active voice, no dashes/hyphens at start, no source names or IDs in text.
...
// Dedicated system prompt aligned with buildBatchSlideUser. Do NOT use
// buildSlideSystem() for the batch — its "headline must name a specific entity"
// and Evidence/Mechanism/Implication rules CONFLICT with the theme structure
// (keep-headline-verbatim, Pattern/Proof/So-what), producing drift.
function buildThemeSlideSystem() {
  return `You write ONE strategic-insight slide for a CISO board briefing. The headline (given to you)
is the insight. Write 3 short bullets a NON-TECHNICAL board member understands in one glance.

════ THE BAR — apply to EVERY bullet ════
  • ONE idea per bullet. Maximum 14 words. No semicolons. No "A, B, C and D" lists.
  • Plain English only. NO version numbers, NO CVE IDs, NO acronyms (RCE, SCA, SAST, PRNG, IAM,
    EXIF, MCP…), NO product-internal jargon. If a smart non-engineer would need to look it up, cut it.
  • Say what it MEANS at a high level — not the technical mechanism, not a catalogue of techniques.

════ THE TEST (your example is the exact failure to avoid) ════
  ✗ BAD (dense, jargon, five ideas crammed in):
    "Supply-chain attacks now span typosquatted model repos, deterministic bucket squatting, path
     traversal, PRNG hijacking, and compilation-triggered backdoors."
  ✓ GOOD (one idea, plain, high-level):
    "Attackers now hide malware inside AI models on public download sites."

  ✗ BAD (version numbers, two incidents, acronym):
    "Vertex AI SDK versions 1.139.0-1.140.0 enabled RCE via predictable bucket naming."
  ✓ GOOD:
    "One fake AI model was downloaded 200,000 times before anyone noticed."

  ✗ BAD (a recommendation — banned):
    "Mandate cryptographic artifact signing for every model pull."
  ✓ (omit entirely — this briefing gives NO recommendations)

════ THE 3 BULLETS, in this order ════
  1. THE SHIFT   (bullet_type "claim")      — what attackers can now do, in one plain sentence. Cite one evidence_id.
  2. THE PROOF   (bullet_type "data_point") — the single most striking real example or number. Cite one evidence_id.
  3. WHY IT MATTERS (bullet_type "implication") — the defense or assumption that no longer works. Cite the evidence_id that drives this implication.

Keep the supplied headline VERBATIM. Never write a recommendation bullet.
Match the verb to the evidence — a lab result is "researchers showed", not "attackers are using".

Return ONLY valid JSON: { "slides": [ { headline, bullets[], speaker_notes, visual_suggestion } ] }`;
}
...
function buildBatchSlideUser(plans, evidenceIndex, judgmentIndex) {
  ...
  const lines = [
    ...
    ``,
    `WHAT EACH SLIDE IS: ONE strategic insight — a shift in the threat landscape. The headline is`,
    `the insight (already written for you). Three bullets EXPLAIN and PROVE it in plain language.`,
    `This is not a data dump. Each bullet is a clear, readable sentence a busy executive follows`,
    `on first read.`,
    ``,
    `━━ WRITE PLAINLY — THIS IS THE MOST IMPORTANT RULE ━━`,
    `  ✗ NEVER write CVE numbers ("CVE-2026-55574"). Say "a critical flaw in the AI model server".`,
    `  ✗ NEVER write version strings ("1.82.8", "1.139.0–1.140.0"). Say "a poisoned package release".`,
    `  ✗ NEVER stack more than 2 product/tool names in one bullet. Gloss the one you keep.`,
    `  ✗ NEVER cram 5 techniques into one sentence. Name the pattern + at most 2-3 plain examples.`,
    `  ✓ Keep ONE striking number per bullet at most (a %, a count, a $ figure).`,
    `  ✓ Explain the idea, don't just name it. The reader should UNDERSTAND, not decode jargon.`,
    ``,
    `━━ HEADLINE ━━`,
    `Use the short headline supplied below VERBATIM (it is already the insight, ≤10 words).`,
    `Do NOT lengthen it, do NOT replace it with a finding.`,
    ``,
    `━━ BULLET STRUCTURE (exactly 3, in this order) ━━`,
    `  1. Pattern      — what is converging, in plain words. Name the sub-vectors as everyday phrases`,
    `                     (e.g. "through images, audio, and multi-step chains"). bullet_type "claim".`,
    `                     Cite the strongest evidence_id.`,
    `  2. Proof        — the 1-2 most striking concrete facts, in plain language, from DIFFERENT`,
    `                     sources. Keep the sharpest number. bullet_type "data_point". Cite an evidence_id.`,
    `  3. So what      — which defender assumption no longer holds, and why it matters. Plain,`,
    `                     one clear sentence. bullet_type "implication". Cite the evidence_id that drives this implication.`,
    ``,
    `Do NOT write a recommendation/action bullet. Exactly 3 bullets. Each ≤22 words, plain English.`,
  ];
  ...
  for (let i = 0; i < plans.length; i++) {
    ...
    lines.push(`\nWHAT CHANGED (the concrete proof — use plainly, drop the CVE/version noise):`);
    lines.push(changed || "(derive from evidence below)");
    lines.push(`\nWHY THIS IS POSSIBLE NOW (background for the Pattern bullet):`);
    lines.push(mechanism || "(derive from evidence below)");
    lines.push(`\nWHICH ASSUMPTION BREAKS (for the "So what" bullet):`);
    ...
    const evLines = evForSlide.map(ev => {
      ...
      const numStr = (ev.numbers || []).length
        ? `  Numbers: ${ev.numbers.map(n => `${n.value} (${n.context})`).join(", ")}`
        : "";
      return `[${ev.evidence_id}][${ev.evidence_type}][${pub}] ${ev.fact}${numStr ? "\n" + numStr : ""}`;
    }).join("\n\n");

    lines.push(`\nEVIDENCE ACROSS SOURCES (cite several of these, from DIFFERENT publishers, in bullets 1-2):\n${evLines || "(none)"}`);
  }
...
function deterministicCategoryPlan(ca, pack, evidenceIndex = {}, preselectedCaseStudy = null) {
  ...
  // Each theme → one INSIGHT slide (headline = the strategic insight, bullets =
  // the converging pattern across MULTIPLE sources). Up to 4 themes per category.
  // The old separate `category_insights` slide was removed — it was redundant with
  // theme slides and had a serialization bug ([object Object]).
    ...
    const generated = await Promise.all(batch.map(async plan => {
      ...
      try {
        ...
        return {
          ...
          bullets:           (raw?.bullets || []).slice(0, isCase ? 4 : 4).map(normaliseBullet),
          speaker_notes:     raw?.speaker_notes || "",
          citations:         raw?.citations || plan.evidence_ids || [],
          visual_suggestion: suggestion,
