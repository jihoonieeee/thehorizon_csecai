...
// ── Sharp signals + 6-month outlook — ONE combined LLM call ──────────────────
// Replaces two deterministic slides (early_signals_watchlist + outlook_structured)
// with a single LLM call that produces exactly 5 named predictions. Each prediction
// names a specific technique/actor/threshold and states the implication.
// This is the most valuable slide in the deck for a forward-looking CISO audience.
...
const SIGNALS_SCHEMA = {
  ...
  properties: {
    ...
    signals: {
      ...
      items: {
        type: "object",
        required: ["prediction", "mechanism", "implication", "evidence_basis", "confidence", "category"],
        properties: {
...
function buildSignalsSystemPrompt() {
  return `You are the principal threat analyst writing the FORWARD SIGNALS slide for a CISO briefing. This is the most important slide in the deck — it tells the CISO what is coming in the next 6 months, with enough specificity to drive resourcing decisions.

PRODUCE EXACTLY 5 predictions. Each must:
  1. NAME a specific technique, attack vector, threat actor type, target system, or threshold — not a generic trend.
  2. STATE THE MECHANISM: one plain-English sentence explaining HOW the attack will work.
  3. STATE THE IMPLICATION: what defenders must change, which control breaks, or what the blast radius is.
  4. CITE EVIDENCE BASIS: what observed behaviour or research grounds this prediction.

QUALITY STANDARD — apply to every prediction before writing:
  BAD:  "AI-assisted social engineering will grow." (names nothing, predicts nothing falsifiable)
  GOOD: "Social engineering via AI-synthesised voice and document will outpace SMS phishing as the primary initial-access vector for business email compromise, because real-time voice-cloning tools now cost under $5/month and require only a 30-second audio sample." (names technique + cost threshold + mechanism + why now)

  BAD:  "Agentic AI systems will face more attacks." (tautology)
  GOOD: "Prompt injection delivered through shared calendar invites and email auto-responders will be weaponised against AI coding assistants to silently exfiltrate repository credentials, targeting developers who have granted agents ambient access to their inboxes." (attack path + target + payload + why possible)

EXAMPLES of the specificity level required:
  • "Fully automated AI malware that modifies its own evasion logic between executions will appear in ransomware affiliate toolkits within 6 months, exploiting the fact that most EDR signatures are static."
  • "Multimodal and multi-step prompt injection chains (image → audio → function call) will bypass text-only guardrails in production LLM APIs, because safety classifiers are trained on text prompts and not on serialised tool-call sequences."
  • "AI-enabled manipulation of crypto-trading algorithms via poisoned social-media sentiment feeds will be attempted by financially-motivated actors, targeting DEX arbitrage bots that ingest unverified on-chain analytics."
  • "Nation-state actors will deploy LLM-generated CVE exploits within 48 hours of NVD publication, compressing the patch-or-exploit window below enterprise patching cycles."
  • "Model context protocol (MCP) servers exposed without authentication will become a primary lateral-movement vector once agentic AI pipelines reach enterprise scale, because MCP has no built-in credential binding."

CONFIDENCE RULES:
  High   — ≥2 observed-exploitation or adversary-adoption signals exist, or the technique is already partially deployed
  Medium — 1 observed-exploitation signal, or 2+ research demonstrations with clear path to weaponisation
  Low    — research-only, or extrapolation from adjacent domain

Return ONLY valid JSON.`;
}

function buildSignalsUserPrompt(categoryAnalyses, allOutlooks) {
  const lines = [];
  lines.push("CATEGORY OUTLOOKS (use as evidence base for predictions):");
  for (const ca of categoryAnalyses) {
    const label = CATEGORY_LABELS[ca.category] || ca.category;
    const out   = allOutlooks?.byCategory?.[ca.category];
    // Top approved judgments (these are the strongest analytical conclusions)
    const topJ  = (ca.judgments || []).filter(j => !j.blocked).slice(0, 3);
    lines.push(`\n=== ${label} ===`);
    if (out?.likely) lines.push(`Outlook tier-1: ${out.likely.forecast || ""}`);
    if (out?.plausible_uncertain) lines.push(`Outlook tier-2: ${out.plausible_uncertain.forecast || ""} (trigger: ${out.plausible_uncertain.escalation_trigger || ""})`);
    for (const j of topJ) {
      lines.push(`Judgment [${j.evidence_maturity||"?"}]: ${j.judgment}`);
      if (j.why_this_matters) lines.push(`  → ${j.why_this_matters}`);
      const sigs = (j.monitoring_signals || []).filter(s => typeof s === "object" && s.signal).slice(0, 2);
      for (const s of sigs) lines.push(`  Watch: ${s.signal} (trigger: ${s.escalation_trigger || ""})`);
    }
  }
  return `${lines.join("\n")}

Generate EXACTLY 5 forward predictions that a CISO should brief their board on for the next 6 months. Each must be specific, named, and mechanistically explained. Distribute across at least 3 of the 4 threat categories.`;
}

async function buildSharpSignalsSlide(categoryAnalyses, allOutlooks, slideNumber, opts = {}) {
  // Fallback: pull monitoring signals deterministically if LLM unavailable
  if (opts.skipLlm) {
    const all = categoryAnalyses.flatMap(ca => {
      ...
      return (ca.judgments || []).filter(j => !j.blocked).flatMap(j =>
        (j.monitoring_signals || []).filter(s => typeof s === "object" && s.signal)
    ...
    const signals = [];
    for (const s of all) {
      const key = `${s.category}|${String(s.signal||"").toLowerCase().slice(0,30)}`;
      if (!seen.has(key)) { seen.add(key); signals.push(s); }
      if (signals.length >= 5) break;
    }
    return {
      type: "early_signals_watchlist", slide_number: slideNumber,
      headline: "6-Month Forward Signals",
      argument: "Specific predictions with named mechanisms and implications",
      bullets: signals.map(s => ({
        text: `${s.category} — ${String(s.signal||"").slice(0,120)}`,
        bullet_type: "signal",
      })),
      speaker_notes: "Run with LLM enabled for sharp, mechanism-level predictions.",
      citations: [], visual_spec: null, visual_suggestion: "none", deterministic: true,
    };
  }

  const sys = buildSignalsSystemPrompt();
  const usr = buildSignalsUserPrompt(categoryAnalyses, allOutlooks);
  try {
    let raw;
    try {
      const { result } = await routedLLM(sys, usr, {
        task: "slide_content", requires_json: true, schema: SIGNALS_SCHEMA,
      });
      raw = typeof result === "string" ? JSON.parse(result) : result;
    } catch {
      const text = await callLLM(sys, usr, { schema: SIGNALS_SCHEMA, json: true });
      raw = typeof text === "string" ? JSON.parse(text) : text;
    }

    const sigs = (raw?.signals || []).slice(0, 5);
    const bullets = sigs.flatMap(s => {
      const out = [];
      // Prediction line (the named claim)
      out.push({ text: `[${s.category}] ${s.prediction}`, bullet_type: "signal" });
      // Mechanism (how it works)
      if (s.mechanism) out.push({ text: `How: ${s.mechanism}`, bullet_type: "mechanism" });
      // Implication
      if (s.implication) out.push({ text: s.implication, bullet_type: "implication" });
      return out;
    });
    ...
    return {
      ...
      headline:      raw?.headline || "6-Month Forward Signals",
      argument:      "Named predictions with mechanisms and implications — each falsifiable",
      bullets:       bullets.slice(0, 15),  // at most 5 predictions × 3 bullets each
      speaker_notes: raw?.speaker_notes || sigs.map(s =>
        `[${s.category}] ${s.prediction} — act when: ${s.escalation_trigger || "first corroborating incident"} (confidence: ${s.confidence})`
      ).join(" | "),
      citations:     [],
      visual_spec:   null,
      visual_suggestion: "none",
    ...
    console.warn(`  [signals] LLM call failed: ${err.message.slice(0,60)} — falling back to deterministic`);
    return buildSharpSignalsSlide(categoryAnalyses, allOutlooks, slideNumber, { ...opts, skipLlm: true });
  }
}
...
// answers "so what?" — what CHANGED and what ASSUMPTION breaks as a result.
...
This slide is your ANALYSIS — not a list of facts. Every insight must answer:
  "What CHANGED and which DEFENDER ASSUMPTION no longer holds?"
An insight that only groups evidence together is NOT an insight — it is aggregation.

THE TEST (apply before writing every sentence):
  Remove the evidence detail. Does the sentence still teach a defender something
  about HOW THE THREAT LANDSCAPE IS SHIFTING? If not — if it is just a summary of
  what happened — rewrite it or drop it.

AGGREGATION (reject these):
  ✗  "Model hubs, vendor libraries, and agent stores are all dangerous."
       → Groups observations. Teaches nothing new.
  ✗  "Prompt injection succeeds at 85%, researchers found."
       → Restates a finding. No "so what."
  ✗  "Agentic AI attack surface has outpaced enterprise defenses."
       → Sounds analytical but asserts an unmeasured comparison.

SYNTHESIS (write these):
  ✓  "AI supply-chain attacks are converging across models, libraries, and registries —
      organizations can no longer govern only software; AI artifacts must be treated as
      executable code."
       → Names the shift, states the broken assumption, gives the implication.
  ✓  "Closed API-only deployment no longer limits jailbreak risk — automated black-box
      attacks now match white-box success rates, removing architecture as a control."
       → Names what changed, names the broken control.

════ EVIDENCE LANGUAGE CALIBRATION ════
Match every insight's language to its evidence maturity — do NOT escalate:
  ...
  observed_exploitation   → "exploited in the wild", "observed in attacks"
  adversary_adoption      → "[named actor] has adopted", "confirmed actor use"
  operational_campaign    → "sustained campaign", "confirmed at scale"
NEVER use "confirmed in the wild", "at scale", "enterprise-wide" unless evidence_maturity
is observed_exploitation or higher. One research paper ≠ "widespread exploitation".

════ CONFIDENCE ════
Assign confidence to each insight (required):
  High   — 2+ strong evidence items from high-trust sources, evidence_maturity ≥ observed_exploitation
  Medium — 1-2 items, high-trust vendor or academic, maturity = disclosed_vulnerability
  Low    — single source, research-only, proof-of-concept only, or maturity = research_demonstration

════ FORMAT ════
A briefing slide is SCANNED in seconds; the presenter SPEAKS the detail. So:
  • insight TEXT is one tight sentence (≤22 words) the audience reads at a glance;
  • confidence label appears alongside each insight on the slide;
  • fuller reasoning and evidence go in speaker_notes (the spoken track).

Each insight TEXT:
  • leads with the strategic claim ("so what"), not the raw observation;
  • names one anchor fact or named entity woven in — no more;
  • states which assumption breaks or what new exposure opens.

BAD:  "Model hubs, vendor libraries and agent stores were found to be dangerous attack surfaces."  (aggregation)
GOOD: "AI artifact distribution — hubs, package indexes, and skill stores — has become a unified supply-chain attack surface with no single control point."  (synthesis, broken assumption)

Rules:
  • 3 to 5 insights (write as many as the evidence justifies — do not pad with weak claims).
  • Each ≤22 words, plain English, DISTINCT (no overlapping claims).
  • Set evidence_id to the single evidence item that best grounds each insight.
  • Set confidence per the maturity rules above.
  • No source names, dates, or [n] markers in insight text — citations added automatically.
  • speaker_notes: for each insight, 1-2 sentences: the evidence detail + the defender implication.
  • headline: ≤10 words, the single most important shift for this category — declarative, not a topic label.

INSIGHT SPECIFICITY BAR — apply before writing every insight:
  • Name a specific attack vector, technique name, tool, actor type, or threshold — not a category.
  • BAD: "AI-enabled social engineering is becoming more common." (names nothing)
  • GOOD: "Multimodal prompt injection via image/audio channels now bypasses text-only defences, leaving API-connected agents exposed." (names mechanism + gap)
  • If you can't make the insight THAT specific from the evidence, lower its confidence rather than diluting it.
...
Write 3 to 5 strategic insights (as many as the evidence justifies — do not pad with
weak claims, do not truncate strong ones) describing WHERE THIS CATEGORY IS HEADING
across the whole corpus timeframe. Each must name a specific technique, actor type,
or threshold — not a generic trend. Plus a 10-word headline.`;
}
...
A slide has a STRUCTURE the reader must grasp in one glance:
  ...
  • BULLETS   = the support, where EACH bullet is exactly ONE of:
      ...
      Mechanism      — HOW the attack works, in 1 plain-English sentence (e.g. "Attacker embeds a hidden instruction in a calendar invite; the AI assistant reads it, silently exfiltrates the thread, and replies as the attacker"). Use this for key development slides — the audience needs to understand the technique, not just that it exists.
...
Do NOT blend these. An Evidence bullet states the fact only; the mechanism goes in a separate Mechanism bullet; the meaning goes in an Implication bullet. This separation is what makes the slide an analysis, not an info dump.

FOR KEY DEVELOPMENT SLIDES (top_happenings): always include at least ONE Mechanism bullet that explains HOW the attack or technique works step-by-step in plain English. An executive who reads this slide should be able to explain the technique to a board member without further research.
...
HEADLINE CALIBRATION — the title must NEVER outrun its evidence:
  If the strongest evidence item is a lab demo → headline uses "demonstrated", not "confirmed" or "deployed"
  If evidence is a single CVE → "a critical vulnerability enables…", not "attackers are exploiting…"
  If evidence is vendor research → "research shows…" or name the vendor: "Researchers found 341 malicious…"
  Reserve "confirmed in the wild", "at scale", "widespread" for evidence_maturity ≥ observed_exploitation.
  A headline that is stronger than its bullets is a misleading headline — downgrade the verb.
...
Match the claim strength to the evidence's own wording. Do NOT escalate:
  ...
  - 15,300 pages containing injections is NOT "15,300 attacks confirmed in the wild".
  - "researchers demonstrated" ≠ "attackers are using" ≠ "deployed at scale".
...
EVIDENCE MATURITY → ALLOWED LANGUAGE:
  research_demonstration  → "researchers demonstrated", "lab-proven", "capability shown"
  disclosed_vulnerability → "a CVE was disclosed", "vulnerability confirmed"
  observed_exploitation   → "exploited in active attacks", "observed in the wild"
  adversary_adoption      → "[actor] has adopted", "used by [named group]"
  operational_campaign    → "sustained campaign", "confirmed at scale across N incidents"

════ KEY FIGURES (stat_cluster slides) ════
Choose figures because they DIRECTLY REINFORCE THE HEADLINE ARGUMENT.
  - A date is almost never a key figure — the scale, actor, or impact is.
  - Pick 2-4 numbers that together make the point the headline claims.
  - BAD set for "AI supply chains are a primary attack surface": Jul 31 / 100+ / 2857 / 341
    → the date adds nothing; 2857 decontextualized is noise.
  - GOOD set: malicious models discovered / packages flagged / organizations affected / hours to exploitation
    → each directly quantifies the claim.
...
// Starred and high-reality evidence surfaces first so the LLM always sees the
// strongest material in the truncated prompt window.
function approvedJudgmentEvidenceIds(ca, limit = 8, evidenceIndex = {}) {
  const approved = (ca.judgments || []).filter(j => !j.blocked);
  const ids = [...new Set(approved.flatMap(j => j.evidence_for || []))];
  // Sort: starred > realized > proven > research — puts best evidence at the top
  const REALITY_RANK = { realized: 3, proven: 2, research: 1 };
  ids.sort((a, b) => {
    const ea = evidenceIndex[a], eb = evidenceIndex[b];
    const sa = ea?.starred ? 1 : 0, sb = eb?.starred ? 1 : 0;
    if (sa !== sb) return sb - sa;
    const ra = REALITY_RANK[ea?.importance_tier] || 0;
    const rb = REALITY_RANK[eb?.importance_tier] || 0;
    return rb - ra;
  });
...
function deterministicCategoryPlan(ca, pack, evidenceIndex = {}) {
  const approved = (ca.judgments || []).filter(j => !j.blocked);
  ...
  const devJudgments = sortedJudgments.slice(0, 3);
  const plan = devJudgments.map((j, i) => ({
    type:         "top_happenings",
    argument:     `Key development — ${label}`,
    // Star-prioritised evidence for each judgment
    evidence_ids: (() => {
  ...
  plan.push({
    ...
    evidence_ids:     approvedJudgmentEvidenceIds(ca, 8, evidenceIndex),
...
export async function buildPresentation(categoryAnalyses, crossCategory, evidenceItemsRaw, opts = {}, externalSlidePlan = null) {
  ...
  const evidenceIdsByCategory = Object.fromEntries(
    categoryAnalyses.map(ca => [ca.category, approvedJudgmentEvidenceIds(ca, 8, evidenceIndex)])
  );
  const crossSlide = buildCrossCategorySlide(crossCategory, 0, evidenceIdsByCategory);
  const crossPlan  = crossSlide ? [crossSlide] : [];

  // ── Step 5: Sharp signals + outlook — ONE LLM call (see buildSharpSignalsSlide)
  // allOutlooks is passed through opts so the pipeline can provide per-category
  // tier-1/tier-2 forecasts as grounding for the prediction LLM call.
  const allOutlooks = opts.allOutlooks || null;
  // Built in Step 7 (after slide plan is assembled) because it needs opts.skipLlm.

  // ── Step 6: Assemble full slide plan ─────────────────────────────────────────
  // Scope & Evidence Snapshot slides removed — audience is analysts/CISOs who do
  // not need corpus housekeeping on their briefing slides.
  const internalSlidePlan = [
    { type: "cover", argument: "AI Cyber Threat Horizon Scan", evidence_ids: [], deterministic: true },
    ...execSummaryPlan,
    ...
    // Placeholder — replaced by the LLM signals slide built below
    { type: "early_signals_watchlist", argument: "6-Month Forward Signals", evidence_ids: [], _placeholder: true },
    ...crossPlan,
  ...
  const signalsSlide = await buildSharpSignalsSlide(categoryAnalyses, allOutlooks, 0, { skipLlm: opts.skipLlm });
  ...
  for (let i = 0; i < internalSlidePlan.length; i += SLIDE_CONCURRENCY) {
    ...
    const generated = await Promise.all(batch.map(async plan => {
      ...
      try {
        ...
        // Confidence label is prepended to the text so it appears on the slide:
        //   "High · Model hubs are now a primary supply-chain attack surface…"
        if (isInsight) {
          const insights = (raw?.insights || []).slice(0, 5).map(it => {
            const conf    = it.confidence ? `${it.confidence} · ` : "";
            const text    = `${conf}${it.insight || it.text || ""}`.trim();
            return normaliseBullet({ text, bullet_type: "claim", evidence_id: it.evidence_id });
          }).filter(b => b.text);
          return {
            ...plan,
