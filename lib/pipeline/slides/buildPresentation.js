...
const SIGNALS_SCHEMA = {
  ...
  properties: {
    ...
    signals: {
      ...
      items: {
        type: "object",
        required: ["prediction", "mechanism", "implication", "evidence_basis", "category"],
        properties: {
  ...
  try {
    ...
    return {
      ...
      speaker_notes: raw?.speaker_notes || sigs.map(s =>
        `[${s.category}] ${s.prediction} — act when: ${s.escalation_trigger || "first corroborating incident"}`
      ).join(" | "),
      citations:     [],
      visual_spec:   null,
      visual_suggestion: "none",
...
function buildCaseStudyUser(plan, evidenceForSlide, judgment) {
  ...
  // If attack stages were pre-extracted from a landscape report, tell the LLM
  // to use them as the attack chain structure rather than inferring it.
  const stagesHint = plan.attack_stages?.length
    ? `\nPRE-EXTRACTED ATTACK CHAIN (from the report — use as the backbone):\n${plan.attack_stages.map((s, i) => `  ${i+1}. ${s}`).join("\n")}\n`
    : "";
...
INSIGHT SPECIFICITY BAR — apply before writing every insight:
  ...
  • If you can't make the insight THAT specific from the evidence, drop it rather than diluting it.
      ...
      try {
        ...
        if (isInsight) {
          const insights = (raw?.insights || []).slice(0, 5).map(it => {
            const text    = `${it.insight || it.text || ""}`.trim();
