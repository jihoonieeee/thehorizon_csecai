...
bullets (3-4) — walk ONE story: what the attacker did → how it worked → how bad it got → what it means:
  ...
  - "implication" = what this means for the business (which protection failed, what's now exposed) — in business terms. The LAST bullet.
...
════ THE 3 BULLETS, in this order ════
  ...
  3. WHY IT MATTERS (bullet_type "implication") — the defense or assumption that no longer works. No evidence_id.
...
function buildBatchSlideUser(plans, evidenceIndex, judgmentIndex) {
  ...
  const lines = [
    ...
    `                     one clear sentence. bullet_type "implication". No evidence_id.`,
    ``,
...
export async function buildPresentation(categoryAnalyses, crossCategory, evidenceItemsRaw, opts = {}, externalSlidePlan = null) {
  ...
  // Cross-category slide removed per design — no cross-category computation.
