...
const CLAIM_FIRST_SYSTEM_PROMPT = `You are rendering slide content for a strategic AI threat horizon scan.

CRITICAL RULE: This slide is anchored to a pre-approved CLAIM. Your only job is to turn that claim and its supporting evidence into concise, accurate slide language.

You MUST NOT:
- Invent new claims
- Introduce facts not present in the provided evidence
- Introduce sources not listed in the provided evidence
- Write trend or growth language unless claim_type = trend_claim
- Write certainty language for outlook slides (must stay conditional)
...
bullets — 3–5 points (max 15 words each) supporting the claim.
  Source from: supporting viewpoints, supporting observations, approved evidence.
  Each bullet must name a specific entity, technique, number, or fact from the evidence.
  ...
  For outlook slides: one bullet = observed basis (what happened), one bullet = trajectory (what may happen).
  For recommendation slides: use action verbs (Deploy, Monitor, Require).

evidence_callouts — 1–3 callouts. EACH must reference an evidence_id from the provided evidence list.
  evidence_id: copy EXACTLY from the supporting_evidence list provided.
  key_fact: a specific fact from that evidence item (a number, name, CVE, or concrete claim).
  Do NOT invent evidence_ids. Do NOT invent key_facts.

citations — one string per source: "Publisher — Title (URL)"
  Only cite sources from the evidence list.
  Do not cite sources not in the evidence list.
...
outlook (6_months):
  ...
  - Observed bullets: "Evidence shows...", "Observed...", "Documented..."
  - Trajectory bullets: "This suggests...", "The trajectory indicates...", "If [condition] holds..."
  - Include confidence level and caveat

recommendation:
  - Lead with action verbs: Deploy, Monitor, Require, Audit, Validate
  - Cite the control gap or risk evidence that motivates each action
  - Do not state urgency without evidence basis

evidence_gap:
  - State explicitly what CANNOT be concluded
 ...
 *   - Supporting viewpoints (analytical interpretation)
 *   - Caveats
 * The LLM renders slide language — it does NOT discover new claims.
 */
function buildClaimFirstPrompt(slidePlan) {
  ...
  const lines = [
    ...
    `CLAIM TEXT (this IS your headline — rephrase for slides but keep the meaning):`,
    `  "${claim_text}"`,
  ];
...
function deterministicClaimFirstContent(slidePlan) {
  const { claim_text, claim_type, supporting_evidence, supporting_viewpoints, caveats } = slidePlan;
  const headline = claim_text?.slice(0, 100) || slidePlan.core_message;

  const evidence_callouts = (supporting_evidence || [])
    .filter((e) => e.evidence_id && (e.fact || e.key_facts?.[0] || e.short_summary))
    .slice(0, 3)
    .map((e) => ({
      title:       clip(e.title || "(source)", 120),
      key_fact:    clip(e.fact || e.key_facts?.[0] || e.short_summary, 150),
  ...
  const candidates = [
    ...(supporting_viewpoints || []).slice(0, 2).map((vp) => clip(vp.viewpoint_text, 90)),
    ...(supporting_evidence  || []).slice(0, 3).map((e) =>
      labeled(e.publisher, e.fact || e.key_facts?.[0] || e.short_summary)
    ),
  ].filter(Boolean);
  ...
  if (!bullets.length) bullets.push("Evidence for this claim is limited this reporting period.");

  // Outlook-specific: add a projection bullet
  if (claim_type === "outlook" && bullets.length < 4) {
    bullets.push("Projected trajectory: based on observed evidence, activity in this area is expected to continue or increase in the next 6 months.");
  }

  const caveatBullet = caveats ? `Caveat: ${caveats}` : null;
  if (caveatBullet && bullets.length < 5) bullets.push(caveatBullet);
  ...
  return {
    ...
    citations: evidence_callouts.map((c) => `${c.publisher} — ${c.title}${c.url ? ` (${c.url})` : ""}`),
...
export async function generateSlideContent(slidePlan, feedSources = [], opts = {}) {
  ...
  for (let i = 0; i < slidePlan.length; i += concurrency) {
    ...
    const batchResults = await Promise.all(batch.map(async (plan) => {
      ...
      try {
        const { result: raw } = await routedLLM(systemPrompt, userPrompt, {
