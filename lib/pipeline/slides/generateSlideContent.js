 ...
 * @param {Function} [opts.llmFn]          - Override LLM function (for testing)
...
export async function generateSlideContent(slidePlan, feedSources = [], opts = {}) {
  ...
  // ── Deterministic info slide types — never need LLM ─────────────────────────
  const DETERMINISTIC_INFO = {
    ...
    scope_and_methodology: deterministicScopeMethodologyContent,
    methodology:           deterministicMethodologyContent,
    taxonomy_framework:    deterministicTaxonomyFrameworkContent,
    taxonomy_reference:    deterministicTaxonomyFrameworkContent,
    source_coverage:       deterministicSourceCoverageContent,
    corpus_analytics:      deterministicCorpusAnalyticsContent,
    landscape:             deterministicCorpusAnalyticsContent,
  };

  // ── Step 1: resolve each slide plan deterministically or flag for batch LLM ──

  /** Build a single slide without any LLM call. */
  function buildDeterministic(plan) {
    ...
    if (plan.slide_type === "category_not_assessed") {
      return assembleSlide(plan, {
        ...
        bullets:  [
          { text: "No validated evidence met the taxonomy threshold for this category this period.", bullet_role: "caveat" },
          { text: "Treat as a coverage gap, not an absence of threat.", bullet_role: "caveat" },
        ],
        evidence_callouts: [], citations: [],
      });
    }
    if (["appendix","appendix_evidence_index","appendix_analytics_tables","appendix_taxonomy"].includes(plan.slide_type)) {
  ...
  // ── Step 2: group slides that need LLM by category ───────────────────────────
  // One batch call per category group rather than one call per slide.
  // Cross-category / structural slides without a category form their own group.

  const resolved = new Map();   // slide_number → assembled slide
  const groups   = new Map();   // groupKey → plan[]

  for (const plan of slidePlan) {
    const det = buildDeterministic(plan);
    if (det) {
      resolved.set(plan.slide_number, det);
    } else if (!hasLlm) {
      resolved.set(plan.slide_number, assembleSlide(plan, deterministicFallback(plan)));
    } else {
      const key = plan.category || "cross_category";
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(plan);
    }
  }

  // ── Step 3: generate each category group as one LLM batch ────────────────────
  ...
  const BATCH_SCHEMA = {
    ...
    properties: {
      slides: {
        ...
        items: {
          type: "object",
          required: ["slide_number", "headline", "bullets", "evidence_callouts", "citations"],
          properties: {
  ...
  /** Build a batch prompt covering all slides in one category group. */
  function buildBatchPrompt(groupSlides) {
    const category = groupSlides[0]?.category;
    const catLabel = CATEGORY_LABELS[category] || category || "cross-category";

    // Pool all evidence across slides — each slide then references by evidence_id
    const evPool = new Map();
    for (const plan of groupSlides) {
      for (const ev of [
        ...(plan.supporting_evidence || []),
        ...(plan.rawfact_evidence   || []),
      ]) {
        if (ev?.evidence_id) evPool.set(ev.evidence_id, ev);
      }
    }
    ...
    const lines = [
      `CATEGORY: ${catLabel}`,
      ``,
      `SHARED EVIDENCE POOL (${evPool.size} item${evPool.size === 1 ? "" : "s"} — use evidence_ids from this list ONLY):`,
      formatSupportingEvidence([...evPool.values()]),
      formatAnalytics(groupSlides.flatMap(p => p.analytics_evidence || []).filter((e, i, a) => a.findIndex(x => x.analytics_id === e.analytics_id) === i).slice(0, 6)),
      ``,
      `Generate content for ALL ${groupSlides.length} slides below. Return strict JSON:`,
      `{ "slides": [ { "slide_number": N, "headline": "...", "bullets": [...], "evidence_callouts": [...], "citations": [...], "visual_plan": {...} }, ... ] }`,
      `Every slide_number in the output MUST match a slide_number listed below.`,
      `evidence_ids in callouts MUST come from the evidence pool above.`,
      ``,
    ];

    for (const plan of groupSlides) {
      const isClaimFirst = CLAIM_FIRST_SLIDE_TYPES.has(plan.slide_type) && !!plan.claim_id;
      const focusInstruction = SLIDE_TYPE_FOCUS[plan.slide_type] || "";
      const evIds = [...new Set([
        ...(plan.supporting_evidence_ids || []),
        ...(plan.supporting_evidence || []).map(e => e.evidence_id),
        ...(plan.rawfact_evidence || []).map(e => e.evidence_id),
      ].filter(Boolean))].slice(0, 5);

      lines.push(`── SLIDE ${plan.slide_number}: ${plan.slide_type}${plan.claim_priority ? ` [${plan.claim_priority.toUpperCase()}]` : ""} ──`);
      lines.push(`Title: ${plan.title}`);
      lines.push(`Core message: ${plan.core_message}`);
      if (focusInstruction) lines.push(focusInstruction);
      if (evIds.length)     lines.push(`Relevant evidence IDs: ${evIds.join(", ")}`);

      if (isClaimFirst) {
        lines.push(`Claim: "${plan.claim_text}"`);
        const rc = plan.reasoning_chain || {};
        if (rc.what_changed)     lines.push(`  What changed: ${rc.what_changed}`);
        if (rc.causal_mechanism) lines.push(`  Why: ${rc.causal_mechanism}`);
        if (rc.why_this_matters) lines.push(`  Implication: ${rc.why_this_matters}`);
        if (rc.uncertainty)      lines.push(`  Caveat: ${rc.uncertainty}`);
        if (plan.slide_type === "evidence_support") {
          lines.push(`  RULE: Do NOT restate the claim. Use only bullet_role=evidence and bullet_role=implication.`);
        }
      } else if (CATEGORY_SLIDE_TYPES.has(plan.slide_type)) {
        const analysis = plan.packet_section || plan.category_analysis;
        if (analysis) lines.push(formatCategoryInsights(analysis).slice(0, 600));
      } else {
        // cross/outlook
        const cc = plan.cross_category_patterns || [];
        const hap = plan.overall_biggest_happenings || [];
        if (cc.length)  lines.push(`Cross-category patterns: ${cc.map(p => p.pattern).join(" | ")}`);
        if (hap.length) lines.push(`Biggest happenings: ${hap.map(h => h.happening).join(" | ")}`);
        if (plan.all_outlook_claims?.length) {
          const c = plan.all_outlook_claims[0];
          lines.push(`Outlook claim: "${c.claim_text}"`);
          const rc = c.reasoning_chain || {};
          if (rc.what_changed)    lines.push(`  Observed: ${rc.what_changed}`);
          if (rc.why_this_matters) lines.push(`  Trajectory: ${rc.why_this_matters}`);
        }
      }
      lines.push("");
    }
  ...
  /** Run one batch LLM call for a group of same-category slides. */
  async function runCategoryBatch(groupSlides) {
    const category = groupSlides[0]?.category || "cross_category";
    const prompt   = buildBatchPrompt(groupSlides);
    try {
      const { result: raw } = await callLlm(
        CLAIM_FIRST_SYSTEM_PROMPT,   // unified system prompt covers both slide types
        prompt,
        {
          task:          "slide_content_batch",
          requires_json: true,
          schema:        BATCH_SCHEMA,
          logLabel:      `L7-batch-${category}`,
        }
      );
      const parsed    = typeof raw === "string" ? JSON.parse(raw) : raw;
      const slideMap  = new Map(
        (parsed?.slides || []).map(s => [s.slide_number, s])
      );
      process.stdout.write(
        `    [L7-batch] ${category}: ${slideMap.size}/${groupSlides.length} slides returned by LLM\n`
      );
      return groupSlides.map(plan => {
        const generated = slideMap.get(plan.slide_number);
        if (!generated) {
          process.stdout.write(`    [L7-batch] slide ${plan.slide_number} missing from batch — using deterministic fallback\n`);
          return assembleSlide(plan, deterministicFallback(plan));
        }
        return assembleSlide(plan, generated);
      });
    } catch (err) {
      process.stdout.write(`    [L7-batch] ${category} batch failed (${err.message}) — falling back per-slide\n`);
      return groupSlides.map(plan => assembleSlide(plan, deterministicFallback(plan)));
    }
  }

  // Run all category batches in parallel
  const groupKeys    = [...groups.keys()];
  process.stdout.write(`  [L7-slide-content] ${groupKeys.length} category batch(es): ${groupKeys.join(", ")}\n`);
  const batchResults = await Promise.all(groupKeys.map(key => runCategoryBatch(groups.get(key))));

  for (const slides of batchResults) {
    for (const slide of slides) resolved.set(slide.slide_number, slide);
  }

  // ── Step 4: reassemble in original plan order ────────────────────────────────
  return slidePlan.map(plan => resolved.get(plan.slide_number) || assembleSlide(plan, deterministicFallback(plan)));
}
