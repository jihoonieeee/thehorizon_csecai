...
function deterministicCategoryContent(slidePlan) {
  ...
  // Biggest happenings → finding role
  for (const h of (analysis?.biggest_happenings || []).filter((x) => x.qa_pass !== false).slice(0, 2)) {
    const t = clip(h.happening, 90);
    const k = normForDedupe(t);
    if (t && k && !seen.has(k)) { seen.add(k); bullets.push({ text: t, bullet_role: "finding" }); }
  }

  // Top insights → implication role
  for (const ins of (analysis?.top_insights || []).filter((x) => x.qa_pass !== false).slice(0, 3)) {
    const t = clip(ins.insight, 90);
    const k = normForDedupe(t);
  ...
  for (const s of (analysis?.early_signals || []).filter((x) => x.qa_pass !== false).slice(0, 1)) {
    const t = labeled("Early signal", s.signal);
    const k = normForDedupe(t);
    if (t && k && !seen.has(k)) { seen.add(k); bullets.push({ text: t, bullet_role: "implication" }); }
  }

  // Still nothing distinct? Use concrete evidence facts
  if (!bullets.length) {
    bullets = evidence_callouts
      .map((c) => labeled(c.publisher, c.key_fact))
      .filter((b) => b && !seen.has(normForDedupe(b)))
      .slice(0, 3)
      .map((t) => ({ text: t, bullet_role: "evidence" }));
  }
  if (!bullets.length) {
...
function deterministicClaimFirstContent(slidePlan) {
  ...
  // Evidence bullets → evidence/finding role
  for (const e of (supporting_evidence || []).slice(0, 3)) {
    const raw = labeled(e.publisher, e.fact || e.key_facts?.[0] || e.short_summary);
    const t   = raw || null;
    const k   = normForDedupe(t);
    if (t && k && !seen.has(k)) {
      seen.add(k);
      ...
      try {
        ...
        if (!validation.valid) {
          ...
          if (should_retry && retry_instructions) {
            ...
            try {
              const { result: retryRaw } = await routedLLM(
                systemPrompt,
                `${retry_instructions}\n\n${userPrompt}`,
                {
