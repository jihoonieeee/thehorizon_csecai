...
const DIAGRAM_SYSTEM_PROMPT = loadPrompt("slides/diagram").system;
...
// Extract CVE IDs from a text string.
const CVE_RE = /CVE-\d{4}-\d{3,}/gi;
function extractCves(text) {
  return [...new Set((String(text || "").match(CVE_RE) || []).map(c => c.toUpperCase()))];
}

function buildRequirement(plan, evForSlide) {
  const facts    = evForSlide.map(ev => (ev.fact || "").trim()).filter(Boolean);

  // ev.entities carries named entities the extraction LLM pulled from the source text
  // (product names, actor names, orgs, CVE IDs). ev.technique_tags carries taxonomy
  // tag IDs which are always empty in practice — using the wrong field was the bug.
  // CVE IDs are also extracted directly from fact text as a belt-and-braces fallback.
  const namedEntities = evForSlide.flatMap(ev => ev.entities || []);
  const cveIds        = evForSlide.flatMap(ev => extractCves(ev.fact));
  const publishers    = evForSlide.map(ev => ev.publisher).filter(Boolean);
  const entities = [...new Set([...namedEntities, ...cveIds])].filter(e => e.length > 1).slice(0, 12);
  ...
  // Prefer the analyst-distilled attack chain (from main_claims) when present —
  // it is a clean, ordered sequence, far better diagram material than raw
  // evidence facts. Fall back to evidence facts only when no chain was supplied.
  const analystStages = (plan.attack_stages || []).map(s => String(s).trim()).filter(Boolean);
  const attackSteps   = analystStages.length >= 2 ? analystStages.slice(0, 6) : facts.slice(0, 6);

  return {
    claim_text:          plan.argument || plan.headline || "",
    key_fact:            analystStages[0] || facts[0] || "",
...
export async function attachSlideDiagrams(slides, evidenceIndex, opts = {}) {
  ...
  const eligible = slides.filter(s =>
    DIAGRAM_ELIGIBLE_TYPES.has(s.type) &&
    !s.diagram_spec &&                       // skip slides already carrying a diagram
    evidenceForSlide(s).length >= 2
  );
