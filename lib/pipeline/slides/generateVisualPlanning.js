...
Your job: decide which slides need a visual and what type, then describe what it should show.
...
- Maximum 4 visuals in the entire deck (assign to the most impactful slides only).
...
function buildPlanningPrompt(slides) {
  ...
  const lines = [
    `DECK HAS ${slides.length} SLIDES. Assign visuals to ≤4 of these ${eligible.length} eligible slides:\n`,
  ];
  for (const s of eligible) {
    lines.push(
      `[Slide ${s.slide_number}] type=${s.slide_type} priority=${s.claim_priority || "medium"} category=${s.category || "N/A"}`,
      `  title: ${s.title || ""}`,
  ...
  lines.push("Assign visual_type and description for each slide. Use visual_type 'none' for slides that don't need a visual.");
  return lines.join("\n");
}
