...
export async function runSlidesLayer(synthesisResult, opts = {}) {
  ...
  const qaFailedSlides = llmQaSlides.filter((s) => s.qa_failed || (s.bullets?.length ?? 0) < 1);
  ...
  const slidesWithNotes = await generateSpeakerNotesForDeck(llmQaSlides, { skipLlm });
  process.stdout.write("    Speaker scripts complete\n");

  // ── Step 3b: Script QA (non-blocking, second-model optional) ────────────
  // Run second-model QA whenever any LLM key is available — independent of skipLlm
  // (which controls content generation, not QA). QA always runs when keys exist.
  const hasLlmForQa = !!(
  ...
  const afterScriptQa = await qaAllScripts(slidesWithNotes, { skipSecondModel: !hasLlmForQa });
  ...
  // ── Step 3d: Visual planning pass (Item 8) ──────────────────────────────────
  ...
  const slidesWithVisuals = await generateVisualPlanning(finalSlides, { skipLlm });
  ...
  // Replace finalSlides for export
  ...
  await writeFile(mdPath,   exportSpeakerScript(exportSlides));
  await writeFile(txtPath,  exportSpeakerScriptTxt(exportSlides));
  await writeFile(docxPath, await exportSpeakerScriptDocx(exportSlides));
