...
export async function runSlidesLayer(synthesisResult, opts = {}) {
  ...
  // If hallucinated stat dropping left a slide with too few bullets, generate
  // safe deterministic content from the slide's evidence_callouts and headline.
  const qaFailedSlides = qaContentSlides.filter((s) => s.qa_failed);
  ...
  if (evidenceRegistry) {
    ...
    for (const slide of qaContentSlides) {
