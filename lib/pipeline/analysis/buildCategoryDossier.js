 ...
 *   1. Evidence strength: strong → usable → context
 *   2. Within same strength: cluster representatives first (is_representative=true)
 *   3. Within same strength+rep: primary/curated trust tier first, then recency
 *
 * Each rawfact evidence item carries:
 *   evidence_id (format: "raw_<source_id>"), title, publisher, published_date,
 *   source_type, key_facts[], numbers_statistics[],
...
function buildRawfactEvidenceItem(source) {
  ...
  return {
    ...
    top_evidence_items:        topItems.map((i) => ({
      ...
      // Preserve full triage_data object so downstream claim chain can read
      // strong_evidence_support, permitted_uses, observed_use, etc. via item.triage_data
      triage_data:        i.triage_data || null,
      ...
      // Provenance from parent source — needed by slides for citation and traceability
