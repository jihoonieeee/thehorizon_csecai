...
function buildRawfactEvidenceItem(source) {
  ...
  const topItems = [...evidenceItems]
    .sort((a, b) => (b.score_data?.evidence_score ?? 0) - (a.score_data?.evidence_score ?? 0))
    .slice(0, 5);
  ...
  return {
    ...
    top_evidence_items:        topItems.map((i) => ({
      ...
      evidence_score:     i.score_data?.evidence_score ?? 0,
      evidence_priority:  i.score_data?.evidence_priority ?? "low",
      entities:           i.entities || [],
      numbers:            i.numbers || [],
 ...
 * @param {object}   [evidencePack] - Optional assembled evidence pack from assembleEvidencePacks().
 * @returns {object} Category dossier with rawfact_evidence, analytics_evidence, and evidence_pack.
 */
export function buildCategoryDossier(category, sources, aggregates, evidencePack = null) {
 ...
 *   When provided, category-relevant items are injected alongside the computed ones.
 * @returns {object[]} Array of category dossiers (only non-empty categories included).
 */
export function buildAllDossiers(sources, aggregates, evidencePacks = [], analyticsEvidence = []) {
  const byCat = {};
  ...
  const catRelevantEvidence = (cat) =>
    analyticsEvidence.filter((e) =>
      !e.category ||
      e.category === "corpus" ||
      (typeof e.insight === "string" && e.insight.toLowerCase().includes(cat.replace(/_/g, " ")))
    ).slice(0, 3);
  ...
  return ANALYSIS_CATEGORIES
    ...
    .map((cat) => {
      ...
      const dossier = buildCategoryDossier(cat, byCat[cat], aggregates, pack);
