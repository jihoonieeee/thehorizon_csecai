 ...
 * 5C external evidence per category) into a uniformly-shaped evidence set that
 * the category-synthesis LLM reasons over — plus an `id_index` the deterministic
 * validator uses to resolve every cited evidence_id.
 *
 * Selection (5A): simple strength + materiality sort, no coverage algorithm.
 * Opus sees the strongest 60 items in the category, ordered by quality.
 * The coverage-aware round-robin was removed: it silently discarded high-quality
 * items in dominant vectors to make room for weak items in under-represented ones.
 * Opus can identify coverage gaps itself (via evidence_gaps[]) — no need to
 * pre-select for diversity at the cost of quality.
 *
 * Deterministic — no LLM. Output:
 * {
 *   category, source_count,
 *   evidence_5A[], evidence_5B[], evidence_5C[],
...
// CAP_5A: the maximum number of 5A items sent to the synthesis LLM per category.
// Raised from 28 → 60 so Opus sees the full picture of the category's evidence,
// not a pre-filtered sample. At ~40 tokens/item this adds ~1k tokens to the prompt —
// well within Opus context and worth it for the analytical depth improvement.
const CAP_5A = 60;
const CAP_5B = 8;   // slightly more analytics context
const CAP_5C = 8;   // slightly more external context
...
function coverageKey(it) {
  // classifyAttackVector removed — kept as a debug label only, not used for selection
  return it.evidence_type || "other";
}
...
function projectItem(it) {
  return {
    ...
    triage_data:       it.triage_data || null,
...
// ── 5A rawfact items — strength-ordered, no coverage algorithm ───────────────
//
// Simple selection: dedupe across all rawfact buckets, sort by strength then
// materiality, cap at CAP_5A. Operational items float to the top because they
// have higher strength scores; no separate guarantee needed.
//
// The previous coverage-aware round-robin was removed because it silently dropped
// high-quality items in dominant vectors to make room for weaker items from
// under-represented vectors. Opus can identify coverage gaps itself and report them
// in evidence_gaps[]. Passing more high-quality evidence is better than passing
// a "balanced" but weaker set.
function compact5A(dossier) {
  ...
  return items
    .sort(compareItems)   // strongest + most material first
    .slice(0, CAP_5A)
    .map(projectItem);
}
