...
// Assertional and mechanism bullets are entailment-checked when they cite a
// specific evidence_id. implication/recommendation/signal/context are analyst
// framing and are not checked. Mechanism bullets without an evidence_id are
// also skipped (the existing `if (!ev) continue` guard handles that case) —
// the check only fires when a mechanism bullet explicitly cites a source,
// ensuring that walkthrough steps attributed to specific evidence are accurate.
const CHECKED_TYPES = new Set(["claim", "data_point", "mechanism"]);
...
//
// key_development slides are included: they anchor on a named event and make
// specific factual claims (actor, CVE, number) that MUST be supported by their
// cited evidence item — the same logic as case_study.
const FACTUAL_SLIDE_TYPES = new Set([
  "top_happenings", "case_study", "executive_summary", "key_development",
]);

const VERIFY_SYSTEM = loadPrompt("analysis/bullet-entailment-qa").system;
