...
function buildEvidenceStrength(dossier, externalEvidence, visualEvidence) {
  const rf = dossier?.rawfact || {};
  const criticalCount = (rf.critical_evidence || []).length;
  const highCount     = (rf.high_evidence     || []).length;
  const totalRawfact  = criticalCount + highCount + (rf.case_studies || []).length +
                        (rf.statistics || []).length;

  const sourceCount   = dossier?.source_count || 0;

  const allItems = [...(rf.critical_evidence || []), ...(rf.high_evidence || [])];
  ...
  if (criticalCount >= 2 && sourceTypeDiversity >= 3 && catExternal.length > 0) {
    confidence_ceiling = "high";
  } else if (criticalCount + highCount >= 3 && sourceTypeDiversity >= 2) {
    confidence_ceiling = "high";
  } else if (criticalCount + highCount >= 2 || catExternal.length > 0) {
