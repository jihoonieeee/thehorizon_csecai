 ...
 * }}
 */
export function classifyJudgmentTier(judgment) {
  ...
  return {
    ...
    approved_for_slides:       meetsMinimum(tier, MINIMUM_FOR.slides),
 ...
 */
export function classifyAllJudgments(judgments) {
  return (judgments || []).map((j) => ({
    ...j,
    ...classifyJudgmentTier(j),
  }));
}
...
/**
 * Rate the quality of a strategic insight.
 * An insight must be pattern-level (not incident-level), have a broken assumption,
 * and have a causal mechanism to be considered analytical quality.
 *
 * @param {object} insight
 * @returns {"analytical"|"descriptive"|"summary_only"|"unsupported"}
 */
export function rateInsightQuality(insight) {
  if (!insight.evidence_ids?.length && !insight.development_ids?.length) {
    return "unsupported";
  }
  if (insight.scope === "incident_level") {
    return "summary_only";  // incident-level insights are blocked from main output
  }
  const hasBrokenAssumption = !isTrivial(insight.broken_assumption) && (insight.broken_assumption || "").length >= 30;
  const hasCausalMechanism  = !isTrivial(insight.causal_mechanism)  && (insight.causal_mechanism  || "").length >= 40;
  const hasDevelopmentLink  = (insight.development_ids || []).length > 0;
  if (!hasDevelopmentLink) return "descriptive";
  if (!hasBrokenAssumption || !hasCausalMechanism) return "descriptive";
  return "analytical";
}
