 ...
 *   likely              — most probable movement, specific and falsifiable, ≤35 words
...
const OUTLOOK_SYSTEM = loadPrompt("analysis/outlook").system;
...
const OUTLOOK_SCHEMA = {
  ...
  properties: {
    likely: {
      type: "object",
      required: ["forecast", "evidence_basis", "timeline_marker"],
      properties: {
        forecast:                 { type: "string" },
        evidence_basis:           { type: "array", items: { type: "string" } },
        timeline_marker:          { type: "string" },
        // Structured forecast fields (spec items 13 + 15)
        named_technique_or_actor: { type: "string" },
    ...
    plausible_uncertain: {
      type: "object",
      required: ["forecast", "evidence_basis", "escalation_trigger"],
      properties: {
        forecast:                 { type: "string" },
        evidence_basis:           { type: "array", items: { type: "string" } },
...
function normaliseOutlookRaw(raw) {
  if (!raw) return raw;
  // Normalise alternate field names the LLM sometimes uses
  const likely = raw.likely || raw.tier_1 || raw.likely_development || {};
  const forecast = likely.forecast || likely.prediction || likely.assessment || likely.statement || "";
  const plaus = raw.plausible_uncertain || raw.plausible || raw.tier_2 || {};
  const watch = raw.watchlist || raw.watch || raw.tier_3 || {};
  return {
    ...raw,
    likely: { ...likely, forecast },
    plausible_uncertain: { ...plaus, forecast: plaus.forecast || plaus.prediction || plaus.scenario || "" },
    watchlist: { ...watch, watch_signals: watch.watch_signals || watch.signals || watch.indicators || [] },
    what_would_invalidate: raw.what_would_invalidate || raw.falsification || raw.invalidation || "",
    category_specific: raw.category_specific ?? true,
  };
}

function validateOutlook(raw, category) {
  raw = normaliseOutlookRaw(raw);
  const issues = [];
...
function buildLegacyFallback(category, legacy) {
  return {
    ...
    watchlist: { forecast: "", watch_signals: [] },
...
export async function generateAllOutlooks(allDevelopments, allInsights, categoryAnalyses, evidenceItems, opts = {}) {
  ...
  // All 4 category outlook calls are independent — run in parallel, then await
  // the overall call which needs the category results as input.
  const categoryEntries = Object.entries(allDevelopments.byCategory || {});
  const categoryResults = await Promise.all(
    categoryEntries.map(([category, developments]) => {
      ...
      return generateOutlook(category, developments, insights, evidenceIndex, {
        ...opts,
  ...
  const results = Object.fromEntries(categoryResults);
  ...
  process.stdout.write(`  [L6.5] ${generated}/${categoryEntries.length} category outlooks generated${overall ? " + overall" : ""}\n`);
