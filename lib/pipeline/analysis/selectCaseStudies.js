...
function buildCandidates(pack, evidenceIndex) {
  // Group evidence by named entity
  const byEntity = {};
  for (const ev of [...(pack.strong || []), ...(pack.usable || [])]) {
  ...
  return Object.entries(byEntity)
    ...
    .map(([entity, items]) => {
      ...
      // Attack stages: rough proxy using distinct trust tiers or multiple items
    ...
    .sort((a, b) => {
      // Rank: outcome > multi-evidence > ai_nexus
      const scoreA = (a.has_outcome ? 3 : 0) + (a.evidence_count >= 2 ? 2 : 0) + (a.ai_nexus ? 1 : 0);
      const scoreB = (b.has_outcome ? 3 : 0) + (b.evidence_count >= 2 ? 2 : 0) + (b.ai_nexus ? 1 : 0);
      return scoreB - scoreA;
    })
    .slice(0, 8);  // top 8 candidates for LLM to choose from
}

// ── LLM selection prompt ──────────────────────────────────────────────────────

const SELECTION_SYSTEM = `You are selecting the BEST case study for a threat category in an executive briefing.
