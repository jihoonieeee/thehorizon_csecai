    ...
    evidence_items: {
      ...
      items: {
        ...
        properties: {
          ...
          time_basis:              { type: "string", enum: ["event_date", "publication_date", "unknown"] },
...
const SYSTEM_PROMPT = loadPrompt("analysis/extract-evidence").system;
...
PUBLICATION_DATE: ${source.date_published || "unknown"}${windowHint}
...
- event_date: the ISO date (YYYY-MM-DD or YYYY-MM) when the incident, experiment, disclosure, or measurement occurred — NOT the source publication date. Use null if the event date is genuinely absent.
- time_basis: "event_date" when you found an event date in the text; "publication_date" when you can only fall back to the source publication date; "unknown" when the date is unclear.
...
export async function extractEvidence(source, opts = {}) {
  ...
  // Short-circuit: use pre-computed report_analysis when present.
  // Avoids an LLM call and produces richer, more structured evidence than
  // generic text extraction would from the same source.
