...
function buildClaimChainView(v, compact) {
  ...
  const add = (text, type, conf, slide, ids, caveat) => {
    if (!text) return;
    const resolvedEv = (ids || []).map((id) => fullById.get(id)).filter(Boolean);
    // 6.7 floor: a claim whose resolved evidence is ALL context / analytics /
    // external (no strong or usable rawfact) cannot be critical or high. Analytics
    // and external items have no evidence_strength here, so they never satisfy it.
    const hasStrongUsable = resolvedEv.some(
      (e) => e.evidence_strength === "strong" || e.evidence_strength === "usable"
    );
    let priority = claimPriority(conf, slide);
    let finalCaveat = caveat || null;
    ...
    claims.push({
      claim_id, claim_type: type, claim_priority: priority,
      claim_text: text, supporting_evidence_ids: ids || [], caveat_if_any: finalCaveat,
    });
    selected_evidence_by_claim.push({ claim_id, selected_evidence: resolvedEv });
  };
...
export async function analyzeCategory(dossier, opts = {}) {
  ...
  try {
    ...
    // Pass the deterministic analytical state (confidence ceiling + pre-structured
    // hypothesis candidates) so the LLM evaluates candidates instead of inventing
    // relationships, and validateCategoryAnalysis can cap confidence to the ceiling.
    if (categoryState) {
      compact.analytical_state = {
        confidence_ceiling: categoryState.evidence_strength?.confidence_ceiling || "low",
        hypothesis_candidates: (categoryState.candidate_judgments || [])
          .slice(0, 6)
          .map((c) => ({
            candidate_claim:        c.candidate_claim,
            confidence_ceiling:     c.confidence_ceiling,
            recommended_output_type:c.recommended_output_type,
            supporting_evidence_ids:(c.supporting_evidence_ids || []).slice(0, 4),
          })),
      };
    }
