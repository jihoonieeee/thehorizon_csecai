...
import { DOMAIN_RULES, buildTaxonomyPromptBlock } from "../understand/taxonomy.js";
...
export function buildScopeBlock(category) {
...
adjacent AI content to fill the section. Produce ONE judgment that states plainly
what in-scope evidence is missing, and list it in evidence_gaps.
...
export function selectDiverseEvidence(items, cap) {
...
export const CHUNK_STRONG  = 60;
export const CHUNK_USABLE  = 25;
export const CHUNK_CONTEXT = 10;

export function buildDossier(category, pack, sources, corpusSummary, itemSubset = null) {
  ...
  // Separate walkthrough items and report insights for dedicated sections.
  const walkthroughItems = allItems.filter(ei => ei.walkthrough_steps?.length > 0);
  const reportInsightItems = allItems.filter(ei => ei._report_insight);
  const regularItems = allItems.filter(ei => !ei.walkthrough_steps?.length && !ei._report_insight);

  // ── Attack walkthroughs section (highest fidelity — direct from reports) ──
  if (walkthroughItems.length > 0) {
    lines.push("ATTACK WALKTHROUGHS (structured — extracted directly from landscape reports):");
    lines.push("");
    for (const ei of walkthroughItems) {
      evidence_index[ei.evidence_id] = {
        fact: ei.fact, source_id: ei.source_id, source_url: ei.source_url,
        trust_tier: ei.trust_tier, specificity: ei.specificity,
      };
      const actor = ei.walkthrough_actor && ei.walkthrough_actor !== "unattributed"
        ? ei.walkthrough_actor : "Unattributed actor";
      lines.push(`[${ei.evidence_id}] WALKTHROUGH — ${actor}`);
      lines.push(`  Technique:  ${ei.walkthrough_technique || ei.fact.slice(0, 100)}`);
      if (ei.walkthrough_mechanism) lines.push(`  Mechanism:  ${ei.walkthrough_mechanism}`);
      if (ei.walkthrough_steps?.length) {
        lines.push(`  Steps:      ${ei.walkthrough_steps.map((s, i) => `${i + 1}. ${s}`).join(" → ")}`);
      }
      if (ei.walkthrough_impact) lines.push(`  Impact:     ${ei.walkthrough_impact}`);
      if (ei.quote && ei.quote_grounded) lines.push(`  Quote:      "${ei.quote}"`);
      lines.push(`  Source:     ${ei.source_title || ei.source_url} [${ei.trust_tier}]`);
      lines.push("");
    }
  }

  // ── Report insights section (analyst findings from deep-extracted reports) ─
  if (reportInsightItems.length > 0) {
    lines.push("REPORT INSIGHTS (key findings from landscape reports — analyst-extracted):");
    lines.push("");
    for (const ei of reportInsightItems) {
      evidence_index[ei.evidence_id] = {
        fact: ei.fact, source_id: ei.source_id, source_url: ei.source_url,
        trust_tier: ei.trust_tier, specificity: ei.specificity,
      };
      lines.push(`[${ei.evidence_id}] INSIGHT — ${ei.insight_finding || ei.fact}`);
      if (ei.insight_significance) lines.push(`  Why it matters: ${ei.insight_significance}`);
      if (ei.insight_taxonomy) lines.push(`  Taxonomy:       ${ei.insight_taxonomy}`);
      if (ei.quote && ei.quote_grounded) lines.push(`  Evidence:       "${ei.quote}"`);
      lines.push(`  Source:         ${ei.source_title || ei.source_url} [${ei.trust_tier}]`);
      lines.push("");
    }
  }
  ...
  if (regularItems.length > 0) {
    if (walkthroughItems.length > 0 || reportInsightItems.length > 0) {
      lines.push("ADDITIONAL EVIDENCE:");
      lines.push("");
    }
    for (const ei of regularItems) {
      evidence_index[ei.evidence_id] = {
        fact: ei.fact, source_id: ei.source_id, source_url: ei.source_url,
        trust_tier: ei.trust_tier, specificity: ei.specificity,
      };
      ...
      if (ei.technique_tags?.length) lines.push(`  TAGS: ${ei.technique_tags.join(", ")}`);
      lines.push("");
    }
  }
...
const SYNTHESIS_SCHEMA = {
  ...
  properties: {
    category_narrative: {
      type: "string",
      description: "One sentence answering: what changed in this threat category this period? Everything else must support this.",
    },
    judgments: {
      type: "array",
      items: {
        ...
        required: [
          ...
          "evidence_maturity", "evidence_for", "caveats", "short_takeaway",
          "confidence",
        ],
      },
    },
    outlook_assessment: {
      ...
      required: ["observed_basis", "likely_next_movement", "escalation_trigger", "what_would_invalidate"],
    },
    coverage_assessment: { type: "string" },
    evidence_gaps:       { type: "array", items: { type: "string" } },
  },
  required: ["category_narrative", "judgments", "coverage_assessment"],
};

// Prompts loaded from lib/prompts/analysis/synthesize-category.md
let _synthPrompts = null;
function getSynthPrompts() {
  if (!_synthPrompts) {
    const raw = loadPrompt("analysis/synthesize-category");
    _synthPrompts = {
      system: interpolate(raw.system, { taxonomy_block: buildTaxonomyPromptBlock() }),
      user:   raw.user,
    };
  }
  return _synthPrompts;
}

function buildSynthesisSystem() {
  return getSynthPrompts().system;
}

function buildSynthesisUser(category, dossier_text) {
  return interpolate(getSynthPrompts().user, {
    category:    category.replace(/_/g, " ").toUpperCase(),
    scope_block: buildScopeBlock(category),
    dossier_text,
  });
}
  ...
  properties: {
    preliminary_findings: {
      ...
      items: {
        ...
        required: ["finding","what_changed","causal_mechanism","why_this_matters","evidence_maturity","evidence_ids"],
      },
    },
  },
...
function buildConsolidationPrompt(category, allPreliminaryFindings, evidenceIndex) {
  ...
  allPreliminaryFindings.forEach((f, i) => {
    ...
    lines.push(`  MATURITY: ${f.evidence_maturity}`);
...
async function callSynthesisMapReduce(category, pack, sources, corpusSummary, opts) {
  ...
  const mapPrompts = loadPrompt("analysis/synthesize-map", "Map Phase");
  const mapSys = mapPrompts.system;
  ...
  const mapResults = await Promise.all(chunks.map(async (chunk, idx) => {
    ...
    const mapUsr = interpolate(mapPrompts.user, { batch_num: idx + 1, total_batches: chunks.length, dossier_text });
    try {
      let raw;
      try {
  ...
  // Cap at 12 findings for consolidation — rank by evidence maturity so
  ...
  const reduceTemplate = loadPrompt("analysis/synthesize-map", "Reduce Phase").user;
  const consolidationUsr = interpolate(reduceTemplate, { consolidation_text: consolidationText });
...
function evaluateJudgments(judgments, evidence_index) {
  return judgments.map(j => {
    ...
    // Soft flag: single-source claim on the weakest evidence tier
    if (j.evidence_maturity === "research_demonstration" && resolvedEvIds.length < 2) {
      issues.push("thin_single_source_claim");
    }

    // Soft flag: no caveats (only expected when evidence is thin)
    if ((j.caveats || []).length === 0 && j.evidence_maturity === "research_demonstration") {
...
export async function synthesizeCategory(category, pack, sources, corpusSummary, opts = {}) {
  ...
  return {
    ...
    category_narrative:  raw.category_narrative || "",
    synthesis_version:   SYNTHESIS_VERSION,
  };
}
...
export async function synthesizeAllCategories(packs, sources, corpusSummary, opts = {}) {
  // Run all categories in parallel — they are fully independent.
  const analyses = await Promise.all(packs.map(async (pack) => {
    ...
    try {
      ...
      const qaLine = qa
        ...
        : "";
      process.stdout.write(`  [L6] ${pack.category}: ${analysis.approved_judgment_count || 0} approved${qaLine}\n`);
      return analysis;
    } catch (err) {
      process.stdout.write(`  [L6] ${pack.category} ERROR: ${err.message}\n`);
      return {
 ...
 */
export async function synthesizeCrossCategory(categoryAnalyses, opts = {}, allDevelopments = null, allInsights = null) {
  ...
  const judgmentBlock = Object.entries(byCategory).map(([cat, judgments]) => {
    ...
    const items = judgments.map((j, i) =>
      `  [${i+1}] ${j.judgment} (${j.evidence_maturity})
  ...
  const crossCatPrompts = loadPrompt("analysis/synthesize-cross-category");
  const sys = crossCatPrompts.system;

  // Enhance prompt with new developments + insights if available
  let devInsightBlock = "";
  if (allDevelopments?.byCategory && allInsights?.byCategory) {
    const devLines = Object.entries(allDevelopments.byCategory).flatMap(([cat, devs]) =>
      devs.filter(d => !d._insight_candidate).slice(0, 2).map(d =>
        `  [${cat}] DEV: ${d.title} (${d.evidence_maturity})`
      )
    ).join("\n");
    const insLines = Object.entries(allInsights.byCategory).flatMap(([cat, ins]) =>
      ins.slice(0, 2).map(i =>
        `  [${cat}] INS: ${i.insight.slice(0, 80)} | assumption: ${i.broken_assumption.slice(0, 60)}`
      )
    ).join("\n");
    devInsightBlock = devLines || insLines
      ? `\n\nTOP DEVELOPMENTS AND INSIGHTS (for deeper cross-category grounding):\nDevelopments:\n${devLines}\nInsights:\n${insLines}`
      : "";
  }

  const usr = interpolate(crossCatPrompts.user, {
    approved_count:  approved.length,
    category_count:  Object.keys(byCategory).length,
    judgment_block:  judgmentBlock,
    dev_insight_block: devInsightBlock,
  });
