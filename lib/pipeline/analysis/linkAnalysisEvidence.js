...
function buildEvidenceIndex(dossiers) {
  ...
  for (const dossier of dossiers) {
    // New fused format: rawfact.critical_evidence, rawfact.high_evidence, etc.
...
function buildCitation(evidenceEntry) {
  ...
  if (evidenceEntry.type === "rawfact") {
    ...
    return {
      ...
      rawfact_score:    item.rawfact_score  || item.score_data?.evidence_score || 0,
      rawfact_priority: item.rawfact_priority || item.score_data?.evidence_priority || "low",
    };
  }
...
function resolveField(fieldItems, index, allCitations, unresolvable) {
  return (fieldItems || []).map((item) => {
    const resolved  = resolveIds(item.supporting_evidence_ids, index, unresolvable);
    ...
    return { ...item, resolved_evidence: resolved.map((e) => e.item), citations };
  });
}
...
function linkAnalysis(analysis, index) {
  ...
  linked.biggest_happenings = resolveField(analysis.biggest_happenings, index, allCitations, unresolvable);
  linked.top_insights       = resolveField(analysis.top_insights,       index, allCitations, unresolvable);
  linked.early_signals      = resolveField(analysis.early_signals,      index, allCitations, unresolvable);
  linked.recommendations    = resolveField(analysis.recommendations,    index, allCitations, unresolvable);
