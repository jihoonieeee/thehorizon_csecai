...
const _judgmentsQaPrompts = loadPrompt("analysis/judgments-qa");
const SECOND_MODEL_SYSTEM = _judgmentsQaPrompts.system;
const SECOND_MODEL_USER   = _judgmentsQaPrompts.user;
...
export async function runSecondModelVerification(judgments, evidence_index, category) {
  ...
  const judgments_block = items.map(({ index, judgment, what_changed, evidence }) =>
    `[J${index}]\nJudgment: "${judgment}"\nWhat changed: "${what_changed}"\nEvidence cited:\n${evidence}`
  ).join("\n\n");
  const userPrompt = interpolate(SECOND_MODEL_USER, {
...
export async function qaJudgments(judgments, evidence_index, category, opts = {}) {
  ...
  // Pass 1: deterministic analytical quality gate
