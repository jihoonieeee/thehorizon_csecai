...
import { rateJudgmentQuality }     from "./analyticalQualityQa.js";
import { checkStatisticalClaims, scrubImpliedQuantitatives } from "./statisticalClaimQa.js";
import { confidenceCeilingForEvidence, strongestReality } from "../scoring/importance.js";
...
function deterministicQa(judgment, evidence_index) {
  ...
  // Evidence-tier confidence ceiling (deterministic version of the Pass-2 rule
  // "'actively exploited' needs incident/threat-intel evidence, not just research").
  // A claim can only be as confident as its STRONGEST evidence is real:
  //   realized → high · proven → medium · research/none → low.
  const citedEvidence = resolved.map(id => evidence_index[id]).filter(Boolean);
  const ceiling = confidenceCeilingForEvidence(citedEvidence);
  if ((CONFIDENCE_RANK[confidence] ?? 1) > (CONFIDENCE_RANK[ceiling] ?? 1)) {
    issues.push(`confidence_capped_to_${ceiling}_by_evidence_reality:${strongestReality(citedEvidence) || "none"}`);
    confidence = ceiling;
  }
...
const SECOND_MODEL_SYSTEM = loadPrompt("analysis/judgments-qa").system;
...
export async function runSecondModelVerification(judgments, evidence_index, category) {
...
export async function qaJudgments(judgments, evidence_index, category, opts = {}) {
  ...
  let result = judgments.map(j => {
    ...
    if ((gated.qa_issues || []).some(x => x.startsWith("confidence_capped_to_") && x.includes("evidence_reality")))
      report.evidence_ceiling_applied++;
    return gated;
  });
