...
export function buildChildSources(parent, items, opts = {}) {
  ...
  return (items || []).map((it, i) => {
    ...
    return {
      ...
      // Use the parent's full text so understandSource has enough context to correctly
      // assess AI materiality and assign taxonomy. The item_summary alone (~200 chars)
      // is too thin — the LLM defaults to rejecting ambiguous or defensively-framed
      // summaries. The supporting_quote is still findable verbatim in the parent text
      // for Layer 5 quote verification. Fall back to summary+quote only when the parent
      // has no full text (e.g. structured API sources like NVD).
      full_text: parent.full_text || parent.clean_text ||
        [it.item_summary, it.supporting_quote ? `"${it.supporting_quote}"` : null]
          .filter(Boolean).join("\n\n") || null,
