...
const DIGEST_TITLE_RE = new RegExp([
  ...
  "m-trends",
  "exploit round-?up",
  // ── Frontier-lab safety reports (OpenAI/Anthropic/DeepMind — many distinct
  //    misuse/jailbreak/eval findings per doc) ───────────────────────────────
  "system card",
  "safety report",
  "responsible scaling",
  "frontier (?:safety|model|risk)",
  "preparedness framework",
  // ── National-agency reports (ACSC/NCSC/CISA/CCCS annual + assessment docs) ──
  "threat assessment",
  "annual (?:cyber|threat)",
].join("|"), "i");

// URL shape often reveals a report even when the title/snippet don't (the same
// feed mixes blogs and reports; the report lives under a telltale path or is a PDF).
// Strong report-only paths (deliberately NOT /research/ or bare .pdf — those would
// over-trigger on vendor blogs and the ~1000 arXiv PDFs, wasting extraction calls).
...
export function detectDigest(source = {}) {
  ...
  if (REPORT_URL_RE.test(String(source.url || ""))) {
    return { is_digest: true, reason: "url_pattern" };          // /reports/, .pdf, /threat-landscape/…
  }
...
// The mechanism-fields block is injected from config at runtime.
export function buildDigestSystemPrompt() {
  return interpolate(loadPrompt("ingest/digest-decompose").system, {
    mechanismBlock: buildMechanismPromptBlock(),
  });
}
