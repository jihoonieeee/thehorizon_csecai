...
export function buildChildSources(parent, items, opts = {}) {
  ...
  return (items || []).map((it, i) => {
    ...
    return {
      ...
      publisher: parent.publisher,
      date_published:  parent.date_published,
      date_confidence: parent.date_confidence || "exact",
      ...
      // Build child-specific full text focused on this finding.
      // Using the parent's full text caused understandSource to classify based on the
      // parent's overall framing (often defensive — "Google disrupting attacks") rather
      // than the specific finding's offensive nature, causing mass mis-rejection.
      // The header gives the LLM enough provenance context; item_summary + quote give
      // enough substance. Falls back to parent text only when item_summary is absent
      // (e.g. structured API sources like NVD that produce very thin items).
      full_text: (() => {
        const parts = [
          `Report: ${parent.title || ""}`,
          `Finding: ${it.item_title || ""}`,
          "",
          it.item_summary || "",
          it.supporting_quote ? `\nKey quote: "${String(it.supporting_quote).slice(0, 400)}"` : "",
        ];
        const childText = parts.join("\n").trim();
        return childText.length >= 80
          ? childText
          : (parent.full_text || parent.clean_text || childText || null);
      })(),
