...
// A date within this many minutes of date_collected is almost certainly the
// scrape timestamp stored as the publish date — "collection-bleed".
const COLLECTION_BLEED_MINUTES = 10;
 ...
 */
/**
 * Post-normalisation date sanity checks. Runs after normalizeSource (which sets
 * the initial date_confidence) and before computeEligibilityFlags. Detects
 * corruption patterns that can only be caught when the source object is fully
 * assembled (e.g. collection-bleed needs date_collected to compare against).
 *
 * Returns a patch { date_published, date_confidence, needs_review, date_integrity_issue }
 * when a problem is found, or null when the date looks clean.
 *
 * Rules (applied in order — first match wins):
 *   1. Future date (> today + 1 day)  → null date, confidence "none"
 *   2. Collection-bleed (exact date within COLLECTION_BLEED_MINUTES of date_collected)
 *      → downgrade to "estimated"
 *
 * Does NOT null dates that were already estimated/low — those are already gated
 * by needs_review=true in computeEligibilityFlags and the downstream filters.
 */
export function validateDateIntegrity(source) {
  const conf        = source.date_confidence;
  const datePub     = source.date_published;
  const dateCollect = source.date_collected || source.collection_metadata?.date_collected;

  if (!datePub) return null;

  const pubMs    = new Date(datePub).getTime();
  const collectMs = dateCollect ? new Date(dateCollect).getTime() : null;
  if (isNaN(pubMs)) return null;

  // Rule 1: future date — null it entirely
  ...
  if (pubMs > tomorrow) {
    return {
      ...
      needs_review:         true,
      date_integrity_issue: "future_date",
    };
  }

  // Rule 2: collection-bleed — exact date suspiciously close to scrape time
  if (conf === "exact" && collectMs != null) {
    const diffMinutes = Math.abs(pubMs - collectMs) / 60000;
    if (diffMinutes <= COLLECTION_BLEED_MINUTES) {
      return {
        date_published:       datePub,
        date_confidence:      "estimated",
        needs_review:         true,
        date_integrity_issue: "collection_bleed",
      };
    }
  }
...
export function computeEligibilityFlags(source, _window = null) {
  ...
  const eligible_for_reference_context = ["primary", "high"].includes(trust);
