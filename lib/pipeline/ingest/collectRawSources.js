...
import { computeEligibilityFlags, validateDateIntegrity } from "./eligibilityFlags.js";
...
function filterByPublishedDateWindow(sources, window) {
  ...
  for (const source of sources) {
    if (source.date_published && isWithinWindow(source.date_published, window)) {
      // Dated, in-window — normal path.
      kept.push(source);
    } else if (!source.date_published) {
      // No date at all: pass through with needs_review=true rather than dropping.
      // upgradeDate() at classify time will attempt text-scan recovery from full_text.
      // If it succeeds, needs_review clears and the source enters normal routing.
      // If it fails, the source stays needs_review=true and is excluded from period
      // reports and the chatbot — but exists for manual review rather than being lost.
      kept.push({ ...source, needs_review: true });
    } else {
      // Dated but outside the reporting window — discard.
      removed.push({
        id: source.id,
        title: source.title,
        url: source.url,
        ...
        reason: "Published date outside reporting window",
      });
    }
  }
...
export async function collectRawSources(customWindow = null, options = {}) {
  ...
  const arxivOnly  = options.arxivOnly ?? false;
  const includeFeeds = arxivOnly ? false : (options.includeFeeds ?? true);
  const requestedConnectors = arxivOnly ? ["arxiv"] : (options.connectors || null);
  const skipArxiv  = options.skipArxiv ?? false;
  ...
  const registryConnectors = SOURCE_REGISTRY
    ...
    .map((source) => ({
      ...
      critical: false,  // RSS feeds: failures are logged as warnings, not degraded
  ...
  const filteredApiConnectors = (requestedConnectors
    ? apiConnectors.filter((connector) => requestedConnectors.includes(connector.key))
    : apiConnectors
  ).filter((connector) => !(skipArxiv && connector.key === "arxiv"));
  ...
  let usableSources = withValidity.filter(
  ...
  // ── L3 pre-filter — skip sources already validated in a prior run ─────────────
  // Connectors re-fetch the last N days on every run, so ~200 sources pass the
  // date window each time but only ~10-70 are genuinely new. The DB upsert uses
  // ignoreDuplicates:true so existing rows are dropped after L3 — wasting ~90% of
  // L3 LLM calls. Query existing IDs up front and skip them entirely.
  let alreadyValidatedCount = 0;
  if (!options.skipLlm && usableSources.length > 0) {
    try {
      const ids = usableSources.map(s => s.id);
      ...
      const newSources = usableSources.filter(s => !existingIds.has(s.id));
      alreadyValidatedCount = usableSources.length - newSources.length;
      if (alreadyValidatedCount > 0) {
        console.log(`  [L3 pre-filter] ${alreadyValidatedCount}/${usableSources.length} already validated — skipping L3`);
      }
      usableSources = newSources;
    } catch (err) {
      console.warn("  [L3 pre-filter] DB lookup failed — running L3 on all sources:", err.message);
    }
  }
  ...
  let dateIntegrityIssues = 0;
  const tagged = taggedRaw.map((source) => {
    const integrityPatch = validateDateIntegrity(source);
    const patched = integrityPatch ? { ...source, ...integrityPatch } : source;
    if (integrityPatch) {
      dateIntegrityIssues++;
      process.stdout.write(`  [date-integrity] ${source.id?.slice(0,8)} ${integrityPatch.date_integrity_issue}: ${source.date_published?.slice(0,10)} → ${integrityPatch.date_published?.slice(0,10) ?? "null"} (was ${source.date_confidence} → ${integrityPatch.date_confidence})\n`);
    }
    return { ...patched, ...computeEligibilityFlags(patched, window) };
  });
  if (dateIntegrityIssues > 0) {
    console.log(`  [date-integrity] ${dateIntegrityIssues} source(s) patched`);
  }
  ...
  // Only CRITICAL connectors (arXiv, CISA KEV, structured API sources) degrade the
  // run when they fail or return empty. RSS registry feeds (critical: false) are
  // supplementary — a dead feed is logged as a warning but does not degrade the run,
  // since feeds are inherently unreliable (Cloudflare blocks, site migrations, 403s).
  const PRIMARY_COVERAGE = new Set(["arXiv"]);
  const degraded_reasons = [];
  const feed_warnings = [];
  for (const run of connectorRuns) {
    if (run.status === "rejected") {
      if (run.critical === false) {
        feed_warnings.push(`${run.name}: ${(run.error || "unknown").slice(0, 80)}`);
      } else {
  ...
  if (feed_warnings.length > 0) {
    console.warn(`  [feed warnings] ${feed_warnings.length} feeds failed (non-critical): ${feed_warnings.join(" | ").slice(0, 300)}`);
  }
  ...
  return {
    ...
    pipeline_counts: {
      ...
      no_date_passed_for_recovery: windowed.filter(s => !s.date_published).length,
      ...
      usable: usableSources.length + alreadyValidatedCount,
      already_validated_skipped: alreadyValidatedCount,
