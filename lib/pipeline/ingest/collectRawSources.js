...
export async function collectRawSources(customWindow = null, options = {}) {
  ...
  const apiConnectors = [
    ...
    },
    {
      ...
      trust_tier: "high",
      retrieval_method: "official_api",
      timeout_ms: 120000,  // 10 queries × ~5s each + rate-limit retries; full-text enrichment runs separately
      ...
      run: (runOpts) => fetchArxivSources({ ...runOpts, ...(options.connectorOptions?.arxiv || {}) }),
    },
