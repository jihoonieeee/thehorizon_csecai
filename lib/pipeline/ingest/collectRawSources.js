...
export async function collectRawSources(customWindow = null, options = {}) {
  ...
  const apiConnectors = [
    ...
    },
    {
      ...
      trust_tier: "high",
      retrieval_method: "official_api",
      // 5 min: covers 10 queries × 5s each + 5s inter-query gap + up to 2 full rate-limit
      // retry cycles (30s + 60s) per query. The old 2-min budget was consumed by a single
      // rate-limited query (30s wait → 60s wait → abort fires mid-wait → 0 sources returned,
      // remaining 9 queries never attempted). ARXIV_INITIAL_DELAY_MS (default 90s) adds a
      // startup pause so our queries arrive after the 22:00 UTC cron-burst window.
      timeout_ms: 300000,
