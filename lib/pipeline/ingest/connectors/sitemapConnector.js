...
// Routes through the task system (date_extraction profile: Gemini Flash, 64 tokens)
// so it respects LLM_ONLY_GEMINI and the provider order rather than using
// whatever provider callLLM picks first.
async function dateFromLLM(title, textHead) {
  ...
  try {
    ...
    const raw = await callLLM(system, user, { task: "date_extraction", logLabel: "date-extract" });
    const text = (typeof raw === "string" ? raw : raw?.date_published || "").trim();
