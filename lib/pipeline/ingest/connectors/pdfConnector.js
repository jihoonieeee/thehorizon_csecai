 ...
 */
async function uploadPdf(pdfBuffer, filename, apiKey) {
...
const EXTRACT_SYSTEM = `You are a precise threat intelligence analyst reading a PDF document.
...
async function extractFromFileId(file_id, apiKey, opts = {}) {
  const { modelId = "claude-haiku-4-5-20251001", timeoutMs = 90000 } = opts;
  const controller = new AbortController();
  const timer      = setTimeout(() => controller.abort(), timeoutMs);
...
export async function extractPdfText(url, opts = {}) {
  ...
  try {
    ...
    file_id = await uploadPdf(pdfBuffer, filename, apiKey);
  } catch (err) {
  ...
  const full_text = await extractFromFileId(file_id, apiKey, { modelId });
