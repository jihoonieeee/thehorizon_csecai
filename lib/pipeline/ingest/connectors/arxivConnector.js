...
export const ARXIV_SHARE_TARGET = 0.20;
...
function buildQueries(window) {
  ...
  return [
    ...
    // ── Traditional AI / adversarial ML (attacks ON ML models) ────────────────
    // The queries above are LLM/agentic/AI-enabled heavy; classic adversarial ML
    // (evasion, poisoning, extraction, inference, robustness) was thin. These
    // widen coverage of the AISec / USENIX / IEEE S&P / CCS / NDSS adversarial-ML
    // literature, which preprints primarily to cs.LG and cs.CV (not just cs.CR).
    //
    // max values are intentionally higher than the LLM/agentic queries because
    // the corpus share throttle (arxivShareScale) reduces all queries proportionally.
    // At 30% arXiv share (scale ≈ 0.67), max:6 → 4 and max:5 → 3, keeping
    // adversarial ML coverage alive. At 40% share (scale floor = 0.2), max:6 → 1
    // which is the same floor hit by max:4, but the higher base ensures better
    // coverage at moderate throttle levels where these queries would otherwise
    // drop to max:1 before the LLM/agentic ones do.
    {
    ...
    // Bridge query: adversarial ML framed in operational/threat-landscape terms.
    // Papers pulled here use vocabulary ("threat", "security risk", "deployed model",
    // "real-world attack") that matches CISO-level chatbot search terms, so they
    // surface in cross-category strategic questions rather than only matching
    // technical academic queries. Covers cs.LG and cs.CV where most AML preprints land.
    {
...
export async function fetchArxivSources(options = {}) {
  ...
  const queries = scale < 1
    ? rawQueries.map(q => ({ ...q, max: Math.max(1, Math.round(q.max * scale)) }))
    : rawQueries;
