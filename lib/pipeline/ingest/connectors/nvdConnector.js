...
export async function fetchNvdSources(options = {}) {
  ...
  return [...byCveId.values()].map((entry) => {
    ...
    // NVD publishes UTC timestamps with NO zone designator (e.g.
    // "2026-04-15T04:17:37.097"). new Date() would parse that in the server's
    // local time, shifting the calendar day by the local offset (e.g. −8h in
    // SGT pushes it to the previous day). Append "Z" so it's always read as UTC.
    const publishedUtc = cve.published && !/[Zz]|[+-]\d{2}:?\d{2}$/.test(cve.published)
      ? `${cve.published}Z`
      : cve.published;
