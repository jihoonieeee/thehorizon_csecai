...
// DELIBERATELY NOT on the platform seam (PLATFORM_AI_API_KEY). This connector
// depends on Gemini's `google_search` grounding tool and reads URLs back out of
// `groundingMetadata.groundingChunks` — a vendor-specific retrieval capability,
// not a chat completion. platformChat() exposes no tool-calling, so routing this
// through it would silently return ungrounded text with no sources. Same
// exception as the Anthropic web_search discovery path and pdfConnector's Files
// API. It needs its own GEMINI_API_KEY and degrades to a no-op without one.
