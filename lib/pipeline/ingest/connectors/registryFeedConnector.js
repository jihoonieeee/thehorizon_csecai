...
export async function fetchRegistryFeedSources(source, options = {}) {
  ...
  // Guard: a feed that has moved/retired often still returns HTTP 200 but serves
  // an HTML landing page, a Cloudflare challenge, or a redirect target instead of
  // RSS/Atom XML. That parses to zero items and the connector vanishes silently
  // (no error, excluded from the degraded-run signal). Detect a non-feed body and
  // fail loudly so it surfaces as a broken connector instead of quietly dropping
  // an entire operational/threat-intel source.
  ...
  if (!hasItems && !isXmlFeed) {
    throw new Error(
      `${source.name} returned a non-feed response (likely HTML; feed URL moved or retired)`
    );
  }
