...
import { foldUrlVariants } from "../../utils/urlCanonical.js";
...
function normalizeUrl(url = "") {
  if (!url) return url;
  // Upgrade HTTP to HTTPS for known-HTTPS domains (arXiv, common sources)
  const HTTPS_DOMAINS = [
    "arxiv.org", "github.com", "nist.gov", "cisa.gov", "openai.com",
    "anthropic.com", "deepmind.google", "microsoft.com", "google.com",
    "owasp.org", "mitre.org", "nvd.nist.gov",
  ];
  if (url.startsWith("http://")) {
    try {
      const host = new URL(url).hostname.toLowerCase();
      if (HTTPS_DOMAINS.some((d) => host === d || host.endsWith(`.${d}`))) {
...
function toCanonicalUrl(url = "") {
  try {
    // Fold known same-item variants (e.g. arXiv abs/pdf/html + version) first so
    // they collapse to one canonical URL — and therefore one source ID.
    const parsed = new URL(foldUrlVariants(url));
    parsed.hash = "";
...
// Detect arXiv URLs so the caller can route them to the arXiv API for an
// exact submission date. The ID alone only encodes year+month (YYMM.NNNNN),
// not the day — so we never infer a date from the URL; we fetch it.
const ARXIV_ID_RE = /arxiv\.org\/(?:abs|html|pdf)\/((?:\d{4}\.\d{4,6}|\d+)(?:v\d+)?)/i;
export function arxivIdFromUrl(url = "") {
  const m = ARXIV_ID_RE.exec(url);
...
export function normalizeSource(item) {
  ...
  const nowDate = now.slice(0, 10);   // YYYY-MM-DD of collection time
  let datePublished = safeDate(item.date_published);

  // Reject dates that are implausibly in the future (>1 day ahead) — likely a
  // timezone or parsing bug in the connector. Null them so eligibilityFlags
  // marks the source needs_review rather than putting it in a future period.
  if (datePublished) {
    const pubDay = datePublished.slice(0, 10);
    const tomorrow = new Date(Date.now() + 86400000).toISOString().slice(0, 10);
    if (pubDay > tomorrow) datePublished = null;
  }

  // Fail-safe default: a connector must EXPLICITLY assert date_confidence:"exact"
  // to have its date trusted for period-report bucketing (see eligibilityFlags —
  // only "exact" is period-eligible). If a connector supplies a date but no
  // confidence, we treat it as "estimated" rather than silently laundering it to
  // "exact". This is what previously let sitemap <lastmod> dates masquerade as
  // authoritative publish dates and leak wrong dates into period reports.
  const dateConfidence = item.date_confidence ??
    (item.collection_metadata?.date_confidence) ??
    (datePublished ? "estimated" : "none");
  ...
  return {
    ...
    // Normalise legacy "curated" tier to "medium" — curated tier has been removed.
    trust_tier: (() => {
      const t = item.trust_tier || item.collection_metadata?.trust_tier || "unknown";
      return t === "curated" ? "medium" : t;
    })(),
    is_curated:       false,
