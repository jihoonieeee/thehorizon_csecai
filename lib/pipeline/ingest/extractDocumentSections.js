...
// Strip banner/hero blocks before section parsing — these appear before the
// first real heading and get incorrectly attached to the h1 section body.
const BANNER_STRIP_PATTERN = /<div[^>]*class="[^"]*(?:article-banner|hero(?:-banner|-section|-image|-area)?|page-hero|post-hero|featured-image|article-header|post-header|entry-header)[^"]*"[^>]*>[\s\S]*?<\/div>/gi;

function extractBody(html) {
  // 1. <article> is the most semantic — use it when present
  const articleMatch = html.match(/<article[^>]*>([\s\S]*?)<\/article>/i);
  if (articleMatch) return articleMatch[1];

  // 2. Specific content-div class patterns (WordPress, custom CMS, etc.)
  //    These are more precise than <main> which often wraps the entire page layout.
  const CONTENT_CLASS_RE = /<div[^>]*class="[^"]*(?:entry-content|post-content|article-content|article-body|post-body|single-content|blog-content|be__contents|wp-block-post-content|td-post-content|post__content|article__body|story-body|article__content|content-body)[^"]*"[^>]*>([\s\S]*?)<\/div>\s*(?:<\/div>|$)/i;
  const contentDivMatch = html.match(CONTENT_CLASS_RE);
  if (contentDivMatch) return contentDivMatch[1];

  // 3. <main> — strip banner/hero noise first before returning
  const mainMatch = html.match(/<main[^>]*>([\s\S]*?)<\/main>/i);
  if (mainMatch) return mainMatch[1].replace(BANNER_STRIP_PATTERN, "");
...
function extractIntelSections(html, maxChars) {
  ...
  while ((match = sectionPattern.exec(cleaned)) !== null) {
    ...
    const isH1 = /^<h1[\s>]/i.test(match[0]);
    // Skip h1 section bodies: the article title h1 is always followed by banner
    // chrome (breadcrumbs, related products, author metadata) before any real
    // content begins. Real content starts at h2.
    sections.push({ heading, body_text, isSignal, len: body_text.length, isH1 });
  }

  // If we found structured sections, prioritize signal ones
  // Exclude h1 section bodies (banner chrome) and short link-list-only sections.
  if (sections.length > 0) {
    const usable      = sections.filter((s) => !s.isH1 && s.len > 50);
    const signalSections = usable.filter((s) => s.isSignal);
    const otherSections  = usable.filter((s) => !s.isSignal);
    ...
    if (text.trim().length > 100) return text.trim().slice(0, maxChars);
  }
