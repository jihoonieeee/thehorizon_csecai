/**
 * Validation 3.3 — Source Data Typing (deterministic fallback)
 *
 * Classifies what kind of source this is.
 * Uses deterministic rules first (sourceTyping.js).
 * Falls back to LLM disambiguation only when rule-based returns "unknown"
 * and the source has enough text to classify.
 *
 * NOTE: In the live validation layer, source typing is normally produced by the
 * 3.2 relevance LLM call (validateAndTypeSource.js). This module remains the
 * deterministic fallback used when the LLM is unavailable or skipped.
 *
 * Output field is source_type_reason (not source_type_method).
 */

import { classifySourceType }     from "./sourceTyping.js";
