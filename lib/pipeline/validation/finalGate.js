...
export function applyFinalGate(validity, relevance, typing, trust, qualityOpts = {}) {
  ...
  if (relevance.relevance_tier === "off_topic") {
    ...
    // primary and curated get review when the LLM found SOME AI signal (focus=passing
    // or adjacent → ai_specificity_score ≥ 20). Without this minimum, every CISA ICS
    // advisory (satellite terminals, industrial PLCs, irrigation controllers) floods
    // unclear_or_adjacent because CISA is primary trust. Keyword scoring may still miss
    // genuine AI advisories written in policy language — the threshold is deliberately
    // low (≥10) to allow "passing" AI mentions through while blocking zero-signal noise.
    // ai_threat_focus="none" → deriveRelevanceFromFocus gives ai_specificity_score=5,
    // so score=5 sources are hard-rejected; score≥10 ("passing", "adjacent", "central") are reviewed.
    if (trust.trust_tier === "primary" || trust.trust_tier === "curated") {
      const hasMinimalAiSignal = (relevance.ai_specificity_score || 0) >= 10 || hasNoveltyPath;
      if (hasMinimalAiSignal) {
        return {
          layer3_status:        "review",
          final_validity_reason: `off_topic_but_${trust.trust_tier}: ai_specificity=${relevance.ai_specificity_score}`,
          downstream_route:     "layer4_with_review",
          route_reason_codes:   [],
        };
      }
      return {
        layer3_status:        "reject",
        final_validity_reason: `off_topic_${trust.trust_tier}_no_ai_signal: ai_specificity=${relevance.ai_specificity_score}`,
        downstream_route:     "discard",
        route_reason_codes:   [],
      };
    }
