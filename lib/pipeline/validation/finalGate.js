...
// Soft flags that, when appearing alone, trigger rejection in the deterministic path.
const SOFT_REJECT_FLAGS = new Set([
  "minimal_text",
  "no_publish_date",
  "stale_publish_date",
]);

// Research source types require higher reading_value to pass the corpus gate.
// Only "essential" and "recommended" research papers enter the corpus — "informative"
// level incremental tests, benchmarks, surveys, and defensive-primary papers are
// rejected here even when L3 passed them. This is the backstop for the research
// gate (researchGate.js) in case the gate's LLM call was unavailable.
const RESEARCH_GATE_TYPES = new Set([
  "research_finding",
  "benchmark_evaluation",
  "capability_demonstration",
]);

// Canonical structured source types where thin content is NORMAL, not a quality failure.
// Only canonical types (from ALL_SOURCE_TYPES) belong here — legacy type names
// (advisory, patch_note, cve, alert) are normalised by layer3Llm before reaching this gate.
 ...
 * @param {string}  [qualityOpts.reading_value]        — LLM reading value (essential/recommended/informative/background)
...
export function applyFinalGate(validity, relevance, typing, trust, qualityOpts = {}) {
  ...
  // ── Hard reject: AI is incidental or merely adjacent ────────────────────
  // Closes contradictions when the LLM returns a permissive verdict but
  // ai_threat_focus signals the source isn't genuinely offensive.
  ...
  // Adjacent sources have no place in an offensive threat intelligence corpus.
  if (ai_threat_focus === "adjacent") {
    return {
      layer3_status:        "reject",
      final_validity_reason: "adjacent_reject: ai_threat_focus=adjacent; governance/context sources are out of scope",
      downstream_route:     "discard",
      route_reason_codes:   reasonCodes("adjacent_reject"),
    };
  }

  // ── Hard reject: research sources below the read-value threshold ─────────
  // Research papers (research_finding, benchmark_evaluation, capability_demonstration)
  // must be "essential" or "recommended" to enter the corpus. "informative"-level papers
  // — routine benchmarks, incremental tests, surveys, defensive-primary work — are
  // rejected. This gate fires regardless of the LLM verdict so it also catches the
  // case where the research gate LLM call was unavailable and L3's lenient judgment
  // passed through unchecked.
  // Whitelist: only confirmed high-value reads enter the corpus. This catches
  // null (LLM failure) and "informative"/"background" — all non-whitelist values reject.
  if (RESEARCH_GATE_TYPES.has(sourceType) &&
      reading_value !== "essential" && reading_value !== "recommended") {
    return {
      layer3_status:        "reject",
      final_validity_reason: `research_low_value: ${sourceType} reading_value=${reading_value}; only essential/recommended research enters the corpus`,
      downstream_route:     "discard",
      route_reason_codes:   reasonCodes("research_low_value_reject", reading_value),
    };
  }
  ...
  if (llm_verdict !== null) {
    ...
    // Guard: if the source has substantial full_text (> 800 chars), don't reject on
    // thin_content alone — the L3 LLM only sees an 8000-char excerpt so it may judge
    // a longer document thin based on its introduction alone.
    const fullTextLen = (source?.full_text?.length || 0);
    if (!isStructured && content_quality === "thin_content" && fullTextLen <= 800) {
      return {
        layer3_status:        "reject",
    ...
    // Note: "informative" passes here for non-research types (incident, threat_intel,
    // operational sources). There is intentionally no floor for operational sources —
    // a low-value incident summary still contributes to corpus analytics and trend data.
    // Research types have a stricter floor enforced above (essential/recommended only).
  ...
  const hasReviewFlag = validity.filter_flags.some((f) => SOFT_REJECT_FLAGS.has(f));
