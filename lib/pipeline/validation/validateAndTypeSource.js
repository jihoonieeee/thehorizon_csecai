...
import { classifyPublisherCanonical } from "./publisherClass.js";
...
export async function validateAndTypeSource(source, opts = {}) {
  ...
  if (validity.hard_fail) {
    ...
    if (l3) {
      ...
      // Hard cap: media publishers can never exceed medium trust, regardless of what
      // the connector or LLM assigned. The LLM inflates trust for media sources when
      // it sees reputable security firms cited in the article content (subject ≠ publisher).
      // classifyPublisherCanonical is used here (not the annotateSourceContext publisher_class)
      // because it applies the full MEDIA_FRAGMENTS list without the trust_tier fallback
      // that can mis-promote unknown publishers to "security_firm".
      const canonicalClass = classifyPublisherCanonical(source);
      if (canonicalClass === "media" && (TRUST_RANK[effective_trust_tier] ?? 0) > (TRUST_RANK["medium"] ?? 2)) {
        effective_trust_tier        = "medium";
        effective_trust_tier_reason = `media_publisher_cap:${source.publisher || source.url}`;
      }
      ...
      // "informative"-level incremental tests, benchmarks, surveys, and defensive-primary
      ...
      if (researchGateApplies) {
        ...
        // finalGate.js will still apply the informative-reading-value hard reject as a backstop.
      }
