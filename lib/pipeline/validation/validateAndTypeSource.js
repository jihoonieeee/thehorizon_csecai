...
export async function validateAndTypeSource(source, opts = {}) {
  ...
  // and confirms reachability. Skipped when skipLlm=true (offline/test mode) or
  // when URL resolution already ran on a previous pass (url_safety_status set).
  ...
  if (source.url && !skipLlm && url_safety_status === null) {
    try {
