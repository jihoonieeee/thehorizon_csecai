...
import { loadPrompt, loadPromptSection, interpolate } from "../../prompts/promptLoader.js";
...
const VALID_READING     = new Set(["essential", "recommended", "informative", "background"]);
...
function buildExcerpt(source) {
  // 8000 chars: enough for the model to assess quality on a full threat-intel
  // report or research paper. 4000 was too short — the model saw only an
  // introduction and called substantive long-form reports "thin_content".
  return (
...
export async function runLayer3Llm(source, priorTrust, opts = {}) {
  ...
  try {
    ...
    // The reading-value rubric is shared with scripts/relabelReadingValues.js so ingest
    // and corpus relabels never drift apart. See lib/prompts/scoring/reading-value.md.
    const filledSystem = interpolate(system, {
    ...
    const { result, llm_metadata } = await llmFn(filledSystem, filledUser, {
    ...
    const content_quality = VALID_QUALITY.has(result.content_quality)
      ...
      : "substantive";   // fail open — finalGate has its own hard text-length check (< 800 chars)

    // Digest children must inherit trust from their parent (set by fan-out).
    // The LLM sees cited institutions in the excerpt and often inflates trust
    // (e.g. child excerpt mentions "AI Now Institute researchers" → LLM assigns high).
    const inheritedTrust = source.parent_source_id && source.trust_tier && source.trust_tier !== "unknown"
      ? source.trust_tier
      : null;
    const trust_tier = inheritedTrust ?? (VALID_TRUST.has(result.trust_tier) ? result.trust_tier : "unknown");

    const verdict = VALID_VERDICT.has(result.verdict)
      ? result.verdict
      : "reject";   // fail safe — missing/invalid verdict treats as reject
    ...
    const trust_tier_reason = inheritedTrust
      ? `inherited_from_parent:${source.parent_source_id?.slice(0, 8)}`
      : (typeof result.trust_tier_reason === "string" ? result.trust_tier_reason.trim().slice(0, 250) : "");
    ...
    const reading_value = VALID_READING.has(result.reading_value)
      ...
      : (focus === "central" ? "informative" : "background");   // safe default
