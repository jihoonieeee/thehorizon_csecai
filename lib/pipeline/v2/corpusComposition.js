...
export const RESEARCH_HARD_CEILING = 0.70;
...
export function buildCorpusComposition(sources = []) {
  ...
  if (researchShare > RESEARCH_HARD_CEILING) {
    warnings.push({
      ...
      message: `Research is ${(researchShare * 100).toFixed(0)}% of the corpus (> ${RESEARCH_HARD_CEILING * 100}% ceiling). The deck will derive operational conclusions from papers — treat agentic/AI-enabled findings with caution and expand vulnerability/incident/threat-intel sources.`,
    });
  }

  // Under-represented operational buckets (the ones we want to grow)
  for (const key of ["vulnerability", "incident", "threat_intelligence", "operational_campaign"]) {
    const d = distribution.find((x) => x.bucket === key);
    if (d.status === "under") {
      warnings.push({
        severity: key === "incident" ? "warning" : "info",
        code: `under_${key}`,
        message: `${d.label} is ${d.pct}% (target ${d.target_pct}). Under-represented operational evidence — conclusions in this area rest on thin real-world grounding.`,
      });
    }
  }
