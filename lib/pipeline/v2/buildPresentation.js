...
function buildEarlySignalsSlide(categoryAnalyses, slideNumber) {
  ...
  return {
    ...
    speaker_notes: `Escalation triggers (the events that move each from watch to action): ${signals.map(s => `${s.category} — ${s.escalation_trigger || "unspecified"}`).join("; ")}.`,
    citations:     [],
    visual_spec:   null,
    visual_suggestion: "none",
    deterministic: true,
...
function buildCrossCategorySlide(crossCategory, slideNumber) {
  ...
  for (const p of patterns) {
    ...
    const mech = p.convergence_mechanism || p.description || "";
    if (mech)                                    bullets.push({ text: `${p.title || p.pattern}${cats ? ` (${cats})` : ""} — ${mech}`, bullet_type: "claim" });
    if (p.compounding_effect)                    bullets.push({ text: p.compounding_effect, bullet_type: "implication" });
    if (p.actionable_recommendation || p.implication) bullets.push({ text: p.actionable_recommendation || p.implication, bullet_type: "recommendation" });
  }
...
function deterministicCategoryPlan(ca) {
  const approved = (ca.judgments || []).filter(j => !j.blocked);
...
export async function buildPresentation(categoryAnalyses, crossCategory, evidenceItems, opts = {}) {
  ...
  for (const ca of categoryAnalyses) {
    if (opts.skipLlm || ca.assessment_status !== "assessed") {
      categorySections[ca.category] = deterministicCategoryPlan(ca);
      continue;
    }
    ...
    try {
      ...
      categorySections[ca.category] = degraded ? deterministicCategoryPlan(ca) : dedupedPlan;
    } catch {
      categorySections[ca.category] = deterministicCategoryPlan(ca);
    }
  }
  ...
  for (const slide of slides) {
    ...
    // Make sure chart/diagram source URLs also land in the reference list.
    for (const eid of (slide.visual_spec?.source_evidence_ids || [])) citeNumFor(eid);
    if (slide.diagram_spec) for (const eid of (slide.diagram_spec.source_evidence_ids || [])) citeNumFor(eid);
  }
