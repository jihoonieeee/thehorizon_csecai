...
export async function buildPresentation(categoryAnalyses, crossCategory, evidenceItems, opts = {}) {
  ...
  if (opts.skipLlm) {
    ...
    slidePlan = [
      { type: "cover",             argument: "AI Threat Intelligence Briefing", evidence_ids: [], audience_signal: "" },
      { type: "executive_summary", argument: "Key findings from this period",   evidence_ids: [], audience_signal: "" },
      ...categoryAnalyses.flatMap(ca => {
        const approved = (ca.judgments || []).filter(j => !j.blocked);
        return [
    ...
    try {
      ...
      slidePlan = [{ type: "executive_summary", argument: `Deck planning failed: ${err.message}`, evidence_ids: [] }];
    }
  }

  // Normalise: LLM returns `type`, schema says `slide_type` — coalesce to `type`
  slidePlan = slidePlan.map((s, i) => ({
    ...s,
  ...
  for (let i = 0; i < slidePlan.length; i += SLIDE_CONCURRENCY) {
    ...
    const generated = await Promise.all(batch.map(async plan => {
      ...
      if (opts.skipLlm || ["cover", "section_intro"].includes(plan.type)) {
        return {
          ...plan,
