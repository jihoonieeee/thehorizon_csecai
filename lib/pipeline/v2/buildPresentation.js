...
export async function buildPresentation(categoryAnalyses, crossCategory, evidenceItems, opts = {}) {
  ...
  const scopeSlide     = buildScopeSlide(corpusSummary, sourcesByCategory);
  ...
  for (const ca of categoryAnalyses) {
    ...
    try {
      ...
      const planned = (raw?.slides || []).map(s => {
        if (s.slide_type === "monitoring_signals" || s.type === "monitoring_signals") {
          return { ...s, type: s.type || s.slide_type, monitoring_signals: monSignals };
        }
        return { ...s, type: s.type || s.slide_type };
      });
      categorySections[ca.category] = planned;
    } catch {
      categorySections[ca.category] = [];
    }
  }
