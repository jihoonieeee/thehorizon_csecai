...
export function buildVisualSpec(evidenceForSlide, suggestion) {
  ...
  const allNums = evidenceForSlide.flatMap(ei =>
    (ei.numbers || [])
      // Only figures whose digits were verified against the source text (extractEvidence
      // sets grounded). Legacy items without the flag (grounded===undefined) are allowed
      // through; a figure explicitly grounded===false is a hallucination and is dropped.
