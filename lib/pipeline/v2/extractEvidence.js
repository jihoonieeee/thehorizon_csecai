...
function verifyQuoteInSource(quote, sourceText) {
  ...
  // Fuzzy: try shorter 40-char window in case of minor whitespace/encoding diff
  const short = quote.slice(0, 40).toLowerCase().replace(/\s+/g, " ").trim();
  return short.length >= 15 && haystack.includes(short);
}
...
export function deduplicateEvidence(items) {
  ...
  return items.map((ei, i) => ({
    ...
    is_cluster_rep: representatives.has(i),
    duplicate_of:  representatives.has(i) ? null : items[representatives.values().next().value]?.evidence_id ?? null,
  }));
}
