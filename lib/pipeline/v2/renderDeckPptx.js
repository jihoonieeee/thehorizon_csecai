...
// Title band height: enough for a 2-line headline at 24pt without overflow.
const TITLE_BLOCK_H = 1.30;
 ...
 *  wrapped 2-line headline (the old horizontal underline struck through line 2).
 *  Returns the Y at which content below the title may begin. */
function addTitle(slide, headline, kicker) {
  ...
  // Vertical accent bar spanning the title block.
  slide.addShape("rect", {
    x: MARGIN, y: TITLE_Y, w: 0.07, h: TITLE_BLOCK_H,
  ...
  slide.addText(clamp(headline || "", 120), {
    x: textX, y: TITLE_Y + (kicker ? 0.24 : 0.02), w: textW, h: TITLE_BLOCK_H - (kicker ? 0.24 : 0.02),
    fontSize: 22, bold: true, color: T.navy, fontFace: T.fontTitle, valign: "top", wrap: true,
  });
  return TITLE_Y + TITLE_BLOCK_H + 0.12;   // content baseline below the title
}
...
function buildContentSlide(pptx, slide, pageNum, total) {
  ...
  // Insights read as 3 numbered strategic statements (larger, more air). Text-only
  // list slides are vertically centred so content isn't clustered at the top with
  // a large empty band beneath.
  const CENTERED = new Set(["scope_methodology", "evidence_snapshot", "category_insights",
                            "early_signals_watchlist", "outlook_structured", "cross_category"]);
  const bulletOpts = slide.type === "category_insights"
    ? { numbered: true, max: 3, fontSize: 18, spaceAfter: 18, valign: "middle" }
    : (CENTERED.has(slide.type) ? { valign: "middle", spaceAfter: 12 } : {});
  addBullets(s, slide.bullets, MARGIN, CONTENT_Y, bw, CONTENT_H, bulletOpts);
...
function buildCaseStudySlide(pptx, slide, pageNum, total) {
  ...
  addTitle(s, slide.headline || slide.argument, [cat, "Case Study"].filter(Boolean).join("  ·  "));
  ...
  // Reserve a fixed bullet strip ABOVE the footer (max 3 bullets) so text never
  // overflows; the attack-chain diagram fills the band above it, full-width.
  ...
  if (slide.diagram_spec) {
    ...
    addBullets(s, bullets, MARGIN, bulletsTop, FULL_W, bulletsH);
  } else {
    // No diagram → bullets get the full content area (and can show all of them).
    addBullets(s, (slide.bullets || []).slice(0, 5), MARGIN, topY, FULL_W, FOOTER_Y - topY - 0.08);
  }
