/**
 * v2 — AI Diagram Generator (Phase 2)
 *
 * Ports the Mermaid attack-flow generator from slides/generateCaseDiagrams.js
 * into the v2 vocabulary. Generates a clean Mermaid diagram for narrative slides
...
// Slide types the LLM may CONSIDER for a diagram (it then decides necessity).
export const DIAGRAM_ELIGIBLE_TYPES = new Set([
  "case_study", "category_insights", "top_happenings", "cross_category",
]);

// ── Mermaid helpers ───────────────────────────────────────────────────────────

function mermaidInkUrl(dsl) {
  // type=png (default endpoint returns JPEG); a large width keeps the embedded
  // image crisp when scaled into the slide panel (default render is too small →
  // blurry); neutral theme + white bg reads cleanly on the light content frame.
  const b64 = Buffer.from(dsl).toString("base64");
  return `https://mermaid.ink/img/${b64}?type=png&theme=neutral&bgColor=FFFFFF&width=2400`;
}
...
function selectDiagramType(req) {
  ...
  if (/layer|tier|component|architect/i.test(hay))                          return "flowchart TD";
  return "flowchart LR";
}
...
- CLEAN LABELS: each node label ≤ 4 words, plain English, no code/variable names.
...
- flowchart LR  — attack chains / exploit sequences (default)
...
export async function generateSlideDiagram(plan, evForSlide, opts = {}) {
  ...
  const render_url = mermaidInkUrl(raw.mermaid_dsl);
