...
async function callSynthesisMapReduce(category, pack, sources, corpusSummary, opts) {
  ...
  if (chunks.length === 1) {
    const { dossier_text } = buildDossier(category, pack, sources, corpusSummary, chunks[0]);
    return callSynthesis(category, dossier_text, opts);
  }
...
Return JSON with "preliminary_findings" array.`;
  ...
  if (allFindings.length === 0) {
    ...
    const { dossier_text } = buildDossier(category, pack, sources, corpusSummary);
    return callSynthesis(category, dossier_text, opts);
  }
...
export async function synthesizeCategory(category, pack, sources, corpusSummary, opts = {}) {
  ...
  let raw;
  try {
    // Use map→reduce when evidence exceeds one chunk; single call otherwise.
    const totalStrong = (pack?.strong?.length || 0);
    const useMapReduce = !opts.skipLlm && totalStrong > CHUNK_STRONG;
    if (useMapReduce) {
      raw = await callSynthesisMapReduce(category, pack, sources, corpusSummary, opts);
    } else {
