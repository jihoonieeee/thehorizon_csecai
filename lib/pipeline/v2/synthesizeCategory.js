...
async function callSynthesis(category, dossier_text, opts) {
  ...
  // Retry once if critical analytical fields are empty — model sometimes returns
  // empty strings to satisfy JSON schema without actually filling the content
  if (!judgmentsHaveCriticalFields(raw)) {
    process.stdout.write(`  [L6] critical fields empty for ${category} — retrying...\n`);
    raw = await attempt();
  }
