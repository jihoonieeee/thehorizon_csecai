...
// Per-chunk evidence caps for the map phase.
// 30 strong + 10 usable + 5 context = 45 items per synthesis call.
// Each chunk fits comfortably in a ~12K token prompt.
const CHUNK_STRONG  = 30;
const CHUNK_USABLE  = 10;
const CHUNK_CONTEXT =  5;
...
function buildConsolidationPrompt(category, allPreliminaryFindings, evidenceIndex) {
  const lines = [
    `CATEGORY: ${category.replace(/_/g, " ").toUpperCase()}`,
    "",
    `You have ${allPreliminaryFindings.length} preliminary findings extracted from evidence sub-batches.`,
    `Your task: consolidate these into 2-4 final strategic judgments, merging overlapping findings,`,
    `resolving contradictions, and surfacing the most important signals.`,
    "",
  ...
  lines.push("EVIDENCE INDEX (for resolving evidence_for IDs):");
  lines.push("");
  for (const [id, ev] of Object.entries(evidenceIndex)) {
    lines.push(`[${id}] ${ev.fact?.slice(0,120) || ""} [${ev.trust_tier}]`);
  }
...
Return JSON with "preliminary_findings" array.`;
  ...
  const allFindings = mapResults.flat();
  process.stdout.write(`  [L6] ${category}: ${allFindings.length} preliminary findings → consolidation call\n`);
  ...
  const consolidationText = buildConsolidationPrompt(category, allFindings, mergedEvidenceIndex);
  const consolidationUsr = `${consolidationText}

Produce 2-4 final strategic judgments. Merge overlapping findings. Resolve contradictions by preferring higher-confidence, higher-maturity findings. Use the exact evidence IDs listed above in evidence_for[].

MANDATORY — every judgment MUST populate all four analytical fields with substantive content (≥1 full sentence each):
  "judgment"          — precise falsifiable claim, not a description
...
Empty strings, placeholder text, or single-word answers for any of these fields will cause the judgment to fail quality checks and be discarded entirely.`;
...
export async function synthesizeCategory(category, pack, sources, corpusSummary, opts = {}) {
  ...
  let raw;
  // evalIndex: the evidence_index to use when evaluating judgments.
  // For map→reduce this is the merged index across all chunks — critical so that
  // evidence IDs from all chunks resolve correctly in evaluateJudgments.
  // For single-call it's the dossier index (already built above).
  let evalIndex = evidence_index;
  try {
    const totalStrong = (pack?.strong?.length || 0);
    const useMapReduce = !opts.skipLlm && totalStrong > CHUNK_STRONG;
    if (useMapReduce) {
      const result = await callSynthesisMapReduce(category, pack, sources, corpusSummary, opts);
      raw       = result.raw;
      evalIndex = result.evidenceIndex;  // use merged index, not single-chunk
    } else {
