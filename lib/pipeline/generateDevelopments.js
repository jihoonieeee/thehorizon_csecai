...
function validateDevelopments(raw, evidenceIndex) {
  const valid = [];
  for (const d of raw) {
    const resolved = (d.evidence_ids || []).filter(id => id in evidenceIndex);
    if (!resolved.length) continue;                           // evidence required
    if ((d.what_changed || "").length < 40) continue;        // substance gate
    if (!EVIDENCE_MATURITY_VALUES.has(d.evidence_maturity)) continue;
    if (!CONFIDENCE_VALUES.has(d.confidence)) continue;

    // Confidence cap: single-source cannot be high
    let confidence = d.confidence;
    if (resolved.length <= 1 && confidence === "high") confidence = "medium";
...
Each development states WHAT CHANGED — a concrete fact, not an interpretation.`;
  ...
  try {
    ...
    let developments = validateDevelopments(raw?.developments || [], evidence_index);
    // Add category tag
