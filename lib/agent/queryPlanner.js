...
function normalizePlan(raw, query, today) {
  ...
  // Deterministic forward-looking override: if the query signals future orientation
  // ("next 90 days", "should defenders watch", etc.) but the LLM planner classified
  // it as "recent" (lookback), correct it to forward_looking so the synthesis prompt
  // receives the right forwardNote and the model doesn't freeze up.
  const FORWARD_SIGNAL_RE = /\bnext\s+\d+\s+(?:day|week|month|year)s?\b|\bprepare for\b|\bgoing forward\b|\bshould (?:defenders?|teams?|analysts?)\s+watch\b|\bwatch (?:for|in|over)\s+(?:the|next)\b/i;
  if (FORWARD_SIGNAL_RE.test(query || "") && temporal.temporal_intent !== "forward_looking") {
    temporal.temporal_intent = "forward_looking";
    temporal.requires_fresh_sources = true;
    if (!temporal.scope_label || /^(?:last|recent)/.test(temporal.scope_label)) {
      temporal.scope_label = "forward-looking (recent context)";
    }
  }

  // For forward-looking queries the evidence base is recent publications, not future
  // events. Correct two common planner mistakes: setting date_from to today (no sources
  // exist in the future) and using event_date (no future events are documented).
  if (temporal.temporal_intent === "forward_looking") {
    temporal.time_field = "publication_date";
    const todayMs = new Date(today).getTime();
    const fromMs  = temporal.date_from ? new Date(temporal.date_from).getTime() : null;
    // If start date is today or in the future, pull it back to 180 days ago.
    if (!fromMs || fromMs >= todayMs) {
      const d = new Date(today);
      d.setDate(d.getDate() - 180);
      temporal.date_from = d.toISOString().slice(0, 10);
    }
  }
...
export async function planQuery(query, opts = {}) {
  ...
  // Compress last 2 prior turns so the planner can resolve follow-up references
  // ("that technique", "in the same period", "tell me more about #2").
  // Framed as optional — the planner must ignore it when the question stands alone.
  //
  // User turns: send the full question (typically <200 chars; the planner needs
  // the full intent, not a truncated fragment).
  //
  // Assistant turns: extract only the "Assessment:" line — the one-sentence signal
  // that captures what was answered. Sending the full 400-word answer floods the
  // planner context with prose and [src-N] markers that mean nothing to it.
  // Falls back to first 200 chars (stripped of citation markers) if no Assessment
  // is found (e.g. general-fallback answers that don't use the Assessment prefix).
  const history = Array.isArray(opts.history) ? opts.history : [];
  function compressTurn(m) {
    const text = String(m.content || "").replace(/\[src-\d+\]/g, "").trim();
    if (m.role === "user") return `Q: ${text}`;
    const assessment = text.match(/Assessment:\s*([^\n]+)/i)?.[1]?.trim();
    return `A: ${assessment || text.slice(0, 200)}`;
  }
  const priorContext = history.length
    ? history.slice(-2).map(compressTurn).join("\n")
    : null;
