...
function normalizePlan(raw, query, today) {
  ...
  // ── Temporal: Haiku is the primary interpreter; normalizeTemporalOutput()
  // applies guardrails (impossible ranges, missing required dates, future starts)
  // and falls back to temporalFallback(query) when validation fails.
  // Accept both new `temporal` key and legacy `timeframe` key for compat.
  ...
  // When the user asks for papers/studies, Haiku translates their academic
  // phrasing into practitioner terminology — which doesn't match arXiv titles.
  // Re-inject the user's own key words alongside Haiku's terms so both
  // vocabularies are searched. Also add "arxiv" and "paper" as anchors.
  let finalTerms = search_terms;
  if (academic_focus) {
    const userTerms = extractSearchTerms(query, 8);
    const academicAnchors = ["arxiv", "paper"];
    finalTerms = [...new Set([...search_terms, ...userTerms, ...academicAnchors])].slice(0, 14);
  }
...
function fallbackPlan(query, today) {
  ...
  return {
    ...
    needs_judgments: /\bmost important|prioriti|top (?:finding|threat|risk)|should (?:i|we) watch|biggest\b|what to (?:watch|monitor)\b/.test(q),
    academic_focus,
