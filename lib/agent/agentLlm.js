...
function buildPlanBlock(plan) {
  ...
  // Expose the planner's synonym expansions so the selector can bridge vocabulary
  // gaps between the user's phrasing and the corpus's actual terminology.
  if (plan.search_terms?.length)  lines.push(`search_terms (semantic equivalents for user's concepts): ${plan.search_terms.slice(0, 12).join(", ")}`);
...
export async function selectSources(query, sources, plan = {}, totalCandidateCount = null, evidenceBySrcId = {}) {
  ...
  // Token budget: larger candidate pool (up to 60 sources) + evidence facts lines
  // increase input; output JSON with selected refs + reasoning needs headroom.
  // Truncated JSON → parse fails → fallback fires (top 8 only), so headroom matters.
  // Raised 1200 → 2000 → 3000 → 4000 → 6000: gemini-2.5-flash generates verbose
  // reasoning before outputting JSON when evidence facts are included in the context
  // (total input ~8000 tokens for 50-source pools with evidence). The model's internal
  // reasoning consumes most of the budget; 6000 output tokens covers it.
  const out = await callHaikuJson({ system, user, maxTokens: 6000 });
  const d = out?.data;
...
/**
 * selectSourcesLenient — fallback selector used when selectSources returns verdict "none"
 * but the retrieval pool is non-empty.
 *
 * Uses a simpler prompt without the SPECIFIC SUBJECT RULE or SCAN-SOURCE RULE: it just
 * finds the most topically related sources in the pool. Verdict is always forced to "thin"
 * by the caller — this function only distinguishes "found something" from "truly off-topic".
 *
 * Receives only the top-N pre-ranked candidates (not the full 50-60 pool) to keep the
 * input small and the cheap-tier model reliable.
 */
export async function selectSourcesLenient(query, sources, plan = {}) {
  const empty = { input_tokens: 0, output_tokens: 0 };
  if (!sources.length) return { selected: [], verdict: "none", coverage: "none", missing: [], usage: empty };

  const system = loadPrompt("agent/selector-lenient").system;

  const srcBlock = sources.map(s => {
    const tagPart  = s.tags?.length ? `\n  tags: ${s.tags.slice(0, 4).join(", ")}` : "";
    return [
      `[${s.ref}] TITLE: ${s.title}`,
      `  ${s.publisher || "?"} | trust: ${s.trust_tier || "?"} | type: ${s.source_type || "?"}`,
      `  pub: ${s.date || "n.d."}` + tagPart,
      `  ${(s.summary || "").slice(0, 300)}`,
    ].filter(Boolean).join("\n");
  }).join("\n\n");

  // For the lenient pass, only surface entity constraints and exclusions.
  // Temporal constraints, time_field, and search_terms are intentionally omitted —
  // they are what caused the strict selector to return none and should not be
  // re-applied here.
  const laxPlanLines = [];
  if (plan.entities?.length)     laxPlanLines.push(`  entities: ${plan.entities.join(", ")}`);
  if (plan.must_exclude?.length) laxPlanLines.push(`  must_exclude: ${plan.must_exclude.join(", ")}`);
  if (plan.category)             laxPlanLines.push(`  category: ${plan.category}`);
  const laxPlanBlock = laxPlanLines.length
    ? `Query constraints:\n${laxPlanLines.join("\n")}`
    : "";
  ...
  const user = [
    ...
    laxPlanBlock,
    ``,
    `Candidate sources (pre-ranked by relevance — work from the top):`,
    ``,
    srcBlock,
  ].filter(Boolean).join("\n");

  const out = await callHaikuJson({ system, user, maxTokens: 2000 });
  const d   = out?.data;

  if (!d || !Array.isArray(d.selected)) {
    return { selected: [], verdict: "none", coverage: "none", missing: [], usage: out?.usage || empty };
  }
  return {
    selected:  d.selected.filter(r => typeof r === "string"),
    verdict:   d.verdict === "none" ? "none" : "thin", // caller forces thin; this just signals found/not-found
    coverage:  "partial",
    missing:   [],
    reasoning: typeof d.reasoning === "string" ? d.reasoning.slice(0, 300) : null,
    usage:     out?.usage || empty,
