 ...
 */
export async function selectSources(query, sources, plan = {}, totalCandidateCount = null) {
  const empty = { input_tokens: 0, output_tokens: 0 };
  ...
  // Token budget raised to 900: plan block + richer source format (lane info,
  // more tags) + coverage/missing JSON fields need the headroom. Truncated JSON
  // → parse fails → fallback fires, so headroom is important.
  const out = await callHaikuJson({ system, user, maxTokens: 900 });
  const d = out?.data;
