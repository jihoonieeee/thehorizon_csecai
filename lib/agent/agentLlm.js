...
/**
 * selectSources — Haiku reads the full candidate pool and picks the most
 * relevant sources for the question. Returns { selected: string[], verdict, usage }.
 * Falls back to returning all sources as selected if the LLM call fails.
 */
export async function selectSources(query, sources) {
  if (!sources.length) return { selected: [], verdict: "none", usage: { input_tokens: 0, output_tokens: 0 } };

  const system = loadPrompt("agent/selector").system;
  const srcBlock = sources.map(s =>
    `[${s.ref}] TITLE: ${s.title}\n  ${s.publisher || "?"} | ${s.date || "n.d."} | trust: ${s.trust_tier || "?"} | type: ${s.source_type || "?"}\n  ${s.summary || ""}`
  ).join("\n\n");

  const user = `Question: ${query}\n\nCandidate sources:\n\n${srcBlock}`;

  const out = await callHaikuJson({ system, user, maxTokens: 400 });
  const d = out?.data;

  if (!d || !Array.isArray(d.selected)) {
    // Fallback: treat all as selected, caller decides verdict.
    return {
