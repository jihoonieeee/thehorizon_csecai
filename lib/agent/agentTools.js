...
export async function retrieveRelevant(plan, { limit } = {}) {
  // exhaustiveness + query_type drive the retrieval pool size:
  //   all_matching   → pull up to 50 so enumeration queries can surface every relevant source
  //   representative → smaller pool; the selector will pick the best handful
  //   trend_analysis → larger pool; trend questions need breadth across a topic over time
  //   best_evidence  → default mid-sized pool (raised from 25 to give selector more choice)
  const baseLimit = plan.exhaustiveness === "all_matching"   ? 50
                  : plan.exhaustiveness === "representative" ? 15
                  : plan.query_type === "trend_analysis"     ? 40
                  : 35;
  const want = Math.min(limit ?? baseLimit, 50);
  ...
  // Rank by: reading_value + maturity + trust tier + keyword title hits +
  // vector similarity bonus. The vector bonus (similarity * 3, capped at ~3)
  // lifts semantically close sources that use different terminology from the
  // query, while keeping keyword breadth (3 pts/hit) as the primary signal.
  ...
  const ranked = allCandidates
    .map(s => {
      ...
      const vecSim  = (vectorSimilarityMap.get(s.id) ?? 0) * 3;
      return { s, score: reading * 3 + maturity * 2 + trust * 2 + hits + vecSim };
    })
...
export async function enrichSourcesWithFullText(sourceRefs) {
  ...
  return sourceRefs.map(s => {
    ...
    return { ...s, summary: text.slice(0, 2000) };
  });
}
