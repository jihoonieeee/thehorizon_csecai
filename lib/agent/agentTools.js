...
async function searchCorpus({ categories, query, tags, date_from, date_to, trust_tiers, limit = 15 }) {
  ...
  let q = supabase
    .from("sources")
    .select("id,title,parent_title,url,publisher,date_published,main_category,trust_tier,tags,summary,short_summary,analyst_brief,primary_tags,source_type,intelligence,reading_value")
  ...
  return {
    ...
    sources: ranked.map((s, i) => ({
      ...
      tags:        s.tags || [],
      summary:     (s.short_summary || s.analyst_brief || s.summary || "").slice(0, 700),
    })),
  };
}
...
function fmtSource(s, i) {
  ...
  const obj = {
    ...
    tags:          s.tags || [],
    summary:       (s.short_summary || (typeof s.analyst_brief === "string" ? s.analyst_brief : "") || s.summary || "").slice(0, 700),
...
export async function retrieveRelevant(plan, { limit } = {}) {
  // exhaustiveness drives the retrieval pool size:
  ...
  //   best_evidence  → default mid-sized pool
  ...
  let q = supabase
    .from("sources")
    .select("id,title,parent_title,url,publisher,date_published,main_category,trust_tier,tags,summary,short_summary,analyst_brief,source_type,intelligence,reading_value")
    .eq("validation_status", "pass")
    .not("needs_review", "is", true)
