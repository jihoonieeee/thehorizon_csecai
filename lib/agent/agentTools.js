...
function relevanceScore(source, terms) {
  ...
  const brief    = (typeof source.analyst_brief === "string" ? source.analyst_brief : "");
...
async function searchCorpus({ categories, query, tags, date_from, date_to, trust_tiers, limit = 15 }) {
  ...
  let q = supabase
    .from("sources")
    .select("id,title,url,publisher,date_published,main_category,trust_tier,tags,summary,analyst_brief,primary_tags,source_type,intelligence")
...
function fmtSource(s, i) {
  ...
  const obj = {
    ...
    trust_tier: s.trust_tier,
    tags:       s.tags || [],
    summary:    (typeof s.analyst_brief === "string" ? s.analyst_brief : s.summary || "").slice(0, 400),
  };
...
export async function retrieveRelevant(plan, { limit = 12 } = {}) {
  const want = Math.min(limit, 30);
  const CANDIDATE_CAP = Math.min(Math.max(want * 6, 80), 220);

  // Search terms = expanded synonyms + named entities; sanitize chars that would
  // break PostgREST's comma-delimited .or() filter.
  const terms = [...new Set([
    ...(plan.search_terms || []),
    ...(plan.entities || []).map(e => String(e).toLowerCase()),
  ])].map(t => String(t).replace(/[,{}()%]/g, " ").trim()).filter(t => t.length >= 3).slice(0, 12);
  const tags = (plan.taxonomy_tags || []).filter(Boolean).slice(0, 6);

  let q = supabase
    .from("sources")
    .select("id,title,url,publisher,date_published,main_category,trust_tier,tags,primary_tags,summary,analyst_brief,source_type,intelligence")
    .eq("validation_status", "pass")
    .order("date_published", { ascending: false })
    .limit(CANDIDATE_CAP);

  // Always search all categories — a hard category filter causes cross-category
  // questions (e.g. "multimodal attacks on LLM guardrails" where the research
  // lives in traditional_ai_threats) to miss relevant sources entirely.
  // The scoring pass below gives a category-match boost when plan.category is set,
  // so on-category sources still rank first when the question is clearly in one category.
  q = q.in("main_category", RETRIEVE_CATS);

  // Recall: any expanded term (title/summary) OR any taxonomy tag (array contains).
  const ors = [];
  for (const w of terms) ors.push(`title.ilike.%${w}%`, `summary.ilike.%${w}%`);
  // `tags` is text[] (holds the coded taxonomy tags); `primary_tags` is jsonb and
  // can't take a .cs.{…} array filter, so tag matching against it happens in the
  // JS scoring pass below, not in the DB query.
  for (const t of tags)  ors.push(`tags.cs.{${t}}`);
  if (ors.length) q = q.or(ors.join(","));

  const tf = plan.temporal || {};
  if (!tf.all_time && tf.date_from) q = q.gte("date_published", tf.date_from);
  if (!tf.all_time && tf.date_to) {
    // Bare YYYY-MM-DD casts to 00:00:00, excluding same-day sources — pin to EOD.
  ...
  const scored = (data || []).map(s => {
    const pTags = Array.isArray(s.primary_tags) ? s.primary_tags : [];
    const sTags = Array.isArray(s.tags) ? s.tags : [];
    const title = String(s.title || "").toLowerCase();
    const body  = `${s.summary || ""} ${typeof s.analyst_brief === "string" ? s.analyst_brief : ""} ${pTags.join(" ")} ${sTags.join(" ")}`.toLowerCase();
    let score = 0, termHits = 0;
    for (const w of terms) {
      const inT = title.includes(w), inB = body.includes(w);
      if (inT) score += 3;
      if (inB) score += 1;
      if (inT || inB) termHits += 1;
    }
    const srcTags = [...pTags, ...sTags];
    let tagHits = 0;
    for (const t of tags) if (srcTags.includes(t)) { score += 4; tagHits += 1; }
    const matched = termHits + tagHits;
    score += matched * 2;
    // Small boost for sources in the planner's primary category — keeps on-category
    // sources ranked first while still allowing cross-category sources to surface.
    if (plan.category && s.main_category === plan.category) score += 1.5;
    score += TRUST_BONUS[s.trust_tier] || 0;
    // When the user explicitly asks for academic papers/studies, boost research
    // source types so arXiv findings outrank vendor reports with the same keywords.
    // The publisher check catches arXiv papers regardless of their assigned source_type.
    if (plan.academic_focus) {
      if (ACADEMIC_SOURCE_TYPES.has(s.source_type)) score += 6;
      if ((s.publisher || "").toLowerCase().includes("arxiv")) score += 4;
    }
    // When recency matters (emerging/latest/current queries), add a small
    // freshness bonus so recently published sources break ties over older ones.
    // Capped at +2 (subordinate to keyword match) — fades to 0 beyond 90 days.
    if (plan.temporal?.requires_fresh_sources && s.date_published) {
      const daysOld = (Date.now() - new Date(s.date_published).getTime()) / 86400000;
      score += Math.max(0, 2 - daysOld / 45);
    }
    score += sourceSignalScore(s) / 100;   // consequence lift, subordinate to match
    return { s, score, matched };
  }).sort((a, b) =>
    b.score - a.score ||
    (b.s.date_published || "").localeCompare(a.s.date_published || ""));

  const relevant_count = scored.filter(x => x.matched > 0).length;
  const verdict = relevant_count === 0 ? "none" : relevant_count < 3 ? "thin" : "good";

  // Surface only sources that actually matched a term/tag (never pad with
  // trust/recency-only rows — those are what made answers cite unrelated sources).
  // Apply a publisher diversity cap (≤2 per publisher) and, when no category
  // filter is set, a category diversity cap (≤4 per category). The category cap
  // prevents corpus skew in a single topic area — e.g. a recent spike in prompt
  // injection coverage — from crowding out all other threat categories in a
  // cross-category CISO-level question.
  const publisherCount = {};
  const PUBLISHER_CAP = 2;
  const categoryCount = {};
  // Only enforce the category cap on cross-category retrieval (plan.category=null).
  // When a question is clearly about one category, let it fill from that category.
  const CATEGORY_CAP = plan.category ? Infinity : 4;
  const surfacedRaw = verdict === "none" ? [] : scored.filter(x => x.matched > 0);
  const surfaced = [];
  for (const item of surfacedRaw) {
    if (surfaced.length >= want) break;
    const pub = (item.s.publisher || "unknown").toLowerCase().trim();
    publisherCount[pub] = (publisherCount[pub] || 0) + 1;
    if (publisherCount[pub] > PUBLISHER_CAP) continue;
    const cat = item.s.main_category || "unknown";
    categoryCount[cat] = (categoryCount[cat] || 0) + 1;
    if (categoryCount[cat] > CATEGORY_CAP) continue;
    surfaced.push(item);
  }
  ...
  return {
    ...
    sources:    surfaced.map((x, i) => fmtSource(x.s, i)),
  };
}
