...
async function getEvidence({ evidence_ids, categories, query, tags, grounded_only, limit = 25 }) {
  ...
  if (Array.isArray(evidence_ids) && evidence_ids.length) {
    ...
    if (query) {
      // Word-level matching, not whole-phrase: an exact-substring ilike on a
      // multi-word query (e.g. "prompt injection bypass") almost never appears
      // verbatim and returns nothing. Match any significant word across fact+quote
      // (recall-first; the category filter keeps it scoped).
      const STOP = new Set(["the","and","for","are","with","that","this","from","into","what","how","why","does","is","of","to","a","in","on","an"]);
      const words = query.toLowerCase().match(/[a-z0-9]{3,}/g)?.filter(w => !STOP.has(w)).slice(0, 5) || [];
      const terms = words.length ? words : [query];
      const ors = [];
      for (const w of terms) {
        const safe = w.replace(/[%_\\]/g, "\\$&");
        ors.push(`fact.ilike.%${safe}%`, `quote.ilike.%${safe}%`);
      }
      q = q.or(ors.join(","));
    }
    if (Array.isArray(tags) && tags.length) {
