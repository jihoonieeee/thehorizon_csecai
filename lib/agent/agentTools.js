...
export async function retrieveRelevant(plan, { limit = 12 } = {}) {
  ...
  // Apply a publisher diversity cap: at most 2 sources from any single publisher
  // to prevent one high-signal broad report from crowding out all other sources.
...
async function getEvidence({ evidence_ids, categories, query, tags, grounded_only, limit = 25 }) {
  ...
  const diverseItems = applyDiversityCap(data || []).slice(0, Math.min(limit, 40));
