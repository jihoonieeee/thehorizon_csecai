...
// Importance tier rank — higher is more useful to surface to the selector.
const IMPORTANCE_RANK = { realized: 4, proven: 3, research: 2, reference: 1, noise: 0 };
const TRUST_RANK      = { primary: 3, curated: 3, high: 2, medium: 1, low: 0, unknown: 0 };
...
export async function retrieveRelevant(plan, { limit = 25 } = {}) {
  ...
  let q = supabase
    .from("sources")
    .select("id,title,url,publisher,date_published,main_category,trust_tier,tags,summary,analyst_brief,source_type,intelligence")
    .eq("validation_status", "pass")
    .not("needs_review", "is", true)
  ...
  // Rank by: importance tier + trust tier + keyword hits in title.
  // This surfaces landmark/confirmed sources above noise-tier recent articles
  // without a complex multi-factor scoring function.
  ...
  const ranked = allCandidates
    .map(s => {
      const imp   = IMPORTANCE_RANK[s.intelligence?.importance?.tier] ?? 0;
      const trust = TRUST_RANK[s.trust_tier] ?? 0;
      const hits  = title_tokens_match(s.title);
      return { s, score: imp * 3 + trust * 2 + hits };
    })
