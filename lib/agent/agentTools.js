...
const READING_RANK = { essential: 4, recommended: 3, informative: 2, analyst: 2, background: 1 };
const MATURITY_RANK = { operational: 4, observed: 3, validated: 2, research: 1 };
...
export async function retrieveRelevant(plan, { limit } = {}) {
  ...
  const ranked = allCandidates
    .map(s => {
      const rv      = s.reading_value ?? s.intelligence?.reading_value ?? "informative";  // raised from "background"
