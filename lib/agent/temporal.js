...
export function resolveExplicitAbsolutePeriod(query, today) {
  ...
  // Guard: compound range queries ("between X and Y", "from X to/through Y") span
  // multiple periods. Firing on the first month/quarter name found would silently
  // truncate the window (e.g. "between 1 January and 31 March" → January only).
  // Return null so Haiku or temporalFallback can parse the full range.
  if (/\bbetween\b/i.test(q) || /\bfrom\b.{1,120}\b(?:to|through|until)\b/i.test(q)) {
    return null;
  }
...
export function normalizeTemporalOutput(raw, today, query) {
  ...
  // Carry time_field from the model output onto the authoritative override so
  // incident queries ("incidents in July") retain event_date even when the date
  // range is determined deterministically.
  if (explicit) {
    explicit.time_field = VALID_TIME_FIELDS.has(raw?.time_field) ? raw.time_field : "either";
    return explicit;
  }
  ...
  // Prefer scope_label (brief human-readable window) over reasoning_summary (verbose
  // explanation). The planner prompt directs the model to put the brief label in
  // scope_label; reasoning_summary is kept for backward compat with older outputs.
  const label =
    (typeof raw.scope_label === "string" && raw.scope_label.trim() && raw.scope_label !== "all available data")
      ? raw.scope_label.trim()
      : (typeof raw.reasoning_summary === "string" && raw.reasoning_summary.trim())
        ? raw.reasoning_summary.trim()
        : null;

  const time_field = VALID_TIME_FIELDS.has(raw.time_field) ? raw.time_field : "either";
  ...
  switch (intent) {
    case "none":
      return {
        ...
        temporal_intent: "none", requires_fresh_sources: false, forecast_horizon: null, time_field,
      };
    ...
    case "forward_looking": {
      ...
      return {
        ...
        all_time: false,
        temporal_intent: "forward_looking", requires_fresh_sources: true, forecast_horizon, time_field,
      };
    }
    ...
    case "recent":
      ...
      return {
        ...
        all_time: false,
        temporal_intent: intent, requires_fresh_sources, forecast_horizon, time_field,
      };
...
export function temporalFallback(query, today) {
  ...
  if (/\bnext\s+\d+\s+(?:day|month|year|week)s?\b|\bprepare for\b|\bupcoming\b|\boutlook\b|\bhorizon\b|\bgoing forward\b|\bshould (?:defenders?|teams?|orgs?|organizations?)\s+watch\b|\bwatch (?:for|in|over)\b/.test(q)) {
    const hm = q.match(/next\s+(\d+\s+(?:day|month|year)s?)/);
    return {
  ...
  // ── "last week" / "last month" (bare, no number) ─────────────────────────
  if (/\b(?:past|last)\s+week\b/.test(q) && !/\b(?:past|last)\s+(?:\d+|\w+)\s+weeks?\b/.test(q)) {
    return { date_from: offsetDays(todayStr, -7), date_to: null,
             scope_label: "last week", all_time: false,
             temporal_intent: "recent", requires_fresh_sources: true, forecast_horizon: null };
  }
  if (/\b(?:past|last)\s+month\b/.test(q) && !/\b(?:past|last)\s+(?:\d+|\w+)\s+months?\b/.test(q)) {
    return { date_from: offsetDays(todayStr, -30), date_to: null,
             scope_label: "last month", all_time: false,
             temporal_intent: "recent", requires_fresh_sources: true, forecast_horizon: null };
  }
  if (/\b(?:past|last)\s+year\b/.test(q) && !/\b(?:past|last)\s+(?:\d+|\w+)\s+years?\b/.test(q)) {
    return { date_from: offsetDays(todayStr, -365), date_to: null,
             scope_label: "last year", all_time: false,
             temporal_intent: "historical", requires_fresh_sources: false, forecast_horizon: null };
  }

  // ── "past / last N units" (numeric or written-out word numbers) ───────────
  ...
  const NUM_PAT = `(\\d+|${Object.keys(WORD_NUMS).join("|")})`;
  const rel = q.match(new RegExp(`\\b(?:past|last)\\s+${NUM_PAT}\\s+(day|week|month|year)s?\\b`)) ||
              q.match(new RegExp(`\\bin (?:the )?(?:past|last)\\s+${NUM_PAT}\\s+(day|week|month|year)s?\\b`)) ||
              q.match(/\b(?:past|last)\s+(couple(?:\s+of)?)\s+(week|day|month)s?\b/);
  if (rel) {
    const raw = rel[1], unit = rel[2];
    const n = raw === "couple" || raw === "couple of" ? 2 : (WORD_NUMS[raw] ?? parseInt(raw, 10));
  ...
  // ── "between Q3 2025 and Q1 2026" / "from Q3 2025 to/through Q1 2026" ─────
  const betweenQQ = q.match(/\b(?:between\s+|from\s+)q([1-4])\s+(\d{4})\s+(?:and|to|through|until)\s+q([1-4])\s+(\d{4})\b/);
  if (betweenQQ) {
    const qi1 = parseInt(betweenQQ[1], 10) - 1, yr1 = betweenQQ[2];
    const qi2 = parseInt(betweenQQ[3], 10) - 1, yr2 = betweenQQ[4];
    const sm  = [1, 4, 7, 10][qi1];
    const em  = [3, 6, 9, 12][qi2];
    const ed  = [31, 30, 30, 31][qi2];
    const from = `${yr1}-${String(sm).padStart(2,"0")}-01`;
    const to   = `${yr2}-${String(em).padStart(2,"0")}-${String(ed).padStart(2,"0")}`;
    return { date_from: from, date_to: to,
             scope_label: `Q${betweenQQ[1]} ${yr1} to Q${betweenQQ[3]} ${yr2}`, all_time: false,
             temporal_intent: "bounded_period", requires_fresh_sources: false, forecast_horizon: null };
  }
