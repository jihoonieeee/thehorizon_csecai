    ...
    const sourcesHtml = catSources.map((s, srcIdx) => {
      ...
      return `
        <div style="padding:24px 0;${isLast ? "" : "border-bottom:1px solid #f3f4f6;"}">
          ...
          <a href="${url}" target="_blank" rel="noopener noreferrer" style="font-size:13px;color:#2563eb;text-decoration:none;font-weight:500;">Read article &rarr;</a>
        </div>`;
    }).join("");
...
<head>
  ...
  <title>AI Horizon &mdash; ${esc(period.label)}</title>
</head>
<body style="margin:0;padding:0;background:#ffffff;-webkit-font-smoothing:antialiased;">
  <div style="max-width:560px;margin:0 auto;padding:52px 24px 64px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI','Inter',Helvetica,Arial,sans-serif;color:#111827;">
    ...
    <div style="margin-bottom:40px;">
      ...
      <div style="font-size:26px;font-weight:700;color:#111827;letter-spacing:-0.02em;line-height:1.2;margin-bottom:8px;">AI Horizon</div>
    ...
    <div style="font-size:12px;color:#9ca3af;text-align:center;letter-spacing:0.02em;">AI Horizon &middot; ${esc(today)}</div>
