...
export async function loadCandidates(supabase, dateFrom, dateTo) {
  ...
  // Filter to sources with usable content, then sort by rough signal so the
  // CANDIDATE_LIMIT cap is quality-biased rather than arbitrary.
  return rows
...
function buildSelectionUser(candidates, period) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const summary = (s.short_summary || s.analyst_brief || intel.source_summary || "").trim().slice(0, 400);
