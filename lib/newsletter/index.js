...
const SRC_SELECT = "id,title,url,publisher,date_published,date_published_actual,date_confidence,main_category,trust_tier,source_type,tags,short_summary,analyst_brief,intelligence";

function summaryText(s) {
  return (s.short_summary || s.analyst_brief || s.intelligence?.source_summary || "").trim();
}

export async function loadReadingList(supabase, dateFrom, dateTo, limit = 10, opts = {}) {
  const { dedup = null, log = () => {} } = opts;
  const { data, error } = await supabase
    .from("sources")
    .select(SRC_SELECT)
    .eq("validation_status", "pass")
    .not("needs_review", "is", true)   // exclude only genuinely-flagged (true), keep NULL/false
    .in("trust_tier", ["primary", "high", "curated"])
    // Only trust an authoritative publish date for a date-windowed reading list.
    // Connectors set date_confidence="exact" ONLY when the date comes from the
    // source itself (arXiv/NVD/GHSA APIs, publisher metadata). Discovery and
    // sitemap sources that couldn't find a real date are stamped with the run
    // time and marked "low"/"estimated" — including them made undated URLs
    // discovered *today* look "published this week". Per normalizeSource, only
    // "exact" is period-eligible; the newsletter now enforces that.
  ...
  // Date guard: only keep sources whose publish DATE falls inside the window and
  // whose date is authoritative. The query already bounds this, but we re-check
  // here so the reading list can never carry a source dated outside — or with a
  // non-authoritative date for — the period it claims to cover.
  const inWindow = (s) => {
    if (s.date_confidence && s.date_confidence !== "exact") return false;
  ...
  // Dedup QA: collapse duplicate coverage of the same event BEFORE the category
  // round-robin, so the surviving canonical source still competes for its slot
  // and category balance is preserved. Only the realistic selection zone (top of
  // the ranked pool) is examined — a tail source is unlikely to be picked. Fails
  // open: if dedup errors it returns an empty set and nothing is dropped.
  if (dedup && ranked.length > 1) {
    const head = ranked.slice(0, Math.max(limit * 2, 16));
    const removeIds = await dedup(head, log);
    if (removeIds?.size) ranked = ranked.filter(s => !removeIds.has(s.id));
  }
...
// ── Dedup QA (Haiku) ───────────────────────────────────────────────────────────
// Collapse sources that are duplicate coverage of the SAME underlying event
// (e.g. two write-ups of the same OpenClaw ClawHub disclosure) down to one
// canonical entry. Fails OPEN: any error returns an empty remove-set so a QA
// hiccup can never drop a legitimate source.

let DEDUP_SYSTEM = null;  // loaded lazily on first call
...
/**
 * Return a Set of source ids to REMOVE as duplicate event coverage.
 * @param {Array} sources  ranked candidate pool (before the final cap)
 * @param {Function} log
 */
export async function dedupReadingList(sources, log = () => {}) {
  const remove = new Set();
  if (!Array.isArray(sources) || sources.length < 2) return remove;
  if (!process.env.ANTHROPIC_API_KEY) return remove;   // fail open — no key, no dedup
  try {
    ...
    const json = parseJson(raw);
    // Only honour removals that (a) name a real id in the pool and (b) have a
    // surviving canonical "keep" also in the pool — never drop the last member.
