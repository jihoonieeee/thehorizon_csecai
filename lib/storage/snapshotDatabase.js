...
export async function saveSnapshotToDatabase(snapshot, opts = {}) {
  ...
  const sourceRows = snapshot.sources.map((source) => ({
    ...
    // ── Layer 3 validation outputs (000_schema.sql section 3 columns) ────────────────────
    ...
    // Writing both so downstream queries on either column work correctly.
    ...
    // Content-quality gate (3.3) + deterministic pre-gate signal strength.
