...
export async function saveSnapshotToDatabase(snapshot, opts = {}) {
  ...
  if (sourceRows.length > 0) {
    ...
    // ignoreDuplicates:true skips re-ingested rows entirely, so needs_review can
    // get stuck at false even when date_confidence is estimated/inferred/low/none.
    // Patch it unconditionally for any source in this batch that has a non-exact date.
    const flagUpdates = sourceRows
      .filter(r => r.date_confidence && r.date_confidence !== "exact")
      .map(r => ({ id: r.id, needs_review: true }));
    if (flagUpdates.length) {
      await supabase.from("sources").upsert(flagUpdates, { onConflict: "id" });
    }
  }
