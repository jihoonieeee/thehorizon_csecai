 ...
 *
 * Columns written (created by docs/migrations/rawfact-analytics-v1.sql):
...
function buildRawfactSummary(source) {
  ...
  if (items.length === 0 && !source.rawfact_score_data) return null;

  const byPriority = (p) =>
    items.filter((i) => i.score_data?.evidence_priority === p).length;

  return {
    priority:    source.rawfact_score_data?.rawfact_priority || "archive_only",
    score:       source.rawfact_score_data?.rawfact_score ?? 0,
...
export async function persistAnalysisEvidence(sources) {
  ...
  if (knownMissing.size > 0) {
    console.warn(
      ...
      `apply docs/migrations/rawfact-analytics-v1.sql to persist them`
    );
  }
