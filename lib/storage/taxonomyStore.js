...
import { allReferenceRecords } from "../config/taxonomyRegistry.js";
...
export async function persistRawfacts(rawfacts = [], snapshotId = null) {
  const rows = rawfacts
    ...
    .map((r) => ({
      ...
      primary_threat_tags:  r.primary_threat_tags ?? null,
...
/**
 * Persist visual evidence (web charts/graphs with an extracted data series).
 * @param {object[]} items - external evidence objects that are is_visual / have chart_data
 * @param {string}   [snapshotId]
 */
export async function persistVisualEvidence(items = [], snapshotId = null) {
  const rows = items.filter((e) => e && e.evidence_id && (e.is_visual || e.chart_data)).map((e) => ({
