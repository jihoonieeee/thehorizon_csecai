...
function itemToRow(item, contentHash) {
  ...
  return {
    ...
    // Event date fields (separated from publication date for window-relevance decisions)
...
export async function getEvidenceHashes(supabase, sourceIds) {
  ...
  try {
    // Chunk size 100: with ~5 evidence items per source this keeps each response
    // well under Supabase's default 1000-row limit, preventing silent truncation
    // that causes sources to appear stale when they already have evidence.
