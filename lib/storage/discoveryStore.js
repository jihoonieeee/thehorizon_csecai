 ...
 * created by docs/migrations/web-discovery-v1.sql.
 *
 * Stores EVERY candidate (accepted, accept_with_review, archive_only, reject) so
 * rejected/derivative candidates are retained for audit and never lost. Accepted
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    if (error) {
      if (isMissingTableError(error)) {
        ...
        console.warn(`discoveryStore: table '${table}' missing — apply docs/migrations/web-discovery-v1.sql to persist it`);
...
function toRow(c, { snapshotId, runId } = {}) {
  return {
    ...
    route_flags:                c.route_flags ?? null,
    manual_review_required:     c.manual_review_required ?? null,
