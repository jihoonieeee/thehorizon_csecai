 ...
 *
 *   PLATFORM_AI_API_KEY   — GovTech AI Platform key (x-api-key auth) + other providers
 *   PLATFORM_AI_PROVIDER  — "platform_ai" | "gemini" | "openai-compatible" | "anthropic"  (default: platform_ai)
 *   PLATFORM_AI_BASE_URL  — base URL for openai-compatible gateways
 *   PLATFORM_AI_BASE_URL  — base URL for platform_ai (default: https://api-public.ai.tech.gov.sg)
 *
...
// ── HTTP proxy (CONNECT tunnel for SG IP restriction) ─────────────────────────
// When PROXY_URL is set (format: http://user:pass@host:port), all outbound
// fetch calls are routed through it via undici ProxyAgent.
...
function getDispatcher() {
  ...
  if (!_proxyAgent) _proxyAgent = new ProxyAgent({
    ...
    connections: 8,          // pool size — reuse tunnels across calls
    pipelining: 1,
    keepAliveTimeout: 60000, // keep tunnel open 60s between calls
...
// GovTech AI Platform (api-public.ai.tech.gov.sg) is the primary chatbot provider.
// Set PLATFORM_AI_PROVIDER=gemini + GEMINI_API_KEY to fall back to direct Gemini.
const DEFAULT_PROVIDER = "platform_ai";

const MODEL_TIERS = {
  // GovTech AI Platform (OpenAI-compatible, x-api-key auth).
  // cheap: fast structured-JSON calls (planner/selector/verifier — ~700 output tokens).
  // synthesis: final grounded answer — quality is paramount.
  platform_ai: {
    cheap:     process.env.PLATFORM_MODEL_CHEAP     || "gemini-2.5-flash-lite",
    standard:  process.env.PLATFORM_MODEL_STANDARD  || "gemini-2.5-flash",
    synthesis: process.env.PLATFORM_MODEL_SYNTHESIS || "azure.claude-sonnet-5",
  },
  gemini: {
...
// Longer prefixes win (e.g. "gemini-2.5-flash-lite" beats "gemini-2.5-flash").
const PRICING = {
  // GovTech Platform AI models (azure.* prefix routes via Azure backend)
  "azure.claude-sonnet":    { input:  3.00, output: 15.00 },
  "azure.claude-haiku":     { input:  1.00, output:  5.00 },
  "azure.claude-opus":      { input:  5.00, output: 25.00 },
  "bedrock.claude-sonnet":  { input:  3.00, output: 15.00 },
  "bedrock.claude-haiku":   { input:  1.00, output:  5.00 },
  ...
  // Gemini (direct or via GovTech platform)
...
export function platformConfig() {
  ...
  const apiKey =
    (provider === "platform_ai"       ? process.env.PLATFORM_AI_API_KEY : null) ||
    ...
    null;
  const baseUrl =
    (provider === "platform_ai"       ? process.env.PLATFORM_API_BASE_URL || "https://api-public.ai.tech.gov.sg" : null) ||
    process.env.PLATFORM_AI_BASE_URL ||
    (provider === "openai-compatible" ? "https://api.openai.com/v1" : null);
  const proxySecret = process.env.PROXY_SECRET || "";
  return { provider, apiKey, baseUrl, proxySecret };
}
    ...
    try {
      if (stream) {
        const res = await fetch(`${base}:streamGenerateContent?alt=sse&key=${cfg.apiKey}`, {
          method: "POST", signal: controller.signal, dispatcher: getDispatcher(),
...
async function openaiChat(cfg, model, { system, messages, json, schema, maxTokens, timeoutMs, stream, onText }) {
  ...
  try {
    if (stream) {
      ...
      const res = await fetch(url, { method: "POST", signal: controller.signal, dispatcher: getDispatcher(), headers, body: JSON.stringify(body) });
  ...
  try {
    if (stream) {
      const res = await fetch(ANTHROPIC_API_URL, {
        method: "POST", signal: controller.signal, dispatcher: getDispatcher(), headers, body: JSON.stringify({ ...body, stream: true }),
      });
    ...
    const res = await fetch(ANTHROPIC_API_URL, { method: "POST", signal: controller.signal, dispatcher: getDispatcher(), headers, body: JSON.stringify(body) });
    clearTimeout(timer);
    if (!res.ok) { const t = await res.text().catch(() => ""); throw new Error(`Anthropic HTTP ${res.status}: ${t.slice(0, 200)}`); }
    const data = await res.json();
    return {
...
// OpenAI-compatible endpoint with x-api-key auth (not Bearer).
async function platformAiChat(cfg, model, { system, messages, json, schema, maxTokens, timeoutMs, stream, onText }) {
  const msgs = [];
  const sysText = systemToString(system);
  if (sysText) msgs.push({ role: "system", content: sysText });
  for (const m of messages) {
  ...
  const baseUrl = (cfg.baseUrl || "https://api-public.ai.tech.gov.sg").replace(/\/$/, "");
  const url = `${baseUrl}/platform/models/v1/chat/completions`;
  const headers = { "Content-Type": "application/json", "x-api-key": cfg.apiKey };
  if (cfg.proxySecret) headers["x-proxy-secret"] = cfg.proxySecret;
  const MAX_RETRIES = 2;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      if (stream) {
        const res = await fetch(url, { method: "POST", headers, dispatcher: getDispatcher(), body: JSON.stringify({ ...body, stream: true }), signal: controller.signal });
        if (!res.ok) { const t = await res.text().catch(() => ""); throw new Error(`PlatformAI HTTP ${res.status}: ${t.slice(0, 200)}`); }
        const reader = res.body.getReader();
        const decoder = new TextDecoder();
        let buf = "", full = "", inputTokens = null, outputTokens = null;
        for (;;) {
        ...
        clearTimeout(timer);
        return { text: full, inputTokens, outputTokens, model, provider: "platform_ai" };
      }

      const res = await fetch(url, { method: "POST", headers, dispatcher: getDispatcher(), body: JSON.stringify(body), signal: controller.signal });
      clearTimeout(timer);
      if (!res.ok) {
        const t = await res.text().catch(() => "");
        if ((res.status === 503 || res.status === 429) && attempt < MAX_RETRIES) {
          const wait = (attempt + 1) * 5000;
          process.stdout.write(` [PlatformAI ${res.status}→retry in ${wait / 1000}s]\n`);
          await sleep(wait);
          continue;
        }
        throw new Error(`PlatformAI HTTP ${res.status}: ${t.slice(0, 200)}`);
      }
      const data = await res.json();
      const choice = data.choices?.[0];
      const finishReason = choice?.finish_reason;
      // "length" means max_tokens was hit before generation completed — the response
      // is truncated. Log it so it's visible rather than silently returning empty text.
      // Common cause: thinking/reasoning models consuming most of the token budget.
      if (finishReason === "length") {
        process.stderr.write(`[PlatformAI] WARNING: finish_reason=length on ${model} — response truncated. Raise maxTokens or reduce context.\n`);
      }
      return {
        text:         choice?.message?.content || "",
        inputTokens:  data.usage?.prompt_tokens     ?? null,
        outputTokens: data.usage?.completion_tokens ?? null,
        finishReason, model, provider: "platform_ai",
      };
    } catch (err) {
      clearTimeout(timer);
      if (err.name === "AbortError") throw new Error(`PlatformAI: timeout after ${timeoutMs}ms`);
      throw err;
    }
  }
  throw new Error("PlatformAI: max retries exceeded");
}
...
export async function platformChat(opts = {}) {
  ...
  // Diagnostic log — shows up in Vercel function logs; safe (key is masked).
  console.log(`[platformChat] provider=${cfg.provider} tier=${tier} model=${model} stream=${stream} key=${cfg.apiKey ? cfg.apiKey.slice(0,8)+"…" : "MISSING"} base=${cfg.baseUrl || "(default)"}`);
  ...
  switch (cfg.provider) {
    ...
    case "platform_ai":       return platformAiChat(cfg, model, args);
