...
export const GEMINI_LITE  = process.env.GEMINI_LITE_MODEL  || "gemini-2.5-flash-lite";  // flash-lite: higher free-tier RPM, cleaner JSON for extraction tasks
export const GEMINI_FLASH = process.env.GEMINI_FLASH_MODEL || "gemini-2.5-flash-lite";  // flash-lite outperforms flash on JSON reliability for 15K-char evidence extraction
...
export const TASK_PROFILES = {
  ...
  discovery_query_planning: {
    ...
    allow_local:         true,
    cost_rule:           "Flash-Lite — once per mission per run; never escalate",
  },
  ...
  layer3_validation: {
    ...
    anthropic_model:     "haiku",
    gemini_tier:         "flash",   // flash not lite: 15+ fields needs more capacity than lite
    max_tokens:          1200,
    requires_json:       true,
  ...
  source_relevance: {
    ...
    // Gemini-first (Flash-Lite): cheap per-source triage, keep validation off
    // Anthropic by default. Anthropic Haiku remains as a quality fallback.
  ...
  source_relevance_qa: {
    ...
    // Gemini-first: second-model check; a cross-provider default (Gemini QA'ing
    // a Gemini verdict) is acceptable here since it also grounds against the text.
    preferred_providers: ["gemini", "anthropic", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          500,
  ...
  source_quality_gate: {
    ...
    allow_local:         true,
    cost_rule:           "Flash-Lite — simple 4-way classification, not analysis",
  },
  ...
  date_extraction: {
    ...
    allow_local:         true,
    cost_rule:           "Flash-Lite — tiny output, runs only on dateless discovery candidates",
  },
  ...
  research_significance: {
    ...
    allow_local:         true,
    cost_rule:           "Haiku — one small call per research source, idempotent (never re-scored unless forced)",
  },
