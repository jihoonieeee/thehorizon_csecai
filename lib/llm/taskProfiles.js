 ...
 *   Anthropic → claude-haiku-4-5 (bulk per-source) / claude-sonnet-4-6 (analysis) / claude-opus-4-8 (slides)
 *
 * ── PROVIDER ORDER ────────────────────────────────────────────────────────────
 *   All layers: anthropic (Haiku/Sonnet/Opus by task) → gemini fallback
 *
 * ── PIPELINE LAYER MAP ────────────────────────────────────────────────────────
 *   L1-L3:  deterministic gates; source_filtering/layer3_validation/source_relevance/
 *           source_quality_gate/source_typing → Haiku
 *   L4:     source_understanding/research_significance → Haiku
 *   L5A:    taxonomy_tagging, evidence_extraction, evidence_judgment → Haiku
 *   L5B:    analytics_extraction → Haiku
 *   L5e:    evidence_search → Sonnet
 *   L6:     category_synthesis/cross_category_synthesis/development_generation/
 *           insight_generation/outlook_generation/strategic_themes → Sonnet
 *   L7:     slide_content/slide_content_critical/batch/standard → Sonnet;
 *           slide_planning/slide_content_appendix/visual_diagram/visual_planning → Haiku
...
export const GEMINI_LITE  = process.env.GEMINI_LITE_MODEL  || "gemini-2.5-flash";
...
export const TASK_PROFILES = {
  ...
  source_filtering: {
    ...
    allow_local:         true,
    cost_rule:           "Haiku — binary pre-filter; no reasoning required",
  },
  ...
  discovery_triage: {
    ...
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter", "cloudflare"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          700,
    requires_json:       true,
    ...
    allow_local:         false,
    cost_rule:           "Haiku — bulk per-candidate triage; Gemini Flash-Lite fallback",
  },

  /**
   * discovery_query_planning — Layer 1B LLM query planner
   * Generates targeted, recency-anchored search queries per mission. Replaces the
   * deterministic taxonomy/artifact/site_scoped/operational families. Runs once
   * per mission per run (~8 missions total). Flash-Lite is cheap and sufficient
   * for structured query generation.
   */
  discovery_query_planning: {
    description:         "Layer 1B: craft targeted web-search queries for a discovery mission",
    preferred_providers: ["anthropic", "gemini", "openai", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          1500,
    requires_json:       true,
    pipeline_layer:      "L1B-web-discovery-query-planning",
    allow_local:         false,
    cost_rule:           "Haiku — once per mission per run; Gemini Flash-Lite fallback",
  },
  ...
  discovery_early_signal_qa: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — QA only moderate/strong early signals; not the full candidate set",
  },

  /**
   * layer3_validation — unified Layer 3 LLM call (runLayer3Llm)
   * Produces 15+ fields: ai_threat_focus, ai_materiality, content_quality,
   * trust_tier, source_type, candidate_domain, reading_value, verdict, summary,
   * boundary_rationale, evidence_quality, etc. Anthropic-first because Haiku is
   * more reliable for multi-field structured JSON than Gemini Flash-Lite.
   * Uses the 52KB layer3.md system prompt (prompt-cached after first call).
   */
  layer3_validation: {
    description:         "Unified Layer 3: relevance gate, source typing, trust tier, reading value, routing verdict",
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          1200,
    requires_json:       true,
    pipeline_layer:      "L3-validate",
    allow_local:         false,
    cost_rule:           "Haiku — per-source validation; 52KB system prompt cached after first call in a run",
  },

  /**
   * reading_value_relabel — standalone reading_value assessment (scripts/relabelReadingValues.js)
   * Re-scores already-classified corpus sources against the shared rubric in
   * lib/prompts/scoring/reading-value.md when editorial definitions change. One call
   * per source, one field out — never runs in the ingest path.
   * Platform-first: this is a bulk editorial judgement and should run on the same
   * PLATFORM_PIPELINE_MODEL as Layer 3 so relabels match what ingest would assign.
   */
  reading_value_relabel: {
    description:         "Standalone reading_value re-assessment for corpus relabel runs",
    preferred_providers: ["platform", "anthropic", "gemini", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          600,
    requires_json:       true,
    pipeline_layer:      "ops-reading-value-relabel",
    allow_local:         false,
    cost_rule:           "One call per source; rubric system prompt is prompt-cached after the first call in a run",
  },
  ...
  source_relevance: {
    ...
    allow_local:         true,
    cost_rule:           "Haiku — per-source triage; gated by a deterministic keyword pre-pass so off-topic noise never reaches the LLM",
  },
  ...
  /**
   * source_quality_gate — Layer 3 content-quality classification
   * Marketing / thin / substantive classification (contentQualityGate.js). Was
   * previously unlisted → inherited the anthropic-first source_understanding
   * profile. Given its own gemini-first profile so cheap gating stays off Anthropic.
   */
  source_quality_gate: {
    description:         "Layer 3 content-quality classification: marketing / thin / substantive",
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          400,
    requires_json:       true,
    pipeline_layer:      "L3-contentQuality",
    allow_local:         true,
    cost_rule:           "Haiku — 3-way classification; no reasoning required",
  },

  /**
   * date_extraction — Layer 1C publish-date recovery for web-discovery sources
   * Reads the first 2 000 chars of article text and returns the explicit publish
   * date if one is clearly stated. Runs only on candidates whose HTML metadata
   * did not yield an authoritative date (i.e. date_confidence !== "exact").
   */
  date_extraction: {
    description:         "Extract publish date from article text for undated discovery candidates",
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          64,
    requires_json:       true,
    pipeline_layer:      "L1C-date-extraction",
    allow_local:         true,
    cost_rule:           "Haiku — tiny output, runs only on dateless discovery candidates",
  },
  ...
  source_typing: {
    ...
    allow_local:         true,
    cost_rule:           "Haiku — disambiguation from bounded options; no reasoning required",
  },
  ...
  /**
   * report_extraction — split a long landscape report into per-finding items.
   * Haiku: bulk structured JSON over MANY chunks × MANY reports, so cost matters.
   * The system prompt (mechanism vocab) is identical across every chunk and every
   * report → prompt-cached, so only the first call pays for it.
   */
  report_extraction: {
    description:         "Extract distinct findings from a multi-topic landscape report (chunked)",
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          3000,
    requires_json:       true,
    pipeline_layer:      "L1-report-fanout",
    allow_local:         true,
    cost_rule:           "Haiku + cached system prompt — chunked over long reports; idempotent (a report is fanned out once)",
  },

  /**
   * research_significance — orthogonal significance overlay for research sources.
   * One cheap call per research_finding/benchmark source to separate landmark work
   * (opens a new attack surface) from routine/incremental papers, so "research" is
   * no longer a flat tier. Advisory only — never changes the deterministic importance
   * tier; used as a secondary sort key + slide/insight preference.
   */
  research_significance: {
    description:         "Rank a research/benchmark source: landmark|notable|routine|incremental + novelty",
    preferred_providers: ["anthropic", "gemini", "groq", "openrouter"],
    anthropic_model:     "haiku",
    gemini_tier:         "lite",
    max_tokens:          700,
    requires_json:       true,
    pipeline_layer:      "L4-significance",
    allow_local:         true,
    cost_rule:           "Haiku — 4-way classification; one call per research source, idempotent",
  },
  ...
  evidence_extraction: {
    ...
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          6000,   // raised from 4000: academic papers with 5-8 items + research_metadata + relationships exceed 4k
    requires_json:       true,
  ...
  category_synthesis: {
    ...
    gemini_tier:         "standard", // "pro" was unrecognized → fell to Flash (too small for large dossiers)
    max_tokens:          6000,
    requires_json:       true,
    pipeline_layer:      "L6-analysis-category-synthesis",
    allow_local:         false,
  ...
  final_qa: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — QA gate; call once per category",
  },
  ...
  evidence_qa: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — second-model QA on high-priority items only; a few calls per run",
  },
  ...
  claim_first_slide: {
    ...
    allow_local:         false,
    cost_rule:           "Anthropic Sonnet — highest-priority slides; claim-grounded analytical content",
  },
  ...
  slide_content: {
    ...
    anthropic_model:     "sonnet",
    gemini_tier:         "standard",
    // A slide output is headline + 3-4 short bullets + speaker notes: ~400-600 tokens.
    // 2000 is a safe ceiling that prevents truncation without paying for 8K headroom.
    max_tokens:          2000,
    requires_json:       true,
   ...
   * One call generates all slides for a category (typically 3–4 theme slides per call).
   * Real output budget: 4 slides × 3 bullets × ~30 words × 1.5 tok/word ≈ 540 tokens
   * of bullets + ~200 headline/notes overhead per slide = ~3000 tokens peak.
   * 5000 is a comfortable ceiling; 12000 was 4× wasteful and biased the model
   * toward verbose output that then gets trimmed by normaliseBullet().
   */
  slide_content_batch: {
  ...
  slide_content_appendix: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — formatting-only; no analytical reasoning",
  },

  /**
   * slide_content_insights — Layer 7, category_insights slides
   * Insight slides receive fully-structured judgments from L5/L6 and reformat
   * them into 3-5 strategic bullets — a structured-output task, not deep reasoning.
   * Haiku is sufficient and costs ~20× less than Sonnet.
   */
  slide_content_insights: {
    description:         "Category insights slide: reformat L5/L6 judgments into 3-5 strategic bullets",
    preferred_providers: ["anthropic", "gemini"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          3000,
    requires_json:       true,
    pipeline_layer:      "L7-slide-content-insights",
    allow_local:         false,
    cost_rule:           "Haiku — structured reformatting of pre-synthesised judgments, not new reasoning",
  },
  ...
  insight_qa: {
    ...
    allow_local:         false,
    cost_rule:           "Gemini primary — intentional cross-provider QA; Haiku fallback; 5 calls max",
  },
   ...
   * Sonnet: this is a structured JSON slot-assignment task (map developments/insights
   * to slide types), not deep reasoning. Sonnet produces identical output at 5× lower
   * cost and 3× lower latency. Opus added no measurable quality improvement here.
   */
  slide_planning: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — structured JSON slot-assignment, not reasoning",
  },
  ...
  case_diagram: {
    ...
    // Mermaid generation from a pre-supplied attack chain is mechanical structured
    // output, not deep reasoning — Haiku handles it well and is ~20× cheaper than
    // Sonnet on the fallback path (when OpenAI gpt-4o-mini is quota-exhausted).
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          600,
    requires_json:       true,
    ...
    allow_local:         false,
    cost_rule:           "Haiku/gpt-4o-mini: bounded JSON output, evidence-grounded diagram only",
  },
  ...
  visual_diagram: {
    ...
    allow_local:         false,
    cost_rule:           "Haiku — structured DSL generation from pre-specified nodes",
  },

  /**
   * visual_planning — Layer 7/8
   * Sonnet pass after slide content generation to plan visuals for each slide.
   */
  visual_planning: {
    description:         "Visual planning: reviews slide content and emits visual_requirement per slide",
    preferred_providers: ["anthropic", "gemini"],
    anthropic_model:     "haiku",
    gemini_tier:         "standard",
    max_tokens:          6000,
    ...
    allow_local:         false,
    cost_rule:           "Haiku — structured JSON slot-assignment from bounded evidence",
  },

  // ── Dashboard insights generation ─────────────────────────────────────────────

  /**
   * dashboard_insights — per-category strategic insights synthesis (was: direct Anthropic Sonnet call)
   * One call per category per insights run (4 max). Gemini-first so LLM_ONLY_GEMINI=1
   * routes correctly; Anthropic Sonnet as fallback when Gemini unavailable.
   */
  dashboard_insights: {
    description:         "Per-category strategic insights synthesis from evidence findings for the dashboard",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "sonnet",
    gemini_tier:         "flash",
    max_tokens:          3200,
    requires_json:       true,
    pipeline_layer:      "L6-dashboard-insights",
    allow_local:         false,
    cost_rule:           "Gemini Flash — 4 calls per insights run (one per category); Anthropic Sonnet fallback",
  },

  /**
   * dashboard_insight_qa — headline QA pass over generated insights
   * Rejects paper summaries, overreach, and fabricated claims.
   */
  dashboard_insight_qa: {
    description:         "QA pass over generated insights: reject summaries, overreach, fabrication",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          1200,
    requires_json:       true,
    pipeline_layer:      "L6-dashboard-insight-qa",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — 4 calls per insights run",
  },

  /**
   * dashboard_attribution — tag the supporting sources for each insight
   * Haiku-class: matching task (pick source numbers), not analytical reasoning.
   */
  dashboard_attribution: {
    description:         "Tag supporting source numbers for each insight citation list",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          700,
    requires_json:       true,
    pipeline_layer:      "L6-dashboard-attribution",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — 4 calls per insights run",
  },

  /**
   * dashboard_citation_grounding — verify explanation bullets vs cited source text
   * Called per category (batched in fix 2); validates each bullet is grounded.
   */
  dashboard_citation_grounding: {
    description:         "Verify insight explanation bullets are supported by their cited sources (batched per category)",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          3500,
    requires_json:       true,
    pipeline_layer:      "L6-dashboard-citation-grounding",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — per-insight now, per-category after fix 2",
  },

  /**
   * dashboard_assessment_qa — validate the one-sentence category posture
   * Short output; Gemini Flash or Haiku sufficient.
   */
  dashboard_assessment_qa: {
    description:         "QA the per-category posture assessment sentence",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          300,
    requires_json:       true,
    pipeline_layer:      "L6-dashboard-assessment-qa",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — 4 calls per insights run",
  },
  ...
  /**
   * newsletter_selection — editorial source selection from candidate pool
   * Sonnet-class: editorial judgment across 8–12 sources from a 60-candidate pool.
   */
  newsletter_selection: {
    description:         "Newsletter editorial source selection: pick 8-12 from the candidate pool",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "sonnet",
    gemini_tier:         "flash",
    max_tokens:          2000,
    requires_json:       true,
    pipeline_layer:      "newsletter-source-selection",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Anthropic Sonnet — once per newsletter run",
  },

  /**
   * newsletter_blurb — 2-sentence reading list blurb per source (batched at 5)
   * Haiku-class: summary reformatting, not analytical reasoning.
   */
  newsletter_blurb: {
    description:         "Reading list blurbs: 2-sentence summaries per source batch (5 per call)",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          1600,
    requires_json:       true,
    pipeline_layer:      "newsletter-blurb-generation",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — batched at 5 sources per call",
  },

  /**
   * newsletter_dedup — identify duplicate event coverage across selected sources
   * Haiku-class: cluster detection over short summaries, not deep analysis.
   */
  newsletter_dedup: {
    description:         "Identify duplicate event coverage across newsletter-selected sources",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "haiku",
    gemini_tier:         "flash",
    max_tokens:          1200,
    requires_json:       true,
    pipeline_layer:      "newsletter-dedup-qa",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Haiku — once per newsletter run",
  },

  /**
   * newsletter_intros — one editorial paragraph per threat category
   * Sonnet-class: synthesis over blurbs + insights, not just reformatting.
   */
  newsletter_intros: {
    description:         "Editorial intro paragraph per threat category for the newsletter",
    preferred_providers: ["gemini", "anthropic"],
    anthropic_model:     "sonnet",
    gemini_tier:         "flash",
    max_tokens:          1200,
    requires_json:       true,
    pipeline_layer:      "newsletter-category-intros",
    allow_local:         false,
    cost_rule:           "Gemini Flash / Anthropic Sonnet — once per newsletter run",
  },
};
...
export const DEFAULT_PROVIDER_ORDER = ["anthropic", "gemini", "openai", "groq", "cloudflare", "openrouter", "ollama"];

export const LAYER_ROUTING = {
  "L1 — Ingest":             "deterministic only",
  "L1B — Web Discovery":     "discovery_query_planning → Flash-Lite (LLM-crafted queries per mission); discovery search → claude web_search (Anthropic)",
  "L1C — Discovery Triage":  "discovery_triage → Haiku; discovery_early_signal_qa → Haiku (moderate/strong only)",
  "L2 — Clean":              "deterministic only",
  "L3 — Validate+Archive":   "layer3_validation → Haiku; source_relevance → Haiku; source_relevance_qa → Haiku; source_quality_gate → Haiku; source_typing → Haiku",
  "L4 — Taxonomy":           "source_understanding → Haiku; research_significance → Haiku",
  "L5A — Rawfacts":          "taxonomy_tagging → Haiku; evidence_extraction → Haiku; evidence_judgment → Haiku",
  ...
  "L5B — Analytics":         "analytics_extraction → Haiku",
  "L6 — Category Analysis":  "category_analysis/category_synthesis → claude-sonnet-4-6 → gemini-2.5-pro; 4 calls max",
  "L6 — Cross-Category":     "cross_category_synthesis → claude-sonnet-4-6 → gemini-2.5-pro; once per run",
  "L6 — QA":                 "final_qa + evidence_qa → Haiku → gemini-2.5-pro",
  "L6 — Dashboard Insights": "dashboard_insights → Gemini Flash (4/run); dashboard_insight_qa/attribution/citation_grounding/assessment_qa → Gemini Flash (4–16/run)",
  "L7 — Slide Content":      "slide_content/critical/batch/standard → claude-sonnet-4-6; planning/appendix/visual → Haiku",
  "L9 — PPTX Export":        "deterministic (PptxGenJS)",
  "Newsletter":               "newsletter_selection/intros → Gemini Flash (1 each); newsletter_blurb → Gemini Flash (batched); newsletter_dedup → Gemini Flash (1)",
};
