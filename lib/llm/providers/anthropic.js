...
export async function callAnthropic(config, systemPrompt, userPrompt, opts = {}) {
  ...
  const body = {
    ...
    // When caching, send `system` as a content-block array with an ephemeral
    // cache_control breakpoint on the single stable block. Any byte change to
    // systemPrompt invalidates the cache, so it must stay identical across a run
    // — buildSystemPrompt()/taxonomy are deterministic (no timestamps), so it does.
    system: useCache
