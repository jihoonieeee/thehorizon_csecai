...
export async function callAnthropic(config, systemPrompt, userPrompt, opts = {}) {
  ...
  const {
    maxTokens = 4000,
    timeoutMs = 60000,
    // Prompt caching: cache the (per-run-identical) system prompt so the hundreds
    // of per-source calls in a run re-read it at ~0.1x instead of 1x. The system
    // prompt is the cacheable PREFIX (tools→system→messages render order); the
    // per-source user prompt stays volatile after the breakpoint. Enabled by
    // default when the system prompt is long enough to plausibly clear the model's
    // minimum cacheable prefix (~2K–4K tokens; ~8K chars). Below that the API
    // silently doesn't cache — harmless, just no benefit.
    cacheSystem = true,
    // When a JSON schema is provided, use tool_use to force structured output.
    // This makes preamble structurally impossible — the model must emit JSON into
    // the tool input block. Far more reliable than prompt instructions alone,
    // especially for Haiku which frequently adds preamble to JSON-only prompts.
    schema = null,
  } = opts;

  // Minimum cacheable prefix is ~1024 tokens (~4K chars) per Anthropic docs, but
  // in practice the API accepts ephemeral blocks for shorter prompts and silently
  // skips the cache when below the internal minimum — so lowering this threshold
  // means we attempt caching for every slide system prompt (~2-4K chars) at no
  // extra cost if the cache miss occurs, and save ~90% on repeated cache reads.
  const useCache = cacheSystem && systemPrompt && systemPrompt.length >= 1000;

  const systemBlock = useCache
    ? [{ type: "text", text: systemPrompt, cache_control: { type: "ephemeral" } }]
    : systemPrompt;

  // When schema provided: use tool_use to enforce structured JSON output.
  // tool_choice: { type: "tool" } forces the model to call the tool (no text block).
  const useToolUse = schema != null;
  ...
  try {
    ...
    let content;
    if (useToolUse) {
      // Extract JSON from the tool_use block — guaranteed structured, no preamble.
      const toolBlock = Array.isArray(data?.content)
        ? data.content.find((b) => b.type === "tool_use" && b.name === "result")
        : null;
      if (!toolBlock?.input) {
        throw new Error(`${label} tool_use returned no input (stop_reason=${data?.stop_reason})`);
      }
      // Return as serialised JSON string so the router's safeParseJson() works uniformly.
    ...
    // costs rather than the character-count estimate fallback. cache_* fields let
    // the router bill cached reads at ~0.1x and surface cache-hit rate.
    return {
      text:                     content,
