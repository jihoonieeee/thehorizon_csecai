 ...
 *
 * Anthropic prompt-caching pricing: cache READS bill at ~0.1x the input rate,
 * cache WRITES (creation) at ~1.25x. `inputTokens` from the API is the uncached
 * remainder only, so cached reads/writes are billed separately here.
 *
 * @param {string} modelId   - Full model ID (e.g. "claude-sonnet-4-6")
 * @param {string} provider  - Provider key (e.g. "anthropic", "gemini")
 * @param {number} inputTokens
 * @param {number} outputTokens
 * @param {number} [cacheReadTokens=0]      - tokens served from cache (~0.1x)
 * @param {number} [cacheCreationTokens=0]  - tokens written to cache (~1.25x)
 * @returns {number} Estimated cost in USD
 */
export function estimateCallCost(modelId, provider, inputTokens, outputTokens, cacheReadTokens = 0, cacheCreationTokens = 0) {
  const pricing =
