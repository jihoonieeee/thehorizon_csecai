...
function geminiApiKeys() {
  return collectKeys("GEMINI_API_KEY");
}
...
function buildProviderExecutors(task, mode) {
  ...
  for (const provider of orderedProviders) {
    switch (provider) {
      case "gemini": {
        const tier    = profile.gemini_tier || "cheap";
        const modelId = tier === "standard" ? GEMINI_STANDARD[mode] : GEMINI_CHEAP[mode];
        if (!modelId) break;
      ...
      case "anthropic": {
        ...
        // Defaults to "sonnet" for cost safety; slide_content and speaker_notes
...
function safeParseJson(raw) {
  ...
  } catch { /* fall through to fence-stripping + extraction */ }
  ...
  // Local/cheap models frequently append explanation after the JSON value (or
  // emit a second object). Recover the first balanced JSON object/array.
...
export async function routedLLM(systemPrompt, userPrompt, opts = {}) {
  ...
  const attemptOnce = async (executor) => {
    ...
    try {
      ...
      const result = parseJson ? safeParseJson(raw) : raw;
      const latency_ms     = Date.now() - start;
      const outputEstimate = estimateTokens(typeof raw === "string" ? raw : JSON.stringify(raw));
      trackTokens(task, executor.provider, inputEstimate, outputEstimate);

      if (logLabel) {
        process.stdout.write(`  [LLM/${task}] ${executor.label} — ${latency_ms}ms ~${inputEstimate + outputEstimate} tokens\n`);
      }
      const output = {
        result,
        llm_metadata: {
          llm_used: true, model_used: executor.modelId, provider_used: executor.provider,
          model_mode: mode, task, cache_hit: false, latency_ms,
          token_estimate: { input: inputEstimate, output: outputEstimate, total: inputEstimate + outputEstimate },
        },
      };
...
export function logProviderStatus() {
  ...
  process.stdout.write(`  Anthropic:   ${anthropicKey}  model=${anthropicModel}  (evidence_search + strategic_insight only)\n\n`);

  const geminiCheap    = GEMINI_CHEAP[mode];
  const geminiStandard = GEMINI_STANDARD[mode];
  process.stdout.write(`  Gemini model selection for mode="${mode}":\n`);
  process.stdout.write(`    cheap tasks    → ${geminiCheap}\n`);
  process.stdout.write(`    standard tasks → ${geminiStandard}\n\n`);
}
