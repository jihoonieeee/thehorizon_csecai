  ...
  for (const provider of orderedProviders) {
    switch (provider) {
      ...
      case "platform": {
        ...
        const rawBase = (process.env.PLATFORM_AI_BASE_URL || "").replace(/\/$/, "");
        if (!apiKey || !rawBase) break;
        const modelId = process.env.PLATFORM_PIPELINE_MODEL || "gpt-4.1";
        ...
        executors.push({
          ...
          call: (sys, usr, opts) => callOpenAI(
            { apiKey, modelId, label: `Platform/${modelId}`, baseUrl: rawBase },
            sys, usr, { ...opts, maxTokens: profile.max_tokens, timeoutMs: timeout }
          ),
        });
        break;
