...
import { callPlatformAI }  from "./providers/platform_ai.js";
...
function buildProviderExecutors(task, mode) {
  ...
  // If a provider leads the global order but isn't in this task's preferred list,
  // prepend it so LLM_PROVIDER_ORDER=platform,gemini overrides task-level preferences
  // across all tasks without touching every profile.
  const globalPrimary = providerOrder[0];
  const taskPreferred = (globalPrimary && !profile.preferred_providers.includes(globalPrimary))
    ? [globalPrimary, ...profile.preferred_providers]
  ...
  for (const provider of orderedProviders) {
    switch (provider) {
      ...
      case "platform": {
        // See docs/platform-ai-api.md for endpoint/auth details.
        const apiKey  = process.env.PLATFORM_AI_API_KEY;
        const baseUrl = process.env.PLATFORM_API_BASE_URL || "https://api-public.ai.tech.gov.sg";
        if (!apiKey) break;
        const modelId = process.env.PLATFORM_PIPELINE_MODEL || "gpt-5.4";
        const timeout = parseInt(process.env.LLM_TIMEOUT_MS || "60000", 10);
        executors.push({
          provider: "platform",
          label:    `Platform (${modelId})`,
          modelId,
          call: (sys, usr, opts) => callPlatformAI(
            { apiKey, modelId, label: `Platform/${modelId}`, baseUrl },
            sys, usr, { ...opts, maxTokens: profile.max_tokens, timeoutMs: timeout }
          ),
        });
        break;
