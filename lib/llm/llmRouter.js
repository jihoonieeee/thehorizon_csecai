...
export async function routedLLM(systemPrompt, userPrompt, opts = {}) {
  ...
  const attemptOnce = async (executor) => {
    ...
    try {
      ...
      persistCallCost({ task, provider: executor.provider, model: executor.modelId, inputTokens: actualInput, outputTokens: actualOutput });

      if (logLabel) {
        const realCounts = raw?.inputTokens != null ? "" : "~";
        process.stdout.write(`  [LLM/${task}] ${executor.label} — ${latency_ms}ms ${realCounts}${actualInput + actualOutput} tokens\n`);
      }
      const output = {
        result,
        llm_metadata: {
