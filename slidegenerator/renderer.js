...
function addTitle(slide, headline, kicker) {
  ...
  // Headers are 28pt. Only shrink for an unusually long headline so it never spills.
  const hlText = String(headline || "");
  const pt     = hlText.length > 62 ? 24 : hlText.length > 48 ? 26 : 28;
  const lines  = estLines(hlText, pt, SAFE_W - MARGIN);
  const headH  = Math.max(0.50, lines * (pt / 24) * 0.44);
  slide.addText(clamp(hlText, 200), {
    x: MARGIN, y, w: SAFE_W - MARGIN, h: headH,
    fontSize: pt, bold: true, color: T.ink, fontFace: T.fontH, valign: "top", wrap: true,
  });
  y += headH;
...
function addBullets(slide, items, x, y, w, h, opts = {}) {
  ...
  slide.addText(runs, {
    x, y, w, h,
    fontSize: opts.sz || 24, fontFace: T.fontB, color: T.body,
...
// ── Masters ───────────────────────────────────────────────────────────────────
const SEC_BG = "0D1F35";   // deep navy — dark, clear contrast for white text
function defineMasters(pptx) {
  ...
  pptx.defineSlideMaster({ title: "SEC_WHITE", background: { color: SEC_BG } });
  pptx.defineSlideMaster({ title: "SEC_LIGHT", background: { color: SEC_BG } });
  pptx.defineSlideMaster({ title: "SEC_DARK",  background: { color: SEC_BG } });
}
...
// Section dividers — all use deep navy background, white headline, steel-blue number.
// Three functions kept so the SEC[n%3] cycle still works; all render identically.
function buildSection(masterName, pptx, slide, num) {
  const s      = pptx.addSlide({ masterName });
  const hl     = clamp(slide.headline || "", 52);
  const lines  = estLines(hl, 46, W - MARGIN * 2);
  const headH  = Math.max(0.80, lines * 0.74);
  const startY = (H - 0.34 - 0.14 - headH) / 2 - 0.10;

  // Section number — small, spaced, steel blue
  s.addText(String(num).padStart(2, "0"), {
    x: MARGIN, y: startY, w: W - MARGIN * 2, h: 0.34,
    fontSize: 13, bold: true, color: "5B8AB8", fontFace: T.fontB,
    charSpacing: 5, align: "left", valign: "middle",
  });
  // Short accent rule under number
  s.addShape("rect", { x: MARGIN, y: startY + 0.38, w: 0.60, h: 0.035,
    fill: { color: "3A6EA8" }, line: { color: "3A6EA8" } });
  // Headline — large, white
  s.addText(hl, {
    x: MARGIN, y: startY + 0.44, w: W - MARGIN * 2, h: headH,
    fontSize: 46, bold: true, color: "FFFFFF", fontFace: T.fontH,
    align: "left", valign: "top", wrap: true,
  });
  // Description — pale blue
  if (slide.description) {
    s.addText(clamp(slide.description, 120), {
      x: MARGIN, y: startY + 0.44 + headH + 0.22, w: W - MARGIN * 2, h: 0.38,
      fontSize: 14, color: "7AAAD0", fontFace: T.fontB, align: "left",
    });
  }
  note(s, slide.speaker_notes);
}
function buildSectionA(pptx, slide, num) { buildSection("SEC_WHITE", pptx, slide, num); }
function buildSectionB(pptx, slide, num) { buildSection("SEC_LIGHT", pptx, slide, num); }
function buildSectionC(pptx, slide, num) { buildSection("SEC_DARK",  pptx, slide, num); }
...
function buildTwoColumn(pptx, slide, pageNum, total) {
  ...
  addBullets(s, all.slice(0, mid), MARGIN,             top, cw, contH, { sz: 18, gap: 12 });
  addBullets(s, all.slice(mid),    MARGIN + cw + 0.32, top, cw, contH, { sz: 18, gap: 12 });
...
/** DEFAULT — bullets + optional diagram/metrics.
 *  Diagrams use a stacked layout (bullets compact at top, diagram full-width below)
 *  because they are horizontal and need the full slide width to be legible.
 *  Metrics use the original side-by-side layout. */
