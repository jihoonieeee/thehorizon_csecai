...
/**
 * Cascade fetch: direct → Jina AI Reader → Tavily Extract.
...
async function fetchArticleText(url) {
  ...
  if (jinaText) return { text: jinaText, via: "jina", status: "ok" };

  const tavilyText = await fetchViaTavily(url);
  if (tavilyText) return { text: tavilyText, via: "tavily", status: "ok" };

  return { text: null, via: null, status: direct.status };
}

async function main() {
  console.log(`\nRescrape short sources (min_chars=${MIN_CHARS}, limit=${LIMIT}, dry_run=${DRY_RUN})\n`);

  // Load all eligible sources whose text is short
  const all = [];
  for (let from = 0; ; from += 1000) {
    const { data, error } = await sb.from("sources")
      .select("id, title, url, publisher, source_family, reading_value, full_text, clean_text")
  ...
  const results = { updated: 0, skipped: 0, failed: 0, blocked: 0, byVia: {} };
  ...
  async function runSlot() {
    while (i < short.length) {
      ...
      const { text, via, status } = await fetchArticleText(src.url);
      ...
      process.stdout.write(`✓ ${oldLen} → ${text.length} chars [${via}]\n`);
