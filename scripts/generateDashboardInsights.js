...
async function main() {
  ...
  for (const cat of CATEGORIES) {
    ...
    // informative-tier sources are included only when the primary pool (essential+recommended)
    // is thin — gives TAI/LLM research depth without diluting richer categories.
    const INFORMATIVE_FALLBACK_THRESHOLD = 10;
    ...
    const catRows = primaryCatRows.length >= INFORMATIVE_FALLBACK_THRESHOLD
      ? primaryCatRows
      : allCatRows; // include informative fallback for thin categories
