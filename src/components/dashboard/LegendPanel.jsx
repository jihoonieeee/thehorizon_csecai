...
export const TAXONOMY_GROUPS = [
  { prefix: "TAI", color: "#3583C9", label: "Traditional AI (TAI)",
    ...
    tags: [
      { id: "TAI01_data_poisoning",            label: "Data Poisoning",            desc: "Manipulates inputs to the training process — data, labels, or learning signals — so the resulting model carries malicious behaviour without touching weights." },
      ...
      { id: "TAI04_adversarial_data",          label: "Adversarial Data",          desc: "Generates or manipulates adversarial input artifacts — cross-modal, semantic, or contextual — used to fool or probe ML systems." },
      ...
      { id: "TAI08_inference_api_abuse",       label: "Inference API Abuse",       desc: "Abuses a classical ML model's inference API for reconnaissance or intelligence gain — short of full model extraction or availability attack." },
      { id: "TAI09_model_denial_of_service",   label: "Model Denial of Service",   desc: "Degrades a classical ML model's availability or exhausts its inference compute — sponge inputs, flooding, or worst-case algorithmic path triggering." },
  ...
  { prefix: "LLM", color: "#9C62A7", label: "LLM Threats (LLM)",
    desc: "Attacks on an LLM's language/prompt/context/RAG/output surface; harm stays in model response or data.",
    tags: [
      { id: "LLM01_prompt_injection",          label: "Prompt Injection",          desc: "Attacker-controlled text overrides developer instructions — direct (user types it) or indirect (hidden in content the model ingests: web page, RAG doc, tool response, email)." },
      ...
      { id: "LLM03_llm_supply_chain",          label: "LLM Supply Chain",          desc: "Compromise of trust in how an LLM-stack component is produced, distributed, or installed — requires a hijacked account, registry, or distribution channel, not just a CVE." },
      ...
      { id: "LLM05_improper_output_handling",  label: "Output Handling Flaw",      desc: "The application (not the model) passes LLM output unvalidated to a downstream system that executes or renders it — SQL, HTML/XSS, shell commands." },
      ...
      { id: "LLM07_system_prompt_leakage",     label: "System Prompt Leakage",     desc: "Attacker extracts hidden system prompt, developer instructions, or orchestration logic — the secrets embedded in the system context, not user data." },
      { id: "LLM08_vector_embedding_weakness", label: "Embedding Weakness",        desc: "Weaknesses in embeddings/vector store — inversion of source text from vectors, retrieval ranking abuse, cross-tenant vector leakage, index poisoning." },
      ...
      { id: "LLM10_unbounded_consumption",     label: "Unbounded Consumption",     desc: "Drives uncontrolled resource or cost consumption against an LLM service — token flooding, denial-of-wallet, recursive context injection, or model theft via mass queries." },
      { id: "LLM11_jailbreak_safety_bypass",   label: "Jailbreak / Safety Bypass", desc: "The direct user defeats the model's own safety alignment/refusal training — adversarial suffixes, roleplay/DAN, many-shot priming, encoding tricks — with no external data channel." },
    ],
  },
  { prefix: "ASI", color: "#19BC9D", label: "Agentic AI (ASI)",
    desc: "Attacks against AI agents that act — tool calls, code execution, persistent memory, identity, multi-agent orchestration.",
    tags: [
      ...
      { id: "ASI02_tool_misuse_exploitation",        label: "Tool Misuse",               desc: "Drives an agent to invoke a legitimate, already-authorized tool in a harmful way — the permission model is fine; the specific action taken is the problem." },
      { id: "ASI03_identity_privilege_abuse",        label: "Identity / Privilege Abuse", desc: "Exploits what the agent is allowed to do — its identity, credentials, delegated permissions, or missing authorization/approval model is the weakness." },
      ...
      { id: "ASI05_unexpected_code_execution",       label: "Unexpected Code Execution", desc: "Code or command execution reached through an agentic execution path — the agent's own tool/shell invocation is the mechanism, not a deterministic API endpoint." },
      { id: "ASI06_memory_context_poisoning",        label: "Memory Poisoning",          desc: "Seeds an agent's long-term memory or context store with malicious data so corrupted state controls future turns or sessions — persists beyond current interaction." },
      { id: "ASI07_insecure_agent_comms",            label: "Insecure Agent Comms",      desc: "Exploits communication channels between agents or between agent and orchestrator — message injection, agent spoofing, replay attacks on handoff protocols." },
      ...
      { id: "ASI09_human_agent_trust_exploitation",  label: "Human-Agent Trust Exploit", desc: "Manipulates a human's trust in agent output to obtain a harmful authorisation — deceptive summaries, misleading recommendations, or action logs that cause the user to approve harm." },
      { id: "ASI10_rogue_agents",                    label: "Rogue Agents",              desc: "Unauthorized, unmonitored, or uncontrolled autonomous agents operating outside governance — shadow sessions, orphaned agents, or agents spawned without oversight." },
    ],
  },
  { prefix: "AE", color: "#FFAA22", label: "AI-Enabled Threats (AE)",
    ...
    tags: [
      { id: "AE01_ai_recon",                        label: "AI Recon",                  desc: "AI accelerates target discovery, profiling, scanning, or OSINT — automated asset enumeration, credential harvesting, or victim profiling from public data." },
      { id: "AE02_ai_social_engineering",            label: "AI Social Engineering",     desc: "AI generates phishing, pretexting, or persuasion content aimed at individuals — personalised spear-phishing, conversational manipulation, BEC pretexts." },
      ...
      { id: "AE04_ai_exploit_dev",                   label: "AI Exploit Dev",            desc: "AI generates, adapts, or weaponizes a working exploit from a known or discovered vulnerability — deliverable is functional attack code." },
      { id: "AE05_ai_malware_dev",                   label: "AI Malware Dev",            desc: "AI authors, mutates, or packages malicious software; also covers conventional malware distributed disguised as an AI artifact (fake model on a hub)." },
      { id: "AE06_ai_evasion_obfuscation",           label: "AI Evasion / Obfuscation",  desc: "AI makes malicious content or behaviour harder to detect — obfuscation, polymorphic packing, AI-rewritten payloads to evade AV/EDR/email security." },
      { id: "AE07_ai_identity_abuse",                label: "AI Identity Abuse",         desc: "AI-driven impersonation or synthetic-identity creation at scale — fake personas for fraud, synthetic KYC identities, automated credential abuse." },
      { id: "AE08_ai_attack_orchestration",          label: "AI Attack Orchestration",   desc: "AI autonomously coordinates a multi-stage attack chain — recon, access, lateral movement, action on objectives — with minimal human direction." },
      { id: "AE09_ai_disinformation",                label: "AI Disinformation",         desc: "AI generates disinformation, propaganda, or coordinated influence operations for population-scale narrative manipulation — troll networks, synthetic articles, astroturfing." },
      { id: "AE10_ai_deepfake",                      label: "AI Deepfake",               desc: "AI-generated synthetic video, audio, or image used as the weapon for individual fraud, impersonation, extortion, or targeted harm — voice cloning, face-swap fraud." },
    ],
  },
];
