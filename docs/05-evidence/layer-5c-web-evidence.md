...
Layer 5C is the pipeline's **single external-evidence branch**: it absorbed the retired Layer 5E
(external statistics search). Its quantitative/benchmark missions now also extract **grounded
statistics** (metric + value + timeframe + source + verbatim quote; a statistic is dropped unless its
number appears verbatim in its quote), and `synthesis/externalEvidence.js#webEvidenceToExternalEvidence`
adapts 5C's output into the `externalEvidence` shape the rest of synthesis consumes.

**Gating:** the branch **auto-enables when a search-provider key (Tavily/SerpAPI) is configured** and is a
graceful no-op otherwise; `WEB_EVIDENCE_ENABLED` is an explicit on/off override. It **degrades** on every
failure (records it, never throws). Heavy tooling (Playwright, poppler, cheerio, readability, pdf-parse) is lazy-imported and optional; without it the branch still runs with built-in regex extraction and marks screenshot/PDF work `manual_review`.
