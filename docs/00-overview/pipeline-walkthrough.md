...
Layer 5A  Rawfact       ← evidence extraction per high-priority source (atomic facts + categorical triage)
Layer 5B  Analytics     ← corpus aggregation, derived risk indexes, chart specs
Layer 5C  Gap-Fill Scrape  ← open-web page scraping to fill corpus gaps: finds missing case studies, stats, visuals (Tavily/SerpAPI; gated WEB_EVIDENCE_ENABLED)
Layer 5E  Stat Lookup     ← LLM looks up authoritative statistics to corroborate category claims (Anthropic web_search, once per category)
Layer 6   Analysis      ← Claim chain: observations → viewpoints → claims → deterministic priority → insights
...
### Database Storage
New columns added by `docs/migrations/000_schema.sql` (sections 7–8):
...
**Step 6 — Evidence Triage (deterministic, source_type-aware):**

Evidence items no longer receive a 0–100 numeric score. Instead, each item is run through a categorical triage that produces: `admissibility`, `evidence_strength`, `permitted_uses[]`, and `limitations[]`. See `docs/evidence-triage.md` for the complete data contract.

*Admissibility gates (deterministic):* An item fails immediately if: no traceable source URL, speculative language without concrete demonstration, generic opener (< 70 chars), marketing language, non-atomic compound claim, or the LLM judges `source_type_fit = false`. Failed items receive `evidence_strength = archive` and `permitted_uses = ["not_used"]`.

*Source-type permissions:* Each source type defines what evidence can prove. `governance_signal` can only produce `context_only`, `recommendation_input`, or `outlook_input` — it cannot prove attacker activity. `research_finding` cannot prove real-world adversary adoption. Triage enforces these bounds deterministically; the LLM cannot expand them.

*Evidence strength (categorical):*
- `strong` — admissibility passed, source-type permission clear, direct demonstration, concrete claim, no blocking limitation
- `usable` — admissibility passed, source-type-allowed claim, but one or more limitations apply
- `context` — useful for framing only; not proof of threat activity
- `archive` — inadmissible, generic, speculative, or source-type mismatch

*Limitations (deterministic + LLM judgement):* `single_source`, `lab_only`, `no_operational_observation`, `unclear_reproducibility`, `unclear_scope`, `unclear_ai_role`, `vendor_self_reported`, `uncertain_attribution`, `narrow_time_window`, `duplicate_reporting`, `weak_source_type_fit`, `missing_quantitative_detail`, `conflicting_evidence`. Limitations block specific claim types: `lab_only` blocks operational claims; `single_source` blocks trend claims without corroboration; `conflicting_evidence` blocks critical claims.

*Backward-compatible fields for downstream code:* `score_data.evidence_priority` maps from strength (`strong → "critical"` as a bucket label, `usable → "high"`, `context → "low"`, `archive → "archive_only"`). `score_data.evidence_score` maps from strength (80/60/30/0). `rawfact_priority` maps from strength (`strong → "must_read"`, `usable → "high"`, `context → "medium"`). Note: `"critical"` in `evidence_priority` is a **bucket label for the strongest evidence**; it is not the same as a claim-level critical priority.
...
**Step 8 — Duplicate Penalty:**
Non-representative cluster members are downgraded one strength level (strong→usable, usable→context, context→archive). This demotes duplicate claims from multiple sources covering the same event.

**Step 8b — Second-Model Evidence QA (optional LLM):**
A frontier model (Anthropic/Gemini Pro) independently verifies strong/usable items before they reach the evidence packs. It checks: is the `source_quote` actually traceable to the source? Is the `fact` a fair reading of the quote? Are numbers accurate? Items that fail are downgraded.

**Step 9 — Assemble Evidence Packs (deterministic):**
Sources are grouped by `main_category`. For each category, items are sorted by triage strength and placed into labelled buckets:
- `critical_evidence` — strong items (evidence_priority = "critical"), ≤5
- `high_evidence` — usable items (evidence_priority = "high"), ≤8
- `supporting_evidence` — context items (evidence_priority = "low"), ≤10
...
When strong + usable totals fewer than 3 items, up to 4 context items are promoted to `supporting_evidence_promoted`.

**Step 10 — Evidence QA (deterministic):**
Final quality pass: removes items with missing required fields, invalid evidence_type, generic facts, marketing claims without numbers, governance sources claiming attack behaviour without concrete evidence, and statistics without numbers. Strong items that fail the triage admissibility check are downgraded.

### Outputs
- Each source: `evidence_items[]` (each with `triage_data`, `score_data` backward-compat, `evidence_role`)
- Per-category: `evidence_packs` (bucketed: critical/high/supporting/stats/cases/mitigations/outlook)
- Source-level: `rawfact_score_data` (best item triage strength used as source-level priority)

### Summary
The rawfact branch takes high-priority sources and drills down to the specific, verifiable facts inside them. Every fact must be backed by a verbatim sentence from the source. Each fact is triaged using categorical states (strong/usable/context/archive) bounded by source-type permissions — no arbitrary numeric weights. Similar facts from different sources are detected and the duplicates are downgraded. The result is a set of pre-bucketed evidence packs that the claim chain and analysis layer draw on directly. See `docs/evidence-triage.md` for the full triage chain.
...
## Component 5C — Gap-Fill Web Scrape (Layer 5C)
...
### Visuals
Each visual gets a usefulness level (high/medium/low/not_useful) and a slide decision (embed/redraw/cite_only/manual_review/reject). Only embed/redraw enter automatic slides; redraw recreates charts from extractable values (never from pixels); every visual must bind to a claim. See `docs/05-evidence/visual-usefulness-logic.md` and `docs/05-evidence/slide-visual-selection-logic.md`.
...
### Detailed docs
`docs/05-evidence/layer-5c-web-evidence.md`. DB: `docs/migrations/000_schema.sql` (section 10).

> Layer 5C **scrapes web pages** to fill evidence gaps the corpus is missing; Layer 5E **looks up authoritative statistics** to put hard numbers behind existing claims. Different tools, different triggers, complementary outputs.

---

## Component 5E — Authoritative Stat Lookup (Layer 5E)
...
### Component 6B — Category Analysis via Claim Chain (analyzeCategory.js)

For each active category (≥2 sources), the analysis runs as a 3-layer claim chain instead of the old 4-section approach. See `docs/evidence-triage.md` for the complete data contract.

**The fundamental design rule:** The LLM may classify, interpret, group, and explain. The LLM must NOT directly decide final critical/high/medium priority. Deterministic gates assign `claim_priority`.

**Step 1 — Observation Generation (LLM):**
*Input:* All strong and usable evidence items from the dossier (with their `triage_data`).
*Task:* Generate 0–8 factual observations — constrained patterns derived from converging evidence.
*Rules:* An observation is generated ONLY when multiple evidence items converge, or one exceptional strong cluster exists. Observation text must be factual, non-executive, non-speculative, close to the evidence. Each observation cites allowed evidence IDs.
*Output fields:* `observation_id`, `observation_text`, `observation_type`, `supporting_evidence_ids`, `evidence_types`, `source_types`, `limitations`, `observation_scope`, `signal_temporality`

**Step 2 — Viewpoint Generation (LLM):**
*Input:* Observations from Step 1 + triaged evidence context.
*Task:* Generate 0–6 viewpoints explaining WHY observations matter analytically — what change is happening and what drives it.
*Rules:* Must not exceed source-type permissions (research cannot prove operational use; governance cannot prove attacker activity). Viewpoint strength (strong/moderate/weak) is self-assigned but bounded by evidence.
*Output fields:* `viewpoint_id`, `viewpoint_text`, `viewpoint_type`, `analytical_change`, `change_driver`, `supporting_observation_ids`, `supporting_evidence_ids`, `caveat_if_any`, `strength`, `reasoning`

**Step 3 — Claim Generation (LLM) + Deterministic Priority:**
*Input:* Viewpoints + observations + triaged evidence.
*Task:* Generate 0–10 claims. The LLM populates all structured fields. Deterministic gates (`assignClaimPriority()`) assign final `claim_priority`. The LLM must NOT decide priority.
*Rules:* Evidence IDs are validated against allowed ID lists. Trend claims without ≥3 non-duplicate items / ≥2 time windows / ≥2 publishers → `evidence_sufficiency = "insufficient"` → `claim_priority = "rejected"`. Adoption claims without observed adversary use → rejected. Hype language without analytical change → suppressed.

*Deterministic critical gates (ALL must pass for critical):*
`evidence_sufficiency = sufficient` + `analytical_change` is non-trivial + `change_driver` is non-trivial + `broad_relevance = true` with valid basis + `multi_scope_impact = true` with ≥2 scope dimensions + `strong_viewpoint_support = true` + `strong_evidence_support = true` + no blocking limitations + `slide_driving_power = true`

*Output fields:* `claim_id`, `claim_text`, `claim_type`, `analytical_change`, `change_driver`, `signal_temporality`, `supporting_viewpoint_ids`, `supporting_observation_ids`, `supporting_evidence_ids`, `evidence_sufficiency`, `broad_relevance`, `broad_relevance_basis`, `multi_scope_impact`, `multi_scope_basis`, `strong_viewpoint_support`, `strong_evidence_support`, `blocking_limitations`, `slide_driving_power`, `caveat_if_any`, `reasoning`, `claim_priority` (deterministic)

**Deterministic Output Mapping (backward-compatible shape):**
The claim chain outputs are mapped to the existing analysis output shape for backward compatibility:
- `observations[]` → `biggest_happenings[]`
- Critical/high `category_insight` and `executive_judgment` claims → `top_insights[]` (with `claim_id`, `claim_priority`, `analytical_change` added)
- Medium `outlook` claims with `signal_temporality = emerging` → `early_signals[]`
- `recommendation` claims → `recommendations[]`
- Best `outlook` claim → `outlook{}` object
- First critical/high claim → `category_headline`

New additive fields on the output (can be ignored by existing consumers): `observations[]`, `viewpoints[]`, `claims[]`, `case_studies[]`, `claim_chain_counts{}`, `chain_version`.

**Deterministic Assembly:** `headline` (first critical/high claim text, or first observation), `overview` (1–2 sentences from happenings + top insight), `analysis_confidence` (high if ≥2 critical or 1 critical+2 high claims; medium if ≥1 critical or ≥2 high; low otherwise).

### Evidence Linking (linkAnalysisEvidence.js)

After the claim chain, every `supporting_evidence_ids[]` reference in happenings, insights, signals, recommendations, and outlook is resolved back to the full evidence object. This creates `resolved_evidence[]` and `citations[]` arrays. The citation carries: `evidence_id`, `source_id`, `title`, `url`, `publisher`, `short_label`, `source_quote`, `quote_verified`.
